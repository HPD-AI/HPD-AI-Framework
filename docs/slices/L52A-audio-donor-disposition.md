...
The ten paths changed by L52A's Silero and ElevenLabs commits are byte-identical to the corresponding paths at the V9 tip. They are compatible and require no conflict resolution. The incompatibility is in the surrounding V9 lifecycle: the later retained-STT commits were implemented against runtime types deliberately removed by current main.
