...
public class PolynomialDivisionTests
{
    ...
    public void DivisionWithRemainder_Identity()
    {
        ...
        var x = SparsePolynomial<Rational>.X;
        var dividend = x * x * x + SparsePolynomial<Rational>.C((Rational)2) * x + SparsePolynomial<Rational>.C((Rational)1);
        var divisor = x * x + SparsePolynomial<Rational>.C((Rational)1);
        var (q, r) = dividend.DivMod(divisor);
    ...
    {
        var x = SparsePolynomial<Rational>.X;
        var dividend = x * x * x + x + SparsePolynomial<Rational>.C((Rational)1);
        var divisor = x * x + SparsePolynomial<Rational>.C((Rational)1);
