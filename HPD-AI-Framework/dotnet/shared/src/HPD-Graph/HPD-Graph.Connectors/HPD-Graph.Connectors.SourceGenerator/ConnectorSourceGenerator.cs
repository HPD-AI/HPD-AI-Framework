...
namespace HPDAgent.Graph.Connectors.SourceGenerator;
...
{
    private const string AttrPrefix = "HPDAgent.Graph.Connectors.Abstractions.Attributes.";
    ...
    private static ConnectorInfo? ResolveConnector(Compilation compilation, TypeDeclarationSyntax node)
    {
        ...
        return new ConnectorInfo(
            ...
            symbol.ContainingNamespace.IsGlobalNamespace ? "HPDAgent.Graph.Connectors.Generated" : symbol.ContainingNamespace.ToDisplayString(),
    ...
    private static string GenerateJsonContext(Model model)
    {
        ...
        var sb = Header();
        sb.AppendLine("namespace HPDAgent.Graph.Connectors.Generated;");
    ...
    private static string GenerateDescriptors(Model model)
    {
        ...
        sb.AppendLine("{");
        sb.AppendLine("    public static HPDAgent.Graph.Connectors.Abstractions.Descriptors.ConnectorPackageDescriptor Create()");
        sb.AppendLine("    {");
        sb.AppendLine("        return new HPDAgent.Graph.Connectors.Abstractions.Descriptors.ConnectorPackageDescriptor");
        sb.AppendLine("        {");
        ...
        foreach (var app in model.Connections.Select(c => c.AppId).Concat(model.Sources.Select(s => s.AppId)).Distinct(StringComparer.Ordinal))
            sb.AppendLine($"                new HPDAgent.Graph.Connectors.Abstractions.Descriptors.AppDescriptor {{ AppId = \"{Esc(app)}\", DisplayName = \"{Esc(ToDisplayName(app))}\" }},");
        sb.AppendLine("            ],");
        sb.AppendLine("            Connections =");
        sb.AppendLine("            [");
        foreach (var c in model.Connections)
        {
            sb.AppendLine("                new HPDAgent.Graph.Connectors.Abstractions.Connections.ConnectionDescriptor");
            sb.AppendLine("                {");
            sb.AppendLine($"                    ConnectionType = \"{Esc(c.ConnectionType)}\", AppId = \"{Esc(c.AppId)}\", DisplayName = \"{Esc(c.DisplayName)}\",");
            sb.AppendLine($"                    AuthKind = HPDAgent.Graph.Connectors.Abstractions.Connections.ConnectionAuthKind.{c.AuthKind}");
            sb.AppendLine("                },");
        }
        sb.AppendLine("            ],");
        ...
        foreach (var config in model.Configs)
        {
            sb.AppendLine("                new HPDAgent.Graph.Connectors.Abstractions.Configuration.ConnectorConfigDescriptor");
            sb.AppendLine("                {");
        ...
        {
            sb.AppendLine("                new HPDAgent.Graph.Connectors.Abstractions.Sources.WorkflowSourceDescriptor");
            sb.AppendLine("                {");
            sb.AppendLine($"                    SourceType = \"{Esc(s.SourceType)}\", AppId = \"{Esc(s.AppId)}\", DisplayName = \"{Esc(s.DisplayName)}\",");
            sb.AppendLine($"                    TriggerKind = HPDAgent.Graph.Connectors.Abstractions.Sources.SourceTriggerKind.{s.TriggerKind}");
            sb.AppendLine("                },");
        }
        sb.AppendLine("            ],");
        ...
        {
            sb.AppendLine("                new HPDAgent.Graph.Connectors.Abstractions.Actions.ConnectorActionDescriptor");
            sb.AppendLine("                {");
            ...
            sb.AppendLine($"                    Traits = HPDAgent.Graph.Connectors.Abstractions.Actions.ConnectorOperationTraits.{a.Traits}");
            sb.AppendLine("                },");
        }
        foreach (var operation in model.OpenApiOperations)
        {
            ...
            sb.AppendLine($"                    Traits = HPDAgent.Graph.Connectors.Abstractions.Actions.ConnectorOperationTraits.{InferOpenApiTraits(operation.Method)},");
        ...
        foreach (var a in model.Actions)
        {
            sb.AppendLine("                new HPDAgent.Graph.Abstractions.Discovery.HandlerDescriptor");
            sb.AppendLine("                {");
            sb.AppendLine($"                    HandlerName = \"{Esc(a.HandlerName)}\", DisplayName = \"{Esc(a.DisplayName)}\", Domain = \"{Esc(FirstSegment(a.ActionType))}\",");
            sb.AppendLine($"                    HandlerType = typeof({a.FullyQualifiedName}).FullName!, ContextType = typeof(HPDAgent.Graph.Core.Context.GraphContext).FullName!,");
            sb.AppendLine("                    Metadata = new System.Collections.Generic.Dictionary<string, string>");
            sb.AppendLine("                    {");
        ...
        foreach (var operation in model.OpenApiOperations)
        {
            ...
            sb.AppendLine("                    HandlerType = typeof(HPDAgent.Graph.Connectors.OpenApi.Handlers.OpenApiCallOperationHandler).FullName!, ContextType = typeof(HPDAgent.Graph.Core.Context.GraphContext).FullName!,");
            AppendOpenApiInputs(sb, operation, 20);
            sb.AppendLine("                    Outputs =");
            sb.AppendLine("                    [");
            sb.AppendLine("                        new HPDAgent.Graph.Abstractions.Discovery.SocketDescriptor { Name = \"response\", TypeName = typeof(object).FullName!, Direction = HPDAgent.Graph.Abstractions.Discovery.SocketDirection.Output, Required = false },");
            sb.AppendLine("                        new HPDAgent.Graph.Abstractions.Discovery.SocketDescriptor { Name = \"error\", TypeName = typeof(object).FullName!, Direction = HPDAgent.Graph.Abstractions.Discovery.SocketDirection.Output, Required = false },");
            sb.AppendLine("                    ],");
            sb.AppendLine("                    Metadata = new System.Collections.Generic.Dictionary<string, string>");
            sb.AppendLine("                    {");
    ...
    private static string GenerateRegistration(Model model)
    {
        ...
        sb.AppendLine("    {");
        sb.AppendLine("        HPDAgent.Graph.Connectors.Core.DependencyInjection.ConnectorCoreServiceCollectionExtensions.AddHPDGraphConnectorsCore(services);");
        if (model.OpenApiOperations.Count > 0)
            sb.AppendLine("        HPDAgent.Graph.Connectors.OpenApi.DependencyInjection.OpenApiConnectorServiceCollectionExtensions.AddHPDGraphConnectorsOpenApi(services);");
        ...
        sb.AppendLine("        services.AddSingleton(HPDAgent.Graph.Connectors.Generated.GeneratedConnectorDescriptors.Create());");
        foreach (var a in model.Actions)
            sb.AppendLine($"        services.AddScoped<HPDAgent.Graph.Abstractions.Handlers.IGraphNodeHandler<HPDAgent.Graph.Core.Context.GraphContext>, HPDAgent.Graph.Connectors.Generated.{Safe(a.ActionType)}ActionHandler>();");
        foreach (var operation in model.OpenApiOperations)
        {
            sb.AppendLine($"        services.AddSingleton(new HPDAgent.Graph.Connectors.OpenApi.Catalog.OpenApiOperationRegistration(\"{Esc(operation.ConnectorId)}\", HPDAgent.Graph.Connectors.Generated.{Safe(operation.ConnectorId + "." + operation.OperationId)}OpenApiOperationFactory.Create()));");
            sb.AppendLine($"        services.AddScoped<HPDAgent.Graph.Abstractions.Handlers.IGraphNodeHandler<HPDAgent.Graph.Core.Context.GraphContext>, HPDAgent.Graph.Connectors.Generated.{Safe(operation.ConnectorId + "." + operation.OperationId)}OpenApiActionHandler>();");
        }
        foreach (var s in model.Sources)
        {
            var wrapper = $"HPDAgent.Graph.Connectors.Generated.{Safe(s.SourceType)}SourceProvider";
            sb.AppendLine($"        services.AddSingleton<{wrapper}>();");
            sb.AppendLine($"        services.AddSingleton<HPDAgent.Graph.Connectors.Abstractions.Sources.IWorkflowSourceProvider>(sp => sp.GetRequiredService<{wrapper}>());");
            if (s.HasFromWebhook)
                sb.AppendLine($"        services.AddSingleton<HPDAgent.Graph.Connectors.Abstractions.Sources.IWebhookWorkflowSourceProvider>(sp => sp.GetRequiredService<{wrapper}>());");
            if (s.HasPoll)
                sb.AppendLine($"        services.AddSingleton<HPDAgent.Graph.Connectors.Abstractions.Sources.IPollingWorkflowSourceProvider>(sp => sp.GetRequiredService<{wrapper}>());");
        }
        foreach (var option in model.Options)
            sb.AppendLine($"        services.AddSingleton<HPDAgent.Graph.Connectors.Abstractions.Options.IConnectorOptionProvider, HPDAgent.Graph.Connectors.Generated.{Safe(option.Name)}OptionProvider>();");
        foreach (var asset in model.AssetCatalogs)
            sb.AppendLine($"        services.AddSingleton<HPDAgent.Graph.Connectors.Abstractions.Assets.IConnectorAssetCatalogProvider, HPDAgent.Graph.Connectors.Generated.{Safe(asset.Name)}AssetCatalogProvider>();");
        foreach (var materialization in model.Materializations)
            sb.AppendLine($"        services.AddSingleton<HPDAgent.Graph.Connectors.Abstractions.Materialization.IConnectorMaterializationProvider, HPDAgent.Graph.Connectors.Generated.{Safe(materialization.Type)}MaterializationProvider>();");
        foreach (var check in model.Checks)
            sb.AppendLine($"        services.AddSingleton<HPDAgent.Graph.Connectors.Abstractions.Materialization.IConnectorAssetCheckProvider, HPDAgent.Graph.Connectors.Generated.{Safe(check.Name)}AssetCheckProvider>();");
        foreach (var io in model.IoManagers)
            sb.AppendLine($"        services.AddSingleton<HPDAgent.Graph.Connectors.Abstractions.IO.IArtifactIOManager, {io.FullyQualifiedName}>();");
    ...
    private static void GenerateActionWrapper(StringBuilder sb, Model model, ActionInfo action)
    {
        ...
        sb.AppendLine($"internal sealed class {name} : HPDAgent.Graph.Abstractions.Handlers.IGraphNodeHandler<HPDAgent.Graph.Core.Context.GraphContext>");
        sb.AppendLine("{");
        ...
        sb.AppendLine("    public async System.Threading.Tasks.Task<HPDAgent.Graph.Abstractions.Execution.NodeExecutionResult> ExecuteAsync(HPDAgent.Graph.Core.Context.GraphContext context, HPDAgent.Graph.Abstractions.Handlers.HandlerInputs inputs, System.Threading.CancellationToken cancellationToken = default)");
        sb.AppendLine("    {");
        ...
        else
        {
            ...
            sb.AppendLine("        return HPDAgent.Graph.Abstractions.Execution.NodeExecutionResult.Success.Single(output, System.TimeSpan.Zero, new HPDAgent.Graph.Abstractions.Execution.NodeExecutionMetadata());");
        }
        sb.AppendLine("    }");
        sb.AppendLine("    private static T DeserializeNodeConfig<T>(HPDAgent.Graph.Core.Context.GraphContext context) where T : new()");
        sb.AppendLine("    {");
    ...
    private static void GenerateOpenApiOperationWrapper(StringBuilder sb, OpenApiOperationInfo operation)
    {
        ...
        sb.AppendLine("    }");
        sb.AppendLine("}");
        sb.AppendLine($"internal sealed class {safeName}OpenApiActionHandler : HPDAgent.Graph.Abstractions.Handlers.IGraphNodeHandler<HPDAgent.Graph.Core.Context.GraphContext>");
        sb.AppendLine("{");
        sb.AppendLine("    private readonly HPDAgent.Graph.Connectors.OpenApi.Catalog.IOpenApiOperationCatalog _operations;");
        sb.AppendLine("    private readonly HPDAgent.Graph.Connectors.Abstractions.Connections.IConnectionProvider _connections;");
        sb.AppendLine("    private readonly System.Collections.Generic.IEnumerable<HPDAgent.Graph.Connectors.OpenApi.IOpenApiConnectionAdapter> _adapters;");
        sb.AppendLine($"    public {safeName}OpenApiActionHandler(HPDAgent.Graph.Connectors.OpenApi.Catalog.IOpenApiOperationCatalog operations, HPDAgent.Graph.Connectors.Abstractions.Connections.IConnectionProvider connections, System.Collections.Generic.IEnumerable<HPDAgent.Graph.Connectors.OpenApi.IOpenApiConnectionAdapter> adapters)");
        sb.AppendLine("    {");
        ...
        sb.AppendLine("    public System.Threading.Tasks.Task<HPDAgent.Graph.Abstractions.Execution.NodeExecutionResult> ExecuteAsync(HPDAgent.Graph.Core.Context.GraphContext context, HPDAgent.Graph.Abstractions.Handlers.HandlerInputs inputs, System.Threading.CancellationToken cancellationToken = default)");
        sb.AppendLine("    {");
        sb.AppendLine("        return new HPDAgent.Graph.Connectors.OpenApi.Handlers.OpenApiCallOperationHandler(_operations, _connections, _adapters).ExecuteAsync(context, inputs, cancellationToken);");
        sb.AppendLine("    }");
        sb.AppendLine("}");
    }
    ...
    private static void GenerateSourceWrapper(StringBuilder sb, Model model, SourceInfo source)
    {
        ...
        sb.AppendLine($"internal sealed class {name} : HPDAgent.Graph.Connectors.Abstractions.Sources.IWebhookWorkflowSourceProvider, HPDAgent.Graph.Connectors.Abstractions.Sources.IPollingWorkflowSourceProvider");
        sb.AppendLine("{");
        sb.AppendLine("    private readonly HPDAgent.Graph.Connectors.Abstractions.Sources.IWorkflowSourceDispatcher _dispatcher;");
        sb.AppendLine("    private readonly System.IServiceProvider _services;");
        sb.AppendLine($"    public {name}(HPDAgent.Graph.Connectors.Abstractions.Sources.IWorkflowSourceDispatcher dispatcher, System.IServiceProvider services)");
        sb.AppendLine("    {");
        ...
        sb.AppendLine("    public System.Threading.Tasks.Task<System.Collections.Generic.IReadOnlyList<HPDAgent.Graph.Connectors.Abstractions.Sources.WorkflowSourceStatus>> GetStatusAsync(System.Threading.CancellationToken ct = default) => System.Threading.Tasks.Task.FromResult<System.Collections.Generic.IReadOnlyList<HPDAgent.Graph.Connectors.Abstractions.Sources.WorkflowSourceStatus>>(System.Array.Empty<HPDAgent.Graph.Connectors.Abstractions.Sources.WorkflowSourceStatus>());");
        sb.AppendLine("    public async System.Threading.Tasks.Task ReceiveAsync(HPDAgent.Graph.Connectors.Abstractions.Sources.WorkflowSource source, HPDAgent.Graph.Connectors.Abstractions.Sources.WebhookEnvelope envelope, System.Threading.CancellationToken ct = default)");
        sb.AppendLine("    {");
        if (source.HasFromWebhook)
        {
            ...
            sb.AppendLine("        await _dispatcher.DispatchAsync(new HPDAgent.Graph.Connectors.Abstractions.Events.WorkflowSourceEmittedEvent { SourceId = source.SourceId, GraphId = source.GraphId, SourceType = source.SourceType, Payload = payload, EventId = candidate.EventId, Summary = candidate.Summary, OccurredAt = candidate.OccurredAt ?? System.DateTimeOffset.UtcNow, DedupeStrategy = candidate.DedupeStrategy ?? HPDAgent.Graph.Connectors.Abstractions.Sources.DedupeStrategy.Unique, Metadata = candidate.Metadata }, ct).ConfigureAwait(false);");
        }
        sb.AppendLine("    }");
        sb.AppendLine("    public async System.Threading.Tasks.Task PollAsync(HPDAgent.Graph.Connectors.Abstractions.Sources.WorkflowSource source, HPDAgent.Graph.Connectors.Abstractions.Sources.WorkflowSourceState? state, System.Threading.CancellationToken ct = default)");
        sb.AppendLine("    {");
        if (source.HasPoll)
        {
            if (source.ConfigType is not null)
        ...
        string sourceIdVariable)
    {
        if (methodName == "UnregisterAsync")
            ...
            sb.AppendLine($"    public async System.Threading.Tasks.Task {methodName}(HPDAgent.Graph.Connectors.Abstractions.Sources.WorkflowSource source, System.Threading.CancellationToken ct = default)");
    ...
    private static void GenerateOptionWrapper(StringBuilder sb, OptionInfo option)
    {
        ...
        sb.AppendLine($"internal sealed class {name} : HPDAgent.Graph.Connectors.Abstractions.Options.IConnectorOptionProvider");
        sb.AppendLine("{");
        sb.AppendLine("    private readonly System.IServiceProvider _services;");
        sb.AppendLine($"    public {name}(System.IServiceProvider services) => _services = services;");
        sb.AppendLine($"    public string OptionProviderName => \"{Esc(option.Name)}\";");
        sb.AppendLine("    public async System.Threading.Tasks.Task<System.Collections.Generic.IReadOnlyList<HPDAgent.Graph.Connectors.Abstractions.Options.ConnectorOption>> GetOptionsAsync(HPDAgent.Graph.Connectors.Abstractions.Options.ConnectorOptionRequest request, System.Threading.CancellationToken ct = default)");
        sb.AppendLine("    {");
        ...
        sb.AppendLine("    }");
        sb.AppendLine("    private static T AdaptOptionRequest<T>(HPDAgent.Graph.Connectors.Abstractions.Options.ConnectorOptionRequest request)");
        sb.AppendLine("    {");
    ...
    private static void GenerateAssetCatalogWrapper(StringBuilder sb, Model model, AssetCatalogInfo asset)
    {
        ...
        sb.AppendLine($"internal sealed class {name} : HPDAgent.Graph.Connectors.Abstractions.Assets.IConnectorAssetCatalogProvider");
        sb.AppendLine("{");
        sb.AppendLine("    private readonly System.IServiceProvider _services;");
        sb.AppendLine($"    public {name}(System.IServiceProvider services) => _services = services;");
        sb.AppendLine($"    public string CatalogProviderName => \"{Esc(asset.Name)}\";");
        sb.AppendLine("    public async System.Threading.Tasks.Task<System.Collections.Generic.IReadOnlyList<HPDAgent.Graph.Connectors.Abstractions.Assets.ConnectorAssetDescriptor>> LoadAssetsAsync(HPDAgent.Graph.Connectors.Abstractions.Assets.ConnectorAssetCatalogRequest request, System.Threading.CancellationToken ct = default)");
        sb.AppendLine("    {");
        if (asset.LoadMethod is null)
            sb.AppendLine("        return System.Array.Empty<HPDAgent.Graph.Connectors.Abstractions.Assets.ConnectorAssetDescriptor>();");
        else
        {
    ...
    private static void GenerateMaterializationWrapper(StringBuilder sb, Model model, MaterializationInfo materialization)
    {
        ...
        sb.AppendLine($"internal sealed class {name} : HPDAgent.Graph.Connectors.Abstractions.Materialization.IConnectorMaterializationProvider");
        sb.AppendLine("{");
        sb.AppendLine("    private readonly System.IServiceProvider _services;");
        sb.AppendLine($"    public {name}(System.IServiceProvider services) => _services = services;");
        sb.AppendLine($"    public string MaterializationType => \"{Esc(materialization.Type)}\";");
        sb.AppendLine("    public async System.Collections.Generic.IAsyncEnumerable<HPD.Events.Event> MaterializeAsync(HPDAgent.Graph.Connectors.Abstractions.Materialization.ConnectorMaterializationContext context, [System.Runtime.CompilerServices.EnumeratorCancellation] System.Threading.CancellationToken ct = default)");
        sb.AppendLine("    {");
    ...
    private static void GenerateCheckWrapper(StringBuilder sb, CheckInfo check)
    {
        ...
        sb.AppendLine($"internal sealed class {name} : HPDAgent.Graph.Connectors.Abstractions.Materialization.IConnectorAssetCheckProvider");
        sb.AppendLine("{");
        sb.AppendLine($"    public string CheckName => \"{Esc(check.Name)}\";");
        sb.AppendLine("    public async System.Collections.Generic.IAsyncEnumerable<HPD.Events.Event> CheckAsync(HPDAgent.Graph.Connectors.Abstractions.Materialization.ConnectorMaterializationContext context, [System.Runtime.CompilerServices.EnumeratorCancellation] System.Threading.CancellationToken ct = default)");
        sb.AppendLine("    {");
    ...
    private static string GenerateConnectorPartial(ConnectorInfo connector, IReadOnlyList<SourceInfo> sources)
    {
        ...
        sb.AppendLine("        return await HandleWebhookAsync(new HPDAgent.Graph.Connectors.Abstractions.Sources.WebhookEnvelope");
        sb.AppendLine("        {");
        ...
        sb.AppendLine("    }");
        sb.AppendLine("    public async System.Threading.Tasks.Task<Microsoft.AspNetCore.Http.IResult> HandleWebhookAsync(HPDAgent.Graph.Connectors.Abstractions.Sources.WebhookEnvelope envelope, System.IServiceProvider services, System.Threading.CancellationToken ct = default)");
        sb.AppendLine("    {");
        ...
        sb.AppendLine("        };");
        sb.AppendLine("        var store = services.GetRequiredService<HPDAgent.Graph.Connectors.Abstractions.Sources.IWorkflowSourceStore>();");
        sb.AppendLine("        var providers = services.GetServices<HPDAgent.Graph.Connectors.Abstractions.Sources.IWebhookWorkflowSourceProvider>();");
    ...
    private static string GenerateBuilderExtensions(Model model)
    {
        ...
        foreach (var action in model.Actions.Where(a => a.ConfigType is not null))
        {
            ...
            sb.AppendLine($"    public static HPDAgent.Graph.Core.Builders.GraphBuilder {method}(this HPDAgent.Graph.Core.Builders.GraphBuilder builder, string id, string name, {action.ConfigType} config)");
            sb.AppendLine("    {");
            sb.AppendLine($"        return builder.AddHandlerNode(id, name, \"{Esc(action.HandlerName)}\", node => node.WithConfig(HPDAgent.Graph.Connectors.Generated.HPDGraphConnectorJsonContext.SerializeToElement(config)));");
            sb.AppendLine("    }");
        }
        foreach (var source in model.Sources.Where(s => s.ConfigType is not null))
        {
            var method = "Add" + ToPascal(source.SourceType) + "Source";
            sb.AppendLine($"    public static HPDAgent.Graph.Core.Builders.GraphBuilder {method}(this HPDAgent.Graph.Core.Builders.GraphBuilder builder, string id, string name, {source.ConfigType} config)");
            sb.AppendLine("    {");
            sb.AppendLine($"        return builder.AddHandlerNode(id, name, \"{Esc(source.SourceType)}\", node => node.WithConfig(HPDAgent.Graph.Connectors.Generated.HPDGraphConnectorJsonContext.SerializeToElement(config)));");
            sb.AppendLine("    }");
        }
        foreach (var operation in model.OpenApiOperations)
        {
            var method = "Add" + ToPascal(operation.ConnectorId + "." + operation.OperationId) + "Node";
            sb.AppendLine($"    public static HPDAgent.Graph.Core.Builders.GraphBuilder {method}(this HPDAgent.Graph.Core.Builders.GraphBuilder builder, string id, string name, string connectionId, System.Text.Json.JsonElement? arguments = null, System.Uri? serverUrlOverride = null)");
            sb.AppendLine("    {");
            ...
            sb.AppendLine("            new HPDAgent.Graph.Connectors.OpenApi.Handlers.OpenApiCallOperationConfig");
            sb.AppendLine("            {");
            ...
            sb.AppendLine("            },");
            sb.AppendLine("            HPDAgent.Graph.Connectors.OpenApi.OpenApiConnectorJsonSerializerContext.Default.OpenApiCallOperationConfig)));");
            sb.AppendLine("    }");
        }
        sb.AppendLine("}");
        return sb.ToString();
        ...
        string? typedOptionRequestVariable = null)
    {
        ...
        foreach (var p in method.Parameters)
        {
            ...
            if (configType is not null && SymbolEqualityByName(type, configType))
            ...
            else if (type == "global::HPDAgent.Graph.Connectors.Abstractions.Options.ConnectorOptionRequest" && requestVariable is not null)
            ...
            else if (type == "global::HPDAgent.Graph.Connectors.Abstractions.Assets.ConnectorAssetCatalogRequest" && requestVariable is not null)
                args.Add(requestVariable);
            else if (type == "global::HPDAgent.Graph.Connectors.Abstractions.Sources.WebhookEnvelope" && envelopeVariable is not null)
                args.Add(envelopeVariable);
            else if (type == "global::HPDAgent.Graph.Connectors.Abstractions.Materialization.ConnectorMaterializationContext" && materializationContext is not null)
                args.Add(materializationContext);
            else if (type == "global::HPDAgent.Graph.Connectors.Abstractions.Sources.WorkflowSource" && workflowSourceVariable is not null)
            ...
            else if (type == "global::HPDAgent.Graph.Connectors.Abstractions.Connections.IConnectionProvider" && serviceProviderVariable is not null)
                args.Add($"{serviceProviderVariable}.GetRequiredService<HPDAgent.Graph.Connectors.Abstractions.Connections.IConnectionProvider>()");
            else if (type == "global::HPDAgent.Graph.Connectors.Abstractions.Connections.IConnectionProvider" && materializationContext is not null)
                args.Add(materializationContext + ".Connections");
            else if (type == "global::HPDAgent.Graph.Connectors.Abstractions.Connections.ResolvedConnection" && connectionIdExpression is not null && serviceProviderVariable is not null)
                args.Add($"await ResolveConnectionAsync({connectionIdExpression}, {cancellationTokenVariable}).ConfigureAwait(false)");
            else if (type == "global::HPDAgent.Graph.Connectors.Abstractions.Connections.ResolvedConnection" && connectionIdExpression is not null && materializationContext is not null)
            ...
            else if (type == "global::HPDAgent.Graph.Abstractions.Artifacts.ArtifactKey" && materializationContext is not null)
                args.Add(materializationContext + ".ArtifactKey");
            else if (graphContextVariable is not null && type == "global::HPDAgent.Graph.Core.Context.GraphContext")
                ...
                args.Add($"await {serviceProviderVariable}.GetRequiredService<HPDAgent.Graph.Connectors.Abstractions.Connections.IConnectorClientFactory<{type}>>().CreateAsync(await ResolveConnectionAsync({connectionIdExpression}, {cancellationTokenVariable}).ConfigureAwait(false), {cancellationTokenVariable}).ConfigureAwait(false)");
    ...
    private static string? FindTypedOptionRequestType(IMethodSymbol method)
    {
        foreach (var parameter in method.Parameters)
        {
            ...
            if (type == "global::System.Threading.CancellationToken" ||
                type == "global::HPDAgent.Graph.Connectors.Abstractions.Options.ConnectorOptionRequest" ||
                type == "global::HPDAgent.Graph.Connectors.Abstractions.Connections.IConnectionProvider" ||
    ...
    {
        sb.AppendLine("    private async System.Threading.Tasks.ValueTask<HPDAgent.Graph.Connectors.Abstractions.Connections.ResolvedConnection> ResolveConnectionAsync(string connectionId, System.Threading.CancellationToken ct)");
        sb.AppendLine("    {");
        sb.AppendLine("        var provider = _services.GetRequiredService<HPDAgent.Graph.Connectors.Abstractions.Connections.IConnectionProvider>();");
        sb.AppendLine("        return await ResolveConnectionAsync(provider, connectionId, ct).ConfigureAwait(false);");
        sb.AppendLine("    }");
        sb.AppendLine("    private static async System.Threading.Tasks.ValueTask<HPDAgent.Graph.Connectors.Abstractions.Connections.ResolvedConnection> ResolveConnectionAsync(HPDAgent.Graph.Connectors.Abstractions.Connections.IConnectionProvider provider, string connectionId, System.Threading.CancellationToken ct)");
        sb.AppendLine("    {");
    ...
    private static void AppendFields(StringBuilder sb, Model model, string? configType, int spaces)
    {
        ...
        foreach (var field in config.Fields)
        {
            sb.AppendLine($"{indent}    new HPDAgent.Graph.Connectors.Abstractions.Configuration.ConnectorFieldDescriptor");
            sb.AppendLine($"{indent}    {{");
    ...
    private static void AppendOpenApiInputs(StringBuilder sb, OpenApiOperationInfo operation, int spaces)
    {
        ...
        sb.AppendLine($"{indent}[");
        sb.AppendLine($"{indent}    new HPDAgent.Graph.Abstractions.Discovery.SocketDescriptor {{ Name = \"arguments\", TypeName = typeof(object).FullName!, Direction = HPDAgent.Graph.Abstractions.Discovery.SocketDirection.Input, Required = false }},");
        foreach (var field in operation.Parameters.Concat(operation.PayloadProperties))
        {
            sb.AppendLine($"{indent}    new HPDAgent.Graph.Abstractions.Discovery.SocketDescriptor");
            sb.AppendLine($"{indent}    {{");
            sb.AppendLine($"{indent}        Name = \"{Esc(field.Name)}\",");
            sb.AppendLine($"{indent}        TypeName = \"{Esc(OpenApiTypeName(field.TypeName))}\",");
            sb.AppendLine($"{indent}        Direction = HPDAgent.Graph.Abstractions.Discovery.SocketDirection.Input,");
            sb.AppendLine($"{indent}        Required = {(field.Required ? "true" : "false")},");
    ...
    private static string InferOpenApiTraits(string method)
        => method switch
        {
            "GET" or "HEAD" => "ReadOnly | HPDAgent.Graph.Connectors.Abstractions.Actions.ConnectorOperationTraits.Idempotent",
            "DELETE" => "Destructive",
            "PUT" => "Idempotent",
            _ => "OpenWorld"
        };
