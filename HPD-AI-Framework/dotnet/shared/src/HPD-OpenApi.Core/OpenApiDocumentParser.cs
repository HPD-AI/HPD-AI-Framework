...
public sealed class OpenApiDocumentParser
{
        ...
        OpenApiDocument document, OpenApiCoreConfig config, Uri? baseUri = null)
    {
        ...
        {
            if (pathPair.Value is not { } pathItem)
                continue;
            ...
            foreach (var opPair in pathItem.Operations)
            {
                ...
                var parameters = CreateParameters(
                    op.OperationId,
                    (op.Parameters ?? []).Union(pathItem.Parameters ?? [], ParameterComparer.Instance));
                ...
                OpenApiServer? effectiveApiServer =
                    ...
                    ?? pathItem.Servers?.FirstOrDefault()
                    ?? globalServer;
        ...
        foreach (var param in parameters)
        {
            var name = param.Name
                ?? throw new OpenApiParseException(
                    $"A parameter in operation '{operationId ?? "(unnamed)"}' is missing a name.");

            if (param.In is null)
                throw new OpenApiParseException(
                    $"Parameter location of '{name}' in operation " +
        ...
        string? operationId, IOpenApiSchema? schema, int level = 0)
    {
        ...
        var allRequired = new HashSet<string>(schema.Required ?? Enumerable.Empty<string>());
        foreach (var s in schemas.Skip(1))
            foreach (var r in s.Required ?? Enumerable.Empty<string>())
                allRequired.Add(r);
        ...
        {
            if (subSchema.Properties is null)
                continue;
