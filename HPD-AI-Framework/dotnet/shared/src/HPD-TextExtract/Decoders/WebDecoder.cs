...
namespace HPD.TextExtract.Decoders
{
    ...
    public sealed class WebDecoder : IContentDecoder, IWebDecoder
    {
        ...
        {
            _log.LogDebug("Extracting text from HTML input {InputKind}", input.Kind);

            if (input.Kind == ContentInputKind.Url)
            {
                return await DecodeFromUrlAsync(input.Url!, options, cancellationToken).ConfigureAwait(false);
            }
        ...
        public async ValueTask<ExtractionResult> DecodeFromUrlAsync(
            Uri url,
            ExtractionOptions? options = null,
            CancellationToken cancellationToken = default)
        {
            ...
            catch (HttpRequestException ex)
            {
                ...
                var extraction = new ExtractionResult(result);
                extraction.Diagnostics.Warnings.Add(ex.Message);
                return extraction;
            }
        }

        private async ValueTask<ExtractionResult> ParseHtmlContent(string html, CancellationToken cancellationToken)
        {
            var result = new FileContent(MimeTypes.PlainText);
            ...
            {
                result.Sections.Add(new Chunk(cleanedText, 1, Chunk.Meta(sentencesAreComplete: true, source: TextExtractionSource.Native)));
            }

            return new ExtractionResult(result)
            {
                Pages = result.Sections.Select(section => new ExtractedPage
                {
