...
public sealed class SimulationSessionBehaviorTests
{
    [Fact]
    ...
    [Fact]
    public void SimulationSession_Run_ReplaysBarHistoryDeterministically()
    {
    ...
    [Fact]
    public void FastVectorApproximation_FillsTouchedLimitInsideEventBoundaryAndDispatchesHooks()
    {
    ...
    [Fact]
    public void ExecutionAllocationGuard_WarmsEachExecutionTransitionPathSeparately()
    {
        RepeatedFillStrategy.Reset();
        var history = SharedHistory.Load([CreateBarClosed(123m), CreateBarClosed(124m)]);

        var result = Rhodium.Simulate<RepeatedFillStrategy>()
            .WithHistory(history)
            .WithMatchingFidelity(MatchingFidelity.FastVectorApproximation)
            .Run();

        Assert.Equal(2, RepeatedFillStrategy.AcceptedCount);
        Assert.Equal(2, RepeatedFillStrategy.FilledCount);
        Assert.Equal(2, result.ExecutionEvents.OfType<OrderAccepted>().Count());
        Assert.Equal(2, result.ExecutionEvents.OfType<OrderFilled>().Count());
        Assert.Equal(new Qty(2m), result.Runs.Single().FinalSnapshot.GetPositions().ToArray().Single().Quantity);
    }

    [Fact]
    public void SimulationSession_BuildsRunTearSheetsFromExchangeFills()
    {
    ...
    [Fact]
    public void QueueAccurateMatching_DoesNotFillOrderOnSubmissionBar()
    {
        var history = SharedHistory.Load([CreateBarClosed(123m)]);
    ...
    [Theory]
    [InlineData(MatchingFidelity.FastVectorApproximation, MarketStatus.PreOpen)]
    [InlineData(MatchingFidelity.FastVectorApproximation, MarketStatus.Closed)]
    [InlineData(MatchingFidelity.FastVectorApproximation, MarketStatus.Halted)]
    [InlineData(MatchingFidelity.QueueAccurate, MarketStatus.PreOpen)]
    [InlineData(MatchingFidelity.QueueAccurate, MarketStatus.Closed)]
    [InlineData(MatchingFidelity.QueueAccurate, MarketStatus.Halted)]
    public void MatchingFidelity_RejectOrdersWhenInitialMarketIsNotOpen(
        MatchingFidelity fidelity,
        MarketStatus status)
    {
        RejectedMarketStatusStrategy.Reset();
        var config = (fidelity == MatchingFidelity.FastVectorApproximation
    ...
    [Fact]
    public void FastVectorApproximation_FillsStopMarketWhenBarTouchesStop()
    {
    ...
    [Fact]
    public void FastVectorApproximation_FillsStopLimitWhenStopAndLimitTouch()
    {
    ...
    [Fact]
    public void QueueAccurateMatching_FillsStopLimitOnLaterTouchedBar()
    {
        StopLimitStrategy.Reset();
        var history = SharedHistory.Load(
        [
    ...
    [Fact]
    public void QueueAccurateMatching_FixedBarOrderingDoesNotFillSellStopLimitWhenLimitTouchedBeforeStop()
    {
    ...
    [Fact]
    public void FastVectorApproximation_AppliesConfiguredSlippageAndFees()
    {
        VectorFillStrategy.Reset();
        var history = SharedHistory.Load([CreateBarClosed(123m)]);
    ...
    [Fact]
    public void FastVectorApproximation_AppliesConfiguredMakerPriceImprovement()
    {
        VectorFillStrategy.Reset();
        var history = SharedHistory.Load([CreateBarClosed(123m)]);
        var config = SimulationConfig.Instant() with
    ...
    [Fact]
    public void QueueAccurateMatching_PartiallyFillsFromTradeSizes()
    {
    ...
    [Fact]
    public void QueueAccurateMatching_TailQueueAdvancesBeforeFill()
    {
        PartialFillStrategy.Reset();
        var history = SharedHistory.Load(
        [
    ...
    [Fact]
    public void FastVectorApproximation_IgnoresPartialFillBehaviorAndFillsFullQuantity()
    {
        PartialFillStrategy.Reset();
    ...
    [Fact]
    public void FastVectorFinalists_ValidateInQueueModeWithSameGeneratedStrategy()
    {
        var grid = ParameterGrid.Create()
    ...
    [Fact]
    public void QueueAccurateAndFastVector_DispatchSameExecutionAndPositionHooksWhenFilled()
    {
        var vectorCounts = RunLifecycleParitySimulation(
            MatchingFidelity.FastVectorApproximation,
    ...
    [Fact]
    public void SimulationSession_ReplaysLifecycleEventsThroughOperationalHooks()
    {
    ...
    private static SimulationResult RunSingleBarSimulation()
        => Rhodium.Simulate<SimulationBuyingStrategy>()
...
internal sealed partial class RepeatedFillStrategy : Strategy
{
    private AssetId _spy;
    ...
    {
        if (bar.AssetId == _spy)
            bar.Buy(new Qty(1m), Execution.Limit().At(new Price(200m, Currency.USD)));
    }
