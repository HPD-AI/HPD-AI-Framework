...
public final class HelperService {
    ...
    public func handle(_ request: HelperEnvelope) -> [String: Any] {
        ...
        case .unitStatus:
            return unitResponse(for: request, operation: .unitStatus, phase: 3)
        case .unitStop:
            return unitResponse(for: request, operation: .unitStop, phase: 7)
        case .unitDelete:
    ...
    private func unitResponse(for request: HelperEnvelope, operation: Operation, phase: Int) -> [String: Any] {
        let payload = unitPayload(from: request, operation: operation)
        let unitId = VmConfigurationValidationRequest.string(payload["UnitId"]) ??
            VmConfigurationValidationRequest.string(request.raw["ResourceId"]) ??
            "unknown-unit"
        let workingDirectory = VmConfigurationValidationRequest.string(payload["WorkingDirectory"]) ?? "/tmp"

        return responseBase(
            for: request,
            operation: operation,
            status: .ok,
            schema: HelperProtocol.unitResponseSchema
        )
        .merging([
            "UnitStatusResponse": [
                "UnitId": unitId,
    ...
    private func unitPayload(from request: HelperEnvelope, operation: Operation) -> [String: Any] {
        switch operation {
        case .unitEnsure:
            return request.raw["UnitEnsureRequest"] as? [String: Any] ?? [:]
        case .unitStatus, .unitStop, .unitDelete:
            return request.raw["UnitLifecycleRequest"] as? [String: Any] ?? [:]
        default:
            return [:]
        }
    }
