...
internal static class ParentProxyResolver
{
    public static ParentProxyResolution Resolve(
        Uri destination,
        ParentProxyPolicy? explicitConfig = null,
        IReadOnlyDictionary<string, string?>? environment = null)
    {
        ...
        var noProxy = GetEnvironment(environment, "NO_PROXY");
    ...
    private static string? GetProxyValue(
        Uri destination,
        ParentProxyPolicy? explicitConfig,
        IReadOnlyDictionary<string, string?>? environment)
    {
        if (explicitConfig?.ProxyUri is not null)
            return explicitConfig.ProxyUri.ToString();

        if (explicitConfig?.AllowEnvironmentProxy is false)
            return null;

        return destination.Scheme == Uri.UriSchemeHttps
            ? GetEnvironment(environment, "HTTPS_PROXY") ?? GetEnvironment(environment, "HTTP_PROXY")
