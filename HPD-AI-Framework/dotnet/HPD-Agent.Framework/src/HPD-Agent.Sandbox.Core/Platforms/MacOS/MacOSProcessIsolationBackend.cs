...
/// </list>
/// </remarks>
internal sealed class MacOSProcessIsolationBackend : ISandboxBackend
{
    ...
    private Task<PreparedSandboxCommand> WrapShellCommandAsync(string command, CancellationToken cancellationToken)
        => WrapShellCommandAsync(command, plan: null, cancellationToken);

    private Task<PreparedSandboxCommand> WrapShellCommandAsync(
        string command,
        SandboxIsolationPlan? plan,
        CancellationToken cancellationToken)
    {
        ...
        var wrappedCommand = new PreparedSandboxCommand(
            "sandbox-exec",
        ...
        _logger?.LogDebug(
            ...
            effectivePlan.Filesystem.AllowWritePaths().Count + SandboxDefaults.DefaultWritePaths.Count,
            dangerousPaths.Count);

        return Task.FromResult(wrappedCommand);
    }

    private IReadOnlyDictionary<string, string> BuildEnvironment(SandboxIsolationPlan? plan = null)
    {
