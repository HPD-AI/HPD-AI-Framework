...
namespace HPD.Agent.Bots.Slack.SocketMode;
    ...
    IHttpClientFactory httpClientFactory)
{
    ...
    public async Task<Uri> OpenConnectionUrlAsync(CancellationToken ct)
    {
        var appToken = options.Value.AppToken
            ?? throw new InvalidOperationException(
                "SlackBotConfig.AppToken must be set to use Socket Mode.");
    ...
    /// <c>BotWebSocketService.SendAsync</c> (which holds the send lock).
    /// </summary>
