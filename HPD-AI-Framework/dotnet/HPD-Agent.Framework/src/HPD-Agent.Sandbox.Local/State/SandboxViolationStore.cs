...
/// <summary>
/// Bounded in-memory tail of sandbox violations with lightweight subscriptions.
/// </summary>
internal sealed class SandboxViolationStore
{
    public const int DefaultCapacity = 100;

    private readonly object _gate = new();
    private readonly Queue<SandboxViolation> _tail;
    private readonly Dictionary<long, Action<IReadOnlyList<SandboxViolation>>> _listeners = [];
    private readonly int _capacity;
    private long _nextListenerId;

    public SandboxViolationStore(int capacity = DefaultCapacity)
    {
        if (capacity <= 0)
            throw new ArgumentOutOfRangeException(nameof(capacity), "Capacity must be positive.");

        _capacity = capacity;
        _tail = new Queue<SandboxViolation>(capacity);
    }
    ...
    public void Add(SandboxViolation violation)
    {
        ArgumentNullException.ThrowIfNull(violation);

        Action<IReadOnlyList<SandboxViolation>>[] listeners;
        IReadOnlyList<SandboxViolation> snapshot;
    ...
    public IReadOnlyList<SandboxViolation> Get(int? limit = null)
    {
        lock (_gate)
        {
            if (limit is null)
    ...
    public IReadOnlyList<SandboxViolation> GetSinceTotalCount(int totalCount)
    {
        lock (_gate)
        {
            if (totalCount < 0)
    ...
    public IDisposable Subscribe(Action<IReadOnlyList<SandboxViolation>> listener)
    {
    ...
    private sealed class Subscription(SandboxViolationStore owner, long id) : IDisposable
    {
        private SandboxViolationStore? _owner = owner;
