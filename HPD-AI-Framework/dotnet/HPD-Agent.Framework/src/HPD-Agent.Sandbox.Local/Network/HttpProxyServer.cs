...
internal sealed class HttpProxyServer : IHttpProxyServer
{
    ...
    private readonly SandboxRequestFilter? _requestFilter;
    ...
    public HttpProxyServer(
        ...
        SandboxRequestFilter? requestFilter = null,
        ILogger? logger = null,
    ...
    private async Task HandleClientAsync(TcpClient client, CancellationToken cancellationToken)
    {
        try
        {
            ...
            await using (var clientStream = client.GetStream())
            {
                ...
                {
                    EmitEvent(SandboxProxyEventKind.MalformedRequest, ex.Message);
                    await WriteBadRequestAsync(clientStream, ex.StatusCode, ex.Message, cancellationToken);
                    return;
                }
                catch (OperationCanceledException) when (!cancellationToken.IsCancellationRequested)
                {
                    EmitEvent(SandboxProxyEventKind.MalformedRequest, "Request headers timed out");
        ...
        CancellationToken cancellationToken)
    {
        ...
        catch (Exception ex) when (ex is IOException or SocketException or InvalidOperationException or OperationCanceledException)
        {
            ...
            EmitEvent(SandboxProxyEventKind.UpstreamFailure, ex.Message, host, port, "CONNECT");
            await WriteUpstreamFailureAsync(clientStream, ex.Message, cancellationToken);
            return;
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var filterDecision = await EvaluateRequestFilterAsync(request, uri, cancellationToken);
            if (filterDecision.Action == SandboxRequestAction.Deny)
            {
                var reason = filterDecision.Reason is { Length: > 0 }
                    ? filterDecision.Reason
                    : "Denied by sandbox request filter";
                EmitEvent(
                    ...
                    request.Method,
                    uri);
                await WriteDeniedAsync(tlsClient, reason, cancellationToken);
                return;
            }
        ...
        {
            EmitEvent(SandboxProxyEventKind.MalformedRequest, ex.Message, host, port);
            await WriteBadRequestAsync(tlsClient, ex.StatusCode, ex.Message, cancellationToken);
        }
        catch (AuthenticationException ex)
        {
            _logger?.LogWarning(ex, "TLS termination failed for {Host}:{Port}", host, port);
            EmitEvent(SandboxProxyEventKind.UpstreamFailure, ex.Message, host, port);
        }
        catch (Exception ex) when (ex is IOException or SocketException or InvalidOperationException or OperationCanceledException)
        {
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (filterDecision.Action == SandboxRequestAction.Deny)
        {
            var reason = filterDecision.Reason is { Length: > 0 }
                ...
                : "Denied by sandbox request filter";
            _logger?.LogWarning(
                "Request filter blocked HTTP request to: {Uri} ({Reason})",
                uri,
                reason);
            EmitEvent(
                SandboxProxyEventKind.RequestFilterDenied,
                ...
                request.Method,
                uri);
            await WriteDeniedAsync(clientStream, reason, cancellationToken);
            return;
        }
    ...
    private async ValueTask<SandboxRequestDecision> EvaluateRequestFilterAsync(
        HttpProxyRequest request,
        Uri uri,
        CancellationToken cancellationToken)
    {
        if (_requestFilter is null)
            return SandboxRequestDecision.Allow;

        try
        {
            return await _requestFilter(
                new SandboxHttpRequest
                {
                    Method = request.Method,
                    Uri = uri,
        ...
        {
            _logger?.LogWarning(ex, "Sandbox request filter threw while evaluating {Uri}", uri);
            return SandboxRequestDecision.Deny("Request filter failed closed");
        }
    }
