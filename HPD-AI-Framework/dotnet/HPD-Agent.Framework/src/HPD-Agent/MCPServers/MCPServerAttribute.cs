...
///
/// Two collapsing modes (controlled via CollapseWithinToolHarness):
/// 1. **Flat** (default): MCP tools appear directly under the parent toolharness when expanded
/// 2. **Nested** (CollapseWithinToolHarness = true): MCP tools sit behind their own container
///    inside the parent toolharness — two-level expand required
///
...
public class MCPServerAttribute : Attribute
{
    ...
    /// nested inside the parent toolharness container. Two expansions required: first the
    /// toolharness, then the MCP server container.
    ///
    /// When false (default), MCP tools appear directly under the parent toolharness
