...
public static class ContentStoreByteExtensions
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        await using var stream = await folder.OpenReadAsync(scope, nameOrId, cancellationToken).ConfigureAwait(false);
        if (stream == null)
            return null;
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return folder.ReadBytesAsync(scope, content.Id, cancellationToken);
    }
}
