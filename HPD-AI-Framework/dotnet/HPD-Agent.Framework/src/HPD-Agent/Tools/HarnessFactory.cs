...
/// <summary>
/// Factory for creating harness instances without reflection.
...
/// <item>CreateFunctions: Direct delegate to generated CreateHarness method</item>
/// <item>GetReferencedHarneses: Direct delegate to generated GetReferencedHarneses method</item>
/// <item>GetReferencedFunctions: Direct delegate to generated GetReferencedFunctions method</item>
/// </list>
///
/// <para>
/// The source generator creates a HarnessRegistry.g.cs file that initializes
/// an array of HarnessFactory records with these delegates, enabling AOT-compatible
/// harness instantiation and function creation.
/// </para>
/// </remarks>
public record HarnessFactory(
    // ========== EXISTING FIELDS ==========

    /// <summary>
    /// Harness name (e.g., "MathHarness", "SearchHarness").
    /// Always the class name (CustomName support removed).
    /// </summary>
    string Name,

    /// <summary>
    /// Harness type (for type checking and compatibility)
    /// </summary>
    Type HarnessType,

    /// <summary>
    /// Direct constructor call delegate - AOT-safe, no Activator.CreateInstance
    /// Example: () => new MathHarness()
    /// </summary>
    ...
    /// Returns harness names that this harness's skills depend on.
    /// </summary>
    Func<string[]> GetReferencedHarneses,

    /// <summary>
    /// Direct delegate to generated GetReferencedFunctions method.
    /// Returns map of harness name -> function names for selective registration.
    /// </summary>
    ...
    /// <summary>
    /// Whether this harness has a description (from [Collapse("description")]).
    ...
    /// <summary>
    /// Delegate to create a harness instance from an ISecretResolver.
    /// Generated when the harness has a constructor whose only parameter is ISecretResolver.
    /// Example: secrets => new StripeHarness(secrets)
    /// Null if harness has no ISecretResolver-only constructor.
    /// </summary>
    ...
    /// Example: typeof(SearchHarnessConfig) for SearchHarness(SearchHarnessConfig config).
    /// Null if harness has no config constructor.
    /// </summary>
    Type? ConfigType = null,

    /// <summary>
    /// Delegate to create a harness instance from JSON config.
    /// Example: json => new SearchHarness(json.Deserialize&lt;SearchHarnessConfig&gt;()!)
    /// Null if harness has no config constructor.
    /// </summary>
    ...
    /// <summary>
    /// Metadata type from [AIFunction&lt;TMetadata&gt;] on harness methods.
    /// Used to deserialize HarnessReference.Metadata at resolution time.
    ...
    /// <summary>
    /// All function names in this harness for selective registration.
    /// Example: new[] { "WebSearch", "ImageSearch", "NewsSearch" }
    /// Used to validate HarnessReference.Functions at resolution time.
    /// </summary>
    ...
    /// <summary>
    /// Whether this harness has [MCPServer] methods.
    ...
    /// Null when harness has no [OpenApi] methods.
    ///
    ...
    ///   - string: parent harness name
    ///
    /// Config is passed as object to avoid HarnessFactory taking a dependency on HPD-Agent.OpenApi.
    ...
    /// Null when the harness has no skills with document uploads or references.
    ...
    /// <c>HPDToolSourceGenerator</c> into <c>HarnessRegistry.g.cs</c>.
    /// Null when the harness declares no parameterless-constructor scoped middlewares.
    ...
    /// <c>HarnessReference.MiddlewareConfigs</c> keys) and a delegate that accepts a
    /// <c>JsonElement</c> and returns an instantiated middleware.
    /// Emitted by <c>HPDToolSourceGenerator</c> into <c>HarnessRegistry.g.cs</c>.
    /// Null when the harness declares no config-constructor scoped middlewares.
    /// </summary>
    IReadOnlyList<CollapseMiddlewareConfigFactory>? CollapseMiddlewareConfigFactories = null
);

/// <summary>
/// Entry for a config-constructor harness-scoped middleware factory .
/// Carries the middleware's simple type name for matching against
/// <c>HarnessReference.MiddlewareConfigs</c> map keys, and the factory delegate.
/// </summary>
/// <param name="MiddlewareTypeName">
/// Simple (unqualified) class name of the middleware type, e.g. <c>"DbRateLimitMiddleware"</c>.
/// Used to match entries in <c>HarnessReference.MiddlewareConfigs</c>.
/// </param>
/// <param name="Factory">
