...
/// <summary>
/// In-memory workspace store for development and contract testing.
/// </summary>
public sealed class InMemoryWorkspaceStore : IWorkspaceStore
{
    private readonly ConcurrentDictionary<string, SpaceRecord> _spaces = new();
    private readonly ConcurrentDictionary<string, ContentRecord> _content = new();
    private readonly ConcurrentDictionary<string, AttachmentRecord> _attachments = new();
    private readonly ConcurrentDictionary<string, AccessRecord> _access = new();
    private readonly ConcurrentDictionary<string, PendingContentWriteRecord> _pendingContentWrites = new();
    private readonly ConcurrentDictionary<string, EventStreamRecord> _eventStreamRecords = new();
        ...
        IWorkspaceEventStreams? eventStreams = null)
    {
        ...
        _contentObjects = contentObjects ?? new InMemoryWorkspaceContentObjects();
        _eventStreams = eventStreams ?? new InMemoryWorkspaceEventStreams();
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        lock (_gate)
        {
            if (!_spaces.ContainsKey(parentSpaceId))
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return Task.FromResult(_spaces.TryGetValue(spaceId, out var record) &&
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        lock (_gate)
        {
            ...
            {
                _spaces.TryRemove(deleteId, out _);

                foreach (var accessId in _access.Values
                    .Where(access => access.SpaceId == deleteId)
                    .Select(access => access.Id)
                    .ToList())
                {
                    _access.TryRemove(accessId, out _);
                }

                foreach (var attachmentId in _attachments.Values
                ...
                foreach (var streamId in _eventStreamRecords.Values
                    .Where(stream => stream.SpaceId == deleteId)
                    .Select(stream => stream.StreamId)
                    .ToList())
                {
                    eventStreamIds.Add(streamId);
                    _eventStreamRecords.TryRemove(streamId, out _);
                }
            }
        }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var result = _spaces.Values.FirstOrDefault(space => Matches(space, query) && CanReadSpace(principal, space.Id));
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        lock (_gate)
        {
            ...
            {
                if (!_attachments.TryGetValue(existingAttachmentId, out var attachment))
                ...
                if (!_content.TryGetValue(attachment.ContentId, out var existingContent))
            ...
            _pendingContentWrites[pending.Id] = pending;
        }
        ...
        catch (Exception ex)
        {
            lock (_gate)
            {
                _pendingContentWrites[pending.Id] = pending.Abort(ex.Message);
            }
        ...
        lock (_gate)
        {
            try
            {
                ...
                if (string.IsNullOrWhiteSpace(existingAttachmentId))
                {
                    ...
                    _attachments[createdAttachment.Id] = createdAttachment;
                    _pendingContentWrites.TryRemove(pending.Id, out _);
                ...
                _content[updatedContent.Id] = updatedContent;

                var updatedAttachment = attachment.WithContentVersion(updatedContent.Version);
                _attachments[updatedAttachment.Id] = updatedAttachment;
                _pendingContentWrites.TryRemove(pending.Id, out _);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        lock (_gate)
        {
            candidates = _pendingContentWrites.Values
        ...
        lock (_gate)
        {
            ...
            {
                if (_pendingContentWrites.TryRemove(id, out _))
                    removed++;
            }
        }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        lock (_gate)
        {
            candidates = _eventStreamRecords.Values
        ...
        lock (_gate)
        {
            ...
            {
                if (_eventStreamRecords.TryGetValue(repair.StreamId, out var current) &&
                    current.LatestSequenceNumber != repair.LatestSequenceNumber)
                {
                    _eventStreamRecords[repair.StreamId] = current.WithLatestSequence(repair.LatestSequenceNumber);
                    repaired++;
                }
            }
        }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var result = _attachments.Values
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        foreach (var space in ResolveSearchSpaces(principal, query))
        {
            ...
            {
                if (!_content.TryGetValue(attachment.ContentId, out var contentRecord))
                    continue;
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        lock (_gate)
        {
            ...
            if (!_eventStreamRecords.ContainsKey(streamId))
            {
                ...
                _pendingContentWrites[pendingDescriptor.Id] = pendingDescriptor;
            }
        }
        ...
        if (pendingDescriptor is not null)
        {
            ...
            catch (Exception ex)
            {
                lock (_gate)
                {
                    _pendingContentWrites[pendingDescriptor.Id] = pendingDescriptor.Abort(ex.Message);
                }
        ...
        lock (_gate)
        {
            ...
            if (!_eventStreamRecords.ContainsKey(streamId))
            {
                ...
                _eventStreamRecords[streamId] = EventStreamRecord.Create(
                    streamId,
                    spaceId,
                    request.Role,
                    request.Name,
                    attachment.Id);
                if (pendingDescriptor is not null)
                    _pendingContentWrites.TryRemove(pendingDescriptor.Id, out _);
            }
        }
        ...
        lock (_gate)
        {
            ...
            if (_eventStreamRecords.TryGetValue(streamId, out var record))
                _eventStreamRecords[streamId] = record.WithLatestSequence(append.SequenceNumber);
    ...
    private AttachmentRecord? FindEventStreamAttachment(
        string spaceId,
        AppendWorkspaceEventRequest request) =>
        _attachments.Values.FirstOrDefault(attachment =>
            attachment.SpaceId == spaceId &&
    ...
    {
        foreach (var child in _spaces.Values.Where(space => space.ParentSpaceId == parentSpaceId).ToList())
        {
        ...
        WorkspaceVisibleContentQuery query)
    {
        IEnumerable<SpaceRecord> spaces = query.TraversalMode switch
        {
            WorkspaceContentTraversalMode.SpaceOnly => string.IsNullOrWhiteSpace(query.SpaceId)
                ? []
                : _spaces.TryGetValue(query.SpaceId, out var space) ? [space] : [],
    ...
    {
        if (!_spaces.TryGetValue(spaceId, out var root))
            return [];
    ...
    {
        foreach (var child in _spaces.Values
    ...
    {
        foreach (var inherited in _access.Values.Where(access =>
    ...
    private bool CanReadContentVersion(WorkspacePrincipalRef principal, string contentId, string contentVersion) =>
        IsSystem(principal) ||
        _attachments.Values.Any(attachment =>
    ...
    private IContentSpacePolicy PolicyFor(string spaceId) => PolicyFor(_spaces[spaceId]);
    ...
    {
        if (!_spaces.TryGetValue(attachment.SpaceId, out var space))
            return false;
    ...
    {
        var now = DateTimeOffset.UtcNow;
        return _access.Values.Any(access =>
            access.SpaceId == spaceId &&
