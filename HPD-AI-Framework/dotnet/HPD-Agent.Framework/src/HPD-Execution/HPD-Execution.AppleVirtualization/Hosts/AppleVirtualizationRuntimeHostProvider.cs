namespace HPD.Execution.AppleVirtualization.Hosts;

using System.Globalization;
using HPD.Execution.AppleVirtualization.Activation;
...
public sealed class AppleVirtualizationRuntimeHostProvider : IRuntimeHostProvider
{
        ...
        string operation)
    {
        ...
        return previous with
        {
            ...
            GuestControl = GuestControl(GuestControlExpected(spec), installed: GuestAgentEvidencePresent(readiness), reachable: verifiedReady, conditions),
            Readiness = Readiness(spec, verifiedReady, now, previous.Readiness, previous.Generations.HostStartGeneration),
            ControlPlane = ControlPlane(GuestControlExpected(spec), guestReachable: verifiedReady),
        ...
        AppleVirtualizationGuestControlStatusResponse? guest)
    {
        ...
        ResourcePhase phase = host?.Phase == ResourcePhase.Unknown || host?.Phase is null
            ? PhaseFor(hostPhase)
            : host.Phase;
        bool guestExpected = guest?.Expected ?? previous.GuestControl?.Expected ?? GuestControlExpected(spec);
