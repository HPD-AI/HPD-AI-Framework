...
namespace HPD.Execution.Local.Platforms.Linux;
...
public sealed class BubblewrapBuilder
{
    ...
    internal BubblewrapBuilder WithFilesystemPlan(
        LocalFilesystemIsolationPlan filesystem,
        IEnumerable<string> mandatoryDenyWritePaths)
    {
        return WithFilesystemPlan(
            filesystem.AllowWritePaths(),
            filesystem.DenyReadPaths(),
            filesystem.AllowReadPaths(),
            mandatoryDenyWritePaths.Concat(filesystem.DenyWritePaths()));
    }
    ...
    {
        foreach (var name in LocalProcessIsolationDefaults.SafeEnvironmentVariables)
    ...
    public PreparedLocalProcessCommand BuildCommand(string command, string shell = "/bin/sh")
    {
        ...
        return new PreparedLocalProcessCommand("bwrap", args);
    }
    ...
    /// </summary>
    public PreparedLocalProcessCommand BuildWithSetupCommand(string setupScript, string command, string shell = "/bin/sh")
    {
    ...
    /// </summary>
    public PreparedLocalProcessCommand BuildWithSeccompCommand(string setupScript, string command, string seccompHelperPath, string shell = "/bin/sh")
    {
    ...
    private static string Render(PreparedLocalProcessCommand command)
    {
