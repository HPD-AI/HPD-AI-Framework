...
using HPD.Execution.Local.Platforms.Linux.Seccomp;
...
/// </list>
/// </remarks>
internal sealed class LinuxProcessIsolationBackend : ILocalProcessIsolationBackend
{
    ...
    private async Task<PreparedLocalProcessCommand> WrapShellCommandAsync(string command, CancellationToken cancellationToken)
        => await WrapShellCommandAsync(command, plan: null, cancellationToken);

    private async Task<PreparedLocalProcessCommand> WrapShellCommandAsync(
        string command,
        LocalProcessIsolationPlan? plan,
        CancellationToken cancellationToken)
    {
        ...
        var writePaths = effectivePlan.Filesystem.AllowWritePaths()
            .Concat(LocalProcessIsolationDefaults.DefaultWritePaths)
            .DefaultIfEmpty(".")
            .Distinct()
            .ToArray();
    ...
    private static string RenderCommand(PreparedLocalProcessCommand command)
    {
    ...
    private bool AllowAllUnixSockets(LocalProcessIsolationPlan? plan) =>
