...
///
/// </remarks>
internal sealed class WindowsProcessIsolationBackend : ILocalProcessIsolationBackend
{
    ...
    public async Task<PreparedLocalProcessCommand> WrapCommandAsync(CommandInvocation command, CancellationToken cancellationToken)
    {
        _ = await WrapCommandAsync(
            HPD.Execution.Local.Security.PosixShellQuoter.RenderCommand(command),
            cancellationToken);

        return new PreparedLocalProcessCommand(command.FileName, command.ArgumentList);
    }
