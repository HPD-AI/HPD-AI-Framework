...
namespace HPD.Execution.Contracts;
...
public enum ScriptSourceKind { Inline, ContentProjection, WorkspaceContent, ProviderSource }
public enum GuestFileContentKind { InlineBytes, InlineText, ContentProjection, WorkspaceContent, ProviderSource }
...
public enum ArtifactSourceKind { RegistryReference, Url, HostFile, WorkspaceContent, ProviderArtifact }
...
public sealed record ContentSelector { public required ContentSelectorKind Kind { get; init; } public ResourceRef<Workspace>? Workspace { get; init; } public ContentProjectionRole? WorkspaceRole { get; init; } public string? PathPrefix { get; init; } public WorkspaceContentSelection? WorkspaceContent { get; init; } public ScratchSelection? Scratch { get; init; } public ProviderSourceSelection? ProviderSource { get; init; } public HostPathSelection? HostPath { get; init; } public IReadOnlyList<ContentFilter> Filters { get; init; } = Array.Empty<ContentFilter>(); }
public enum ContentSelectorKind { WorkspaceRole, WorkspacePath, WorkspaceContentQuery, Scratch, ProviderSource, HostPath }
...
public sealed record WorkspaceContentSelection(string? Scope = null, string? Role = null, string? PathHint = null, string? Name = null, string? ContentType = null, IReadOnlyDictionary<string, string>? Tags = null);
...
public interface IProcessIsolationProvider { ProviderId ProviderId { get; } ValueTask<ProcessIsolationPlan> PlanIsolationAsync(ProcessInvocationSpec invocation, ProcessIsolationPolicy policy, CancellationToken cancellationToken = default); ValueTask<IsolatedProcessCommand> PrepareAsync(ProcessInvocationSpec invocation, ProcessIsolationPolicy policy, ProcessIsolationPlan? plan = null, CancellationToken cancellationToken = default); }
public sealed record ProcessIsolationPlan { public static ProcessIsolationPlan Empty { get; } = new(); public IReadOnlyList<string> Diagnostics { get; init; } = Array.Empty<string>(); public IReadOnlyList<ProviderExtensionData> ProviderExtensions { get; init; } = Array.Empty<ProviderExtensionData>(); }
public sealed record IsolatedProcessCommand { public required ProcessInvocationSpec Invocation { get; init; } public ProcessIsolationPlan Plan { get; init; } = ProcessIsolationPlan.Empty; public IReadOnlyList<ProviderExtensionData> ProviderExtensions { get; init; } = Array.Empty<ProviderExtensionData>(); }
...
public interface IExecutionWorkspaceStore { ValueTask<WorkspaceStatus> GetStatusAsync(ResourceRef<Workspace> workspace, CancellationToken cancellationToken = default); ValueTask<ContentEnumerationPage> EnumerateAsync(ResourceRef<Workspace> workspace, ContentSelector selector, ContentPageCursor cursor, CancellationToken cancellationToken = default); ValueTask CopyContentAsync(ContentSelector selector, IBufferWriter<byte> destination, CancellationToken cancellationToken = default); }
