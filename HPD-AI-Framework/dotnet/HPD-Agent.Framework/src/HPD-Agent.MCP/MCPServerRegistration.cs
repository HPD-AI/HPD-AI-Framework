...
/// ParentToolHarness is used at runtime to stamp ParentContainer on the MCP tools/container,
/// wiring them into the parent toolharness's collapse/expand lifecycle.
/// </summary>
public class MCPServerRegistration
{
    ...
    /// <summary>
    /// Name of the toolharness class that owns this MCP server.
    /// Stamped as ParentContainer on runtime AIFunctions so ToolVisibilityManager
    /// can hide/show them based on the parent toolharness's expansion state.
    /// </summary>
    public string ParentToolHarness { get; set; } = string.Empty;

    /// <summary>
    /// When true, MCP tools sit behind their own MCP_* container nested inside the
    /// parent toolharness. When false, tools appear directly under the parent toolharness.
    /// </summary>
    ...
    /// Receives the toolharness instance as object, returns config.
    /// Exactly one of StaticConfigProvider or InstanceConfigProvider is set.
    /// </summary>
