...
public sealed class HttpProxyServerTests
{
    ...
    public async Task PlainHttp_ToBlockedDomain_ReturnsForbidden()
    {
        ...
        events[0].Protocol.Should().Be(SandboxProxyProtocol.Http);
    ...
    public async Task PlainHttp_WithParentProxy_ForwardsAbsoluteFormToParent()
    {
        ...
        await using var proxy = new HttpProxyServer(
            ...
            {
                HttpProxy = $"http://127.0.0.1:{parent.Port}"
            });
        await proxy.StartAsync();
    ...
    public async Task PlainHttp_RequestFilterAllow_ForwardsRequest()
    {
        ...
        await using var proxy = new HttpProxyServer(
            ["127.0.0.1"],
            [],
            requestFilter: (_, _) => ValueTask.FromResult(SandboxRequestDecision.Allow));
        await proxy.StartAsync();
    ...
    [Fact]
    public async Task PlainHttp_RequestFilterDeny_ReturnsForbidden()
    {
        var events = new List<SandboxProxyEvent>();
        await using var proxy = new HttpProxyServer(
            ["example.com"],
            [],
            requestFilter: (_, _) => ValueTask.FromResult(SandboxRequestDecision.Deny("blocked by test filter")),
            eventSink: events.Add);
        await proxy.StartAsync();
        ...
        await stream.WriteAsync(Encoding.ASCII.GetBytes(
            "GET http://example.com/blocked HTTP/1.1\r\nHost: example.com\r\n\r\n"));

        var response = await ReadAvailableResponseAsync(stream);

        response.Should().Contain("403 Forbidden");
        response.Should().Contain("blocked by test filter");
        events.Should().ContainSingle();
        events[0].Protocol.Should().Be(SandboxProxyProtocol.Http);
        events[0].Kind.Should().Be(SandboxProxyEventKind.RequestFilterDenied);
        events[0].Reason.Should().Be("blocked by test filter");
    ...
    public async Task MalformedRequestLine_EmitsProxyEvent()
    {
        ...
        events.Should().ContainSingle();
        events[0].Protocol.Should().Be(SandboxProxyProtocol.Http);
        events[0].Kind.Should().Be(SandboxProxyEventKind.MalformedRequest);
        events[0].Reason.Should().Be("Malformed HTTP request line.");
    }

    [Fact]
    public async Task PlainHttp_RequestFilterThrow_FailsClosed()
    {
        await using var proxy = new HttpProxyServer(
            ["example.com"],
            [],
            requestFilter: (_, _) => throw new InvalidOperationException("boom"));
        await proxy.StartAsync();
        ...
        await stream.WriteAsync(Encoding.ASCII.GetBytes(
            "GET http://example.com/fail HTTP/1.1\r\nHost: example.com\r\n\r\n"));

        var response = await ReadAvailableResponseAsync(stream);

        response.Should().Contain("403 Forbidden");
        response.Should().Contain("Request filter failed closed");
    }

    [Fact]
    public async Task PlainHttp_RequestFilterReceivesMethodUriAndHeaders()
    {
        SandboxHttpRequest? observed = null;
        await using var proxy = new HttpProxyServer(
            ["example.com"],
            [],
            requestFilter: (request, _) =>
            {
                observed = request;
                return ValueTask.FromResult(SandboxRequestDecision.Deny("captured"));
            });
        await proxy.StartAsync();
        ...
        await stream.WriteAsync(Encoding.ASCII.GetBytes(
            "POST http://example.com/filter HTTP/1.1\r\nHost: example.com\r\nX-Test: yes\r\n\r\n"));

        _ = await ReadAvailableResponseAsync(stream);

        observed.Should().NotBeNull();
        observed!.Method.Should().Be("POST");
        observed.Uri.Should().Be(new Uri("http://example.com/filter"));
        observed.Headers["X-Test"].Should().Be("yes");
    }
    ...
    public async Task Connect_WithParentProxy_UsesConnectTunnel()
    {
        ...
        await using var proxy = new HttpProxyServer(
            ...
            new ParentProxyConfig
            {
                HttpsProxy = $"http://127.0.0.1:{parent.Port}"
            });
        await proxy.StartAsync();
    ...
    [Fact]
    public async Task Connect_WithTlsTermination_RequestFilterDeny_ReturnsForbiddenInsideTls()
    {
        ...
        await using var proxy = new HttpProxyServer(
            ["example.com"],
            [],
            requestFilter: (_, _) => ValueTask.FromResult(SandboxRequestDecision.Deny("blocked https")),
            eventSink: events.Add,
        ...
        await using var tls = new SslStream(
            ...
            (_, _, _, _) => true);
        await tls.AuthenticateAsClientAsync("example.com");
        ...
        response.Should().Contain("403 Forbidden");
        response.Should().Contain("blocked https");
        events.Should().ContainSingle(e =>
            e.Kind == SandboxProxyEventKind.RequestFilterDenied &&
            e.Uri == new Uri("https://example.com:443/blocked"));
    }

    [Fact]
    public async Task Connect_WithTlsTermination_RequestFilterAllow_ForwardsToTlsOrigin()
    {
        ...
        await using var proxy = new HttpProxyServer(
            ...
            {
                observed = request;
                return ValueTask.FromResult(SandboxRequestDecision.Allow);
            },
            tlsIssuerCertificate: authority.Certificate,
        ...
        origin.Request.Should().StartWith("GET /allowed?q=1 HTTP/1.1");
        origin.Request.Should().Contain("X-Test: yes");
        observed.Should().NotBeNull();
        observed!.Uri.Should().Be(new Uri($"https://127.0.0.1:{origin.Port}/allowed?q=1"));
        observed.Headers["X-Test"].Should().Be("yes");
    }
