...
/// transform to UriContent(hpd-content://...), and verify the middleware resolves and transcribes them.
/// </summary>
public class ContentReferenceResolutionTests
{
    // -------------------------------------------------------------------------
    // 33. Full round-trip: upload -> UriContent(hpd-content://) -> middleware resolves -> STT called
    // -------------------------------------------------------------------------

    [Fact]
    public async Task EndToEnd_UploadAudio_ThenTranscribe_ViaContentUri()
    {
        ...
        var content = await contentStore.WriteBytesAsync(sessionId, audioBytes, "audio/mpeg");
        ...
        // Build a message with UriContent(hpd-content://...) as the middleware will see post-upload
        var uriContent = new UriContent(ContentReferenceResolverMiddleware.CreateContentUri(content.Id), "audio/mpeg");
    ...
    // -------------------------------------------------------------------------
    // 34. Two audio contents uploaded - both resolved and transcribed separately
    // -------------------------------------------------------------------------

    [Fact]
    public async Task EndToEnd_MultipleContentReferences_EachResolved()
    {
        ...
        var content1 = await contentStore.WriteBytesAsync(sessionId, bytes1, "audio/mpeg");
        var content2 = await contentStore.WriteBytesAsync(sessionId, bytes2, "audio/mpeg");
        ...
        var message = new ChatMessage(ChatRole.User, [
            new UriContent(ContentReferenceResolverMiddleware.CreateContentUri(content1.Id), "audio/mpeg"),
            new UriContent(ContentReferenceResolverMiddleware.CreateContentUri(content2.Id), "audio/mpeg")
        ]);
