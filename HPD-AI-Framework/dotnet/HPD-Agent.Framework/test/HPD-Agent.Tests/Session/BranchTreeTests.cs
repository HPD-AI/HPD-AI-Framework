...
public class BranchTreeTests : AgentTestBase
{
    ...
    {
        // Arrange - Create branch with all tree navigation fields
    ...
    [Fact]
    public void Test04_NameField_ProperlySerializedDeserialized()
    {
        // Arrange
        var branch = new Branch("session-1", "branch-1")
        {
    ...
    public async Task Test09_ForkBranch_UpdatesAllSiblingMetadata_Atomically()
    {
        ...
        var agent = await CreateAgentWithStore(repository);
        var session = new HPD.Agent.Session("test-session");
        ...
        await repository.SaveInitialBranchAsync("test-session", main);
        ...
        var reloadedMain = await repository.LoadBranchAsync("test-session", "main");
        var reloadedFork1 = await repository.LoadBranchAsync("test-session", "fork-1");
    ...
    public async Task Test10_ForkBranch_PersistsBranchMetadata()
    {
        ...
        // Assert
        var reloaded = await repository.LoadBranchAsync("test-session", "fork-with-metadata");
        Assert.NotNull(reloaded);
        Assert.Equal("hpdos-main", reloaded.Metadata["workspaceId"].ToString());
        Assert.Equal("chat-left", reloaded.Metadata["paneId"].ToString());
    }
    ...
    public async Task ForkBranchAsync_WithoutMessageId_ForksFromLatestMessage()
    {
        ...
        var fork = await repository.LoadBranchAsync("test-session", "latest-fork");
        Assert.NotNull(fork);
    ...
    public async Task ForkBranch_InvokesBeforeBranchForkCommit_WithPreparedTargetBranch()
    {
        ...
        var agent = await CreateAgentWithStore(repository, middleware);
        var session = new HPD.Agent.Session("test-session");
        await repository.SaveSessionAsync(session);
        ...
        // Assert
        var reloaded = await repository.LoadBranchAsync("test-session", "fork-with-hook");
        Assert.NotNull(reloaded);
        ...
        Assert.Equal("from-hook", reloaded!.Metadata["branchHook"].ToString());
    ...
    public async Task ForkBranch_FlushesMiddlewareStateUpdatedByBeforeBranchForkCommit()
    {
        // Arrange
        var repository = new WorkspaceSessionRepository(new InMemoryWorkspaceStore());
        var agent = await CreateAgentWithStore(repository, new BranchForkStateMiddleware());
        var session = new HPD.Agent.Session("test-session");
        await repository.SaveSessionAsync(session);
        ...
        // Assert
        var reloaded = await repository.LoadBranchAsync("test-session", "fork-with-state");
        Assert.NotNull(reloaded);
    ...
    public async Task ForkBranch_WhenBeforeBranchForkCommitThrows_DoesNotPersistTargetOrSourceTreeUpdates()
    {
        // Arrange
        var repository = new WorkspaceSessionRepository(new InMemoryWorkspaceStore());
        var agent = await CreateAgentWithStore(repository, new ThrowingBranchForkMiddleware());
        var session = new HPD.Agent.Session("test-session");
        await repository.SaveSessionAsync(session);
        ...
        Assert.Null(await repository.LoadBranchAsync("test-session", "fork-fails"));
    ...
    public async Task ForkBranch_WithCompactionOnFork_CompactsOnlyTargetBranch()
    {
        ...
        // Assert
        var reloadedMain = await repository.LoadBranchAsync("test-session", "main");
        var reloadedFork = await repository.LoadBranchAsync("test-session", "fork-compacted");
    ...
    public async Task ForkBranch_WithCompactionOnForkDisabled_DoesNotInvokeCompactionStrategy()
    {
        ...
        var reloadedFork = await repository.LoadBranchAsync("test-session", "fork-uncompacted");
        reloadedFork!.Messages.Select(m => m.MessageId)
    ...
    public async Task ForkBranch_WithCompactionIntentEnabled_CompactsEvenWhenGlobalForkCompactionIsDisabled()
    {
        ...
        var reloadedFork = await repository.LoadBranchAsync("test-session", "fork-force-compacted");
        reloadedFork!.Messages.Select(m => m.MessageId)
    ...
    public async Task ForkBranch_WithCompactionIntentDisabled_SkipsGlobalForkCompaction()
    {
        ...
        // Assert
        strategy.CallCount.Should().Be(0);
        var reloadedFork = await repository.LoadBranchAsync("test-session", "fork-force-uncompacted");
        reloadedFork!.Messages.Select(m => m.MessageId)
            .Should().Equal("message-0", "message-1", "message-2");
    }
    ...
    public async Task ForkBranch_WhenMessageIdWasCompactedOut_ThrowsWithReplacementCandidates()
    {
        ...
        Assert.Null(await repository.LoadBranchAsync("test-session", "fork-missing"));
    }
    ...
    public async Task Test11_ForkBranch_SetsCorrectSiblingIndex_ChronologicalOrder()
    {
        ...
        main = (await repository.LoadBranchAsync("test-session", "main"))!;
        main.Session = session;
        ...
        var reloadedMain = await repository.LoadBranchAsync("test-session", "main");
        var reloadedFork1 = await repository.LoadBranchAsync("test-session", "fork-1");
        var reloadedFork2 = await repository.LoadBranchAsync("test-session", "fork-2");
        var reloadedFork3 = await repository.LoadBranchAsync("test-session", "fork-3");
    ...
    public async Task N3_ForkAtDifferentMessageIndices_IndependentGroups()
    {
        ...
        var forkAt0 = await repository.LoadBranchAsync("test-session", "fork-at-0");
        var forkAt2 = await repository.LoadBranchAsync("test-session", "fork-at-2");
    ...
    public async Task N4_ThirdForkFromSamePoint_PreviousForkIndexStable()
    {
        ...
        var f1 = await repository.LoadBranchAsync("test-session", "fork-1");
        var f2 = await repository.LoadBranchAsync("test-session", "fork-2");
        var f3 = await repository.LoadBranchAsync("test-session", "fork-3");
