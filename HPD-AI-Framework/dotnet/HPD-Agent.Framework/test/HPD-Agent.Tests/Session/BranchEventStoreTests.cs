...
public class BranchEventStoreTests : AgentTestBase
{
    private static string EventType(AgentEvent evt) => AgentEventSerializer.GetEventTypeName(evt);

    [Fact]
    public async Task InMemoryStore_ProjectsBranchFromEvents()
    {
        ...
        await store.AppendBranchEventAsync(session.Id, branch.Id, BranchEventFactory.MessageStarted(session.Id, branch.Id, userMessage));
        await store.AppendBranchEventAsync(session.Id, branch.Id, BranchEventFactory.TextMessageStarted(session.Id, branch.Id, null, "msg-1", ChatRole.User.Value, 0));
        await store.AppendBranchEventAsync(session.Id, branch.Id, BranchEventFactory.TextDelta(session.Id, branch.Id, null, "msg-1", "hello", 0));
        await store.AppendBranchEventAsync(session.Id, branch.Id, BranchEventFactory.TextMessageCompleted(session.Id, branch.Id, null, "msg-1", 0));
        await store.AppendBranchEventAsync(session.Id, branch.Id, BranchEventFactory.MessageCompleted(session.Id, branch.Id, "msg-1"));

        var loaded = await store.LoadBranchAsync(session.Id, branch.Id);
    ...
    public async Task InMemoryStore_ReadBranchEvents_UsesReplayOptions()
    {
        ...
        await foreach (var evt in store.ReadBranchEventsAsync(
            session.Id,
            branch.Id,
    ...
    public async Task Agent_PersistsReasoningEvents_WhenReasoningExcludedFromModelHistory()
    {
        ...
        var loaded = await store.LoadBranchAsync(session.Id, branch.Id, TestCancellationToken);
        Assert.NotNull(loaded);
