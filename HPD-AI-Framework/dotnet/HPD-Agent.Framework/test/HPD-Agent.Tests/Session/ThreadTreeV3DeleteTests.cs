...
/// <summary>
/// V3 Thread Tree Architecture - Delete Operation Tests
...
public class ThreadTreeV3DeleteTests : AgentTestBase
{
    ...
    [Fact]
    public async Task Test15_DeleteThread_WithChildren_ThrowsError()
    {
        // Arrange
        ...
        await store.SaveInitialThreadAsync("test-session", fork1);

        var fork2 = await agent.ForkThreadAsync(fork1, "fork-2", fromMessageId: fork1.Messages[0].MessageId!);

        // Act & Assert - Cannot delete fork-1 because it has children
        var ex = await Assert.ThrowsAsync<InvalidOperationException>(
            async () => await agent.DeleteThreadAsync("test-session", "fork-1"));

        Assert.Contains("child", ex.Message, StringComparison.OrdinalIgnoreCase);
    }

    [Fact]
    public async Task Test17_DeleteThread_ReindexesRemainingSiblings()
    {
        // Arrange
        var store = new InMemorySessionStore();
        var agent = await CreateAgentWithStore(store);
        ...
        main.Session = session;
        var fork2 = await agent.ForkThreadAsync(main, "fork-2", fromMessageId: main.Messages[0].MessageId!);
        main = (await store.LoadThreadAsync("test-session", "main"))!;
        main.Session = session;
        var fork3 = await agent.ForkThreadAsync(main, "fork-3", fromMessageId: main.Messages[0].MessageId!);

        // Initial state: main(0), fork-1(1), fork-2(2), fork-3(3) — TotalSiblings=4
        var beforeMain = await store.LoadThreadAsync("test-session", "main");
        var beforeFork1 = await store.LoadThreadAsync("test-session", "fork-1");
        var beforeFork2 = await store.LoadThreadAsync("test-session", "fork-2");
        var beforeFork3 = await store.LoadThreadAsync("test-session", "fork-3");
        ...
        await agent.DeleteThreadAsync("test-session", "fork-2");

        // Assert - Remaining: main(0), fork-1(1), fork-3(2) — TotalSiblings=3
        var afterMain = await store.LoadThreadAsync("test-session", "main");
        var afterFork1 = await store.LoadThreadAsync("test-session", "fork-1");
        var afterFork2 = await store.LoadThreadAsync("test-session", "fork-2");
        var afterFork3 = await store.LoadThreadAsync("test-session", "fork-3");
    ...
    [Fact]
    public async Task Test18_DeleteThread_UpdatesNavigationPointers()
    {
        // Arrange
        var store = new InMemorySessionStore();
        var agent = await CreateAgentWithStore(store);
    ...
    [Fact]
    public async Task Test19_DeleteThread_UpdatesParentChildThreads()
    {
        // Arrange
        var store = new InMemorySessionStore();
        var agent = await CreateAgentWithStore(store);
        ...
        var beforeMain = await store.LoadThreadAsync("test-session", "main");
        Assert.Contains("fork-1", beforeMain!.ChildThreads);
        Assert.Equal(1, beforeMain.TotalForks);

        // Act - Delete fork
        await agent.DeleteThreadAsync("test-session", "fork-1");

        // Assert - Parent updated
        var afterMain = await store.LoadThreadAsync("test-session", "main");
        Assert.DoesNotContain("fork-1", afterMain!.ChildThreads);
        Assert.Equal(0, afterMain.TotalForks);
    }

    [Fact]
    public async Task Test40_CannotDeleteMainThread()
    {
        // Arrange
        var store = new InMemorySessionStore();
        var agent = await CreateAgentWithStore(store);
        ...
        // Act & Assert - Cannot delete main thread
        var ex = await Assert.ThrowsAsync<InvalidOperationException>(
            async () => await agent.DeleteThreadAsync("test-session", "main"));
    ...
    public async Task Test43_ForkWithMissingMessageId_FailsGracefully()
    {
        ...
        var ex = await Assert.ThrowsAsync<MessageNotPresentOnThreadException>(
            async () => await agent.ForkThreadAsync(main, "fork-1", fromMessageId: "missing-message"));

        Assert.Equal("test-session", ex.SessionId);
        Assert.Equal("main", ex.ThreadId);
    ...
    [Fact]
    public async Task Test33_CreateSession_ForkThread_VerifyTreeStructure()
    {
        // Arrange & Act
        var store = new InMemorySessionStore();
        var agent = await CreateAgentWithStore(store);
        ...
        Assert.Empty(reloadedFork1.ChildThreads);
    }

    [Fact]
    public async Task Test34_ForkMultipleThreads_VerifySiblingOrder()
    {
        // Arrange
        var store = new InMemorySessionStore();
        var agent = await CreateAgentWithStore(store);
        ...
        var forks = new List<Thread>();
        for (int i = 0; i < 5; i++)
        {
            var fork = await agent.ForkThreadAsync(main, $"fork-{i}", fromMessageId: main.Messages[0].MessageId!);
            forks.Add(fork);
            main = (await store.LoadThreadAsync("test-session", "main"))!;
            main.Session = session;
        ...
        for (int i = 0; i < 5; i++)
        {
            var reloaded = await store.LoadThreadAsync("test-session", $"fork-{i}");
    ...
    [Fact]
    public async Task Test35_DeleteThread_VerifySiblingsReindexed()
    {
        // Arrange
        var store = new InMemorySessionStore();
        var agent = await CreateAgentWithStore(store);
        ...
        for (int i = 0; i < 5; i++)
        {
            await agent.ForkThreadAsync(main, $"fork-{i}", fromMessageId: main.Messages[0].MessageId!);
            main = (await store.LoadThreadAsync("test-session", "main"))!;
            main.Session = session;
        }
        ...
        Assert.Null(await store.LoadThreadAsync("test-session", "fork-2"));

        var reloadedMain = await store.LoadThreadAsync("test-session", "main");
        var fork0 = await store.LoadThreadAsync("test-session", "fork-0");
        var fork1 = await store.LoadThreadAsync("test-session", "fork-1");
        var fork3 = await store.LoadThreadAsync("test-session", "fork-3");
        var fork4 = await store.LoadThreadAsync("test-session", "fork-4");
    ...
    public async Task Test36_ForkAgain_VerifyMultiLevelAncestry()
    {
        ...
        Assert.Contains("fork-1", reloadedMain!.ChildThreads);
        Assert.Contains("fork-2", reloadedFork1!.ChildThreads);
    }
    ...
    [Fact]
    public async Task TestEdge_ForkEmptyThread_WithMissingMessageId_Throws()
    {
        // Arrange
        var store = new InMemorySessionStore();
        var agent = await CreateAgentWithStore(store);
        ...
        // Act & Assert - empty threads have no valid fork message id
        var ex = await Assert.ThrowsAsync<MessageNotPresentOnThreadException>(
            async () => await agent.ForkThreadAsync(main, "fork-1", fromMessageId: "missing-message"));
    ...
    [Fact]
    public async Task TestEdge_ForkEmptyThread_WithDifferentMissingMessageId_Throws()
    {
        // Arrange
        var store = new InMemorySessionStore();
        var agent = await CreateAgentWithStore(store);
        ...
        // Act & Assert - no arbitrary message id can be resolved on an empty thread
        var ex = await Assert.ThrowsAsync<MessageNotPresentOnThreadException>(
            async () => await agent.ForkThreadAsync(main, "fork-1", fromMessageId: "another-missing-message"));
    ...
    [Fact]
    public async Task D3_DeleteSourceThread_IsRejected()
    {
        // Arrange
        var store = new InMemorySessionStore();
        var agent = await CreateAgentWithStore(store);
        ...
        // Give main a fork so it's a real source thread with children
        await agent.ForkThreadAsync(main, "fork-1", fromMessageId: main.Messages[0].MessageId!);
    ...
    public async Task D4_DeleteAllForks_SourceIsAlone()
    {
        ...
        var finalMain = await store.LoadThreadAsync("test-session", "main");
        ...
        Assert.Empty(finalMain.ChildThreads);
    }
