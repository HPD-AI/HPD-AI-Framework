...
namespace HPD.Execution.Contracts.Tests;
...
public sealed class ExecutionContractShapeTests
{
    ...
    public void Public_contract_uses_execution_namespace_and_not_container_legacy_names()
    {
        ...
        Assert.Equal("HPD-Execution.Contracts", typeof(RuntimeHost).Assembly.GetName().Name);
        Assert.All(publicTypes, type => Assert.StartsWith("HPD.Execution.Contracts", type.Namespace));
    ...
    private static Type[] ContractTypes() =>
        typeof(RuntimeHost).Assembly.GetExportedTypes()
            .Where(type => type.Namespace?.StartsWith("HPD.Execution.Contracts", StringComparison.Ordinal) == true)
            .ToArray();
    ...
    private sealed class RecordingProviderRegistrationBuilder : IProviderRegistrationBuilder
    {
        ...
        public void AddProcessIsolationProvider(IProcessIsolationProvider provider) => Calls.Add(nameof(AddProcessIsolationProvider));
        ...
        public void AddExecutionWorkspaceStore(IExecutionWorkspaceStore provider) => Calls.Add(nameof(AddExecutionWorkspaceStore));
...
[JsonSerializable(typeof(ProcessIsolationPlan))]
[JsonSerializable(typeof(IsolatedProcessCommand))]
