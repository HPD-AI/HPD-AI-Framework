...
public class IncrementalMaterializationTests
{
    private GraphOrchestrator<GraphContext> CreateOrchestrator(
        ...
        HPD.Graph.Abstractions.Graph.Graph? graph = null,
        HPD.Graph.Abstractions.Registry.IGraphRegistry? graphRegistry = null)
    {
        var services = TestServiceProvider.Create();
        var registry = graphRegistry ?? new HPD.Graph.Core.Registry.InMemoryGraphRegistry();
    ...
    public async Task MaterializeAsync_UnchangedInputs_SkipsCachedNodes()
    {
        ...
        var snapshotStore = new HPD.Graph.Core.Caching.InMemoryGraphSnapshotStore();
