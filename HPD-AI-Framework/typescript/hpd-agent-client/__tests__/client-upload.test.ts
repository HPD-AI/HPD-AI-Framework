...
describe('AgentClient.uploadContent() — AgentHttpApi', () => {
  ...
  it('calls POST /sessions/{sid}/branches/{bid}/content', async () => {
    mockFetchJson(CONTENT_REFERENCE, 200);
    await client.uploadContent('sess-1', 'branch-1', makeFile());

    expect(vi.mocked(fetch)).toHaveBeenCalledOnce();
    const [url, init] = vi.mocked(fetch).mock.calls[0];
    expect(String(url)).toBe(`${BASE}/sessions/sess-1/branches/branch-1/content`);
  ...
  it('returns the parsed ContentReference from the response', async () => {
    mockFetchJson(CONTENT_REFERENCE);
    const result = await client.uploadContent('sess-1', 'branch-1', makeFile());
  ...
  it('uses the File name as the form field filename by default', async () => {
    ...
    await client.uploadContent('sess-1', 'branch-1', file);
  ...
  it('uses "upload" as filename for a plain Blob', async () => {
    mockFetchJson(CONTENT_REFERENCE);
    await client.uploadContent('sess-1', 'branch-1', makeBlob());
  ...
  it('uses the name param to override the filename', async () => {
    mockFetchJson(CONTENT_REFERENCE);
    await client.uploadContent('sess-1', 'branch-1', makeFile('original.png'), 'override.png');
  ...
  it('throws with HTTP status in the message on non-2xx response', async () => {
    ...
    await expect(client.uploadContent('sess-1', 'branch-1', makeFile())).rejects.toThrow('413');
  });
});
