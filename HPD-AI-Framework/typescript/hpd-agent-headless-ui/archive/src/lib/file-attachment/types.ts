...
export interface FileAttachmentProps {
	...
	client?: { uploadContent(sessionId: string, threadId: string, file: File | Blob, name?: string): Promise<ContentReference> };
	/** Active session ID — used when state is not provided */
	sessionId?: string | null;
	/** Active thread ID — used when state is not provided */
