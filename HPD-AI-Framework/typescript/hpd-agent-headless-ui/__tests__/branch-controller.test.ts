...
import { createBranchController } from '../src/index.js';
...
describe('createBranchController', () => {
  it('rehydrates and connects to the exact branch scope', async () => {
  ...
  it('stamps sendText inputs with branch scope', async () => {
    const client = fakeClient();
    const controller = createBranchController({
      client,
      agentId: 'agent',
  ...
  it('projects only events that belong to its branch scope by default', async () => {
