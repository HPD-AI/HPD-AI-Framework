...
import { EventTypes, type BranchMessage } from '@hpd-research/hpd-agent-client';
import { createBranchProjection, eventBelongsToScope } from '../src/index.js';

describe('createBranchProjection', () => {
  it('rehydrates settled branch messages without streaming state', () => {
  ...
  it('tracks branch run lifecycle', () => {
    const projection = createBranchProjection();

    projection.project({
      type: EventTypes.BRANCH_RUN_STARTED,
      runtimeRunId: 'run1',
      agentId: 'agent',
    ...
    expect(projection.getSnapshot().branchRun?.status).toBe('active');
    expect(projection.getSnapshot().streaming).toBe(true);

    projection.project({
      type: EventTypes.BRANCH_RUN_COMPLETED,
      runtimeRunId: 'run1',
      agentId: 'agent',
      cancelled: false,
      sessionId: 's1',
    ...
    expect(projection.getSnapshot().branchRun?.status).toBe('completed');
    expect(projection.getSnapshot().streaming).toBe(false);
  });
});

describe('eventBelongsToScope', () => {
  it('matches events scoped to the same session and branch', () => {
    expect(eventBelongsToScope(
      { type: EventTypes.TEXT_DELTA, messageId: 'm1', text: 'x', sessionId: 's1', branchId: 'main' },
      { agentId: 'a1', sessionId: 's1', branchId: 'main' },
    )).toBe(true);
  });

  it('rejects events scoped to another branch', () => {
    expect(eventBelongsToScope(
      { type: EventTypes.TEXT_DELTA, messageId: 'm1', text: 'x', sessionId: 's1', branchId: 'other' },
      { agentId: 'a1', sessionId: 's1', branchId: 'main' },
    )).toBe(false);
  });
  ...
  it('rejects events scoped to another agent', () => {
    expect(eventBelongsToScope(
      { type: EventTypes.BRANCH_RUN_STARTED, runtimeRunId: 'r1', agentId: 'a2', startedAt: 'now' },
      { agentId: 'a1', sessionId: 's1', branchId: 'main' },
    )).toBe(false);
  });
  ...
  it('requires an explicit option for scope-less events', () => {
    ...
    const scope = { agentId: 'a1', sessionId: 's1', branchId: 'main' };
