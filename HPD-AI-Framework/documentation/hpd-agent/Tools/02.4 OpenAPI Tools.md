...
### Via Harness Class (Recommended)

Add `[OpenApi]` to a method inside a harness class that returns `OpenApiConfig`:
...
The harness approach also gives you collapsing, permissions, and conditional visibility — the same as any other capability.
...
| `CollapseWithinHarness` | `bool` | `false` | Group all generated functions behind their own nested container inside the parent harness. |
...
### Collapse inside a harness

`ISecretResolver` is injected automatically here too:

```csharp
[Collapse("Stripe billing tools")]
public class BillingHarness(ISecretResolver secrets)
{
    [OpenApi(Prefix = "stripe")]
    public OpenApiConfig Stripe() => new()
    {
        SpecPath = "stripe.json",
        CollapseWithinHarness = true,  // stripe_* grouped behind own sub-container
        AuthCallback = async (req, ct) =>
        {
            var key = await secrets.RequireAsync("stripe:ApiKey", "Stripe", ct: ct);
            req.Headers.Authorization = new("Bearer", key);
...
```
Before expanding BillingHarness:
┌──────────────────────┐
│ BillingHarness       │
│ (Stripe billing...)  │
└──────────────────────┘

After expanding BillingHarness:
┌──────────────────────┐
│ OpenApi_stripe       │  ← sub-container (CollapseWithinHarness)
