...
The per-run `clientToolKits` option adds tool groups for that run. The `onClientToolInvoke` handler on the client still executes the tool request.
...
await client.run({
    ...
    clientToolKits: [ideGroup],          // Tool groups for this run
...
2. **Group registered after the run started** — use `clientToolKits` in run options, or register before calling `run(...)`
...
| Tools disappear between turns | Per-run registration without persistence | Register at client level or use `client.registerToolKit()` before calling `run(...)` |
