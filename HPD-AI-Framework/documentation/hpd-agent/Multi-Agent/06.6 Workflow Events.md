...
| `ExecutionContext` | `AgentExecutionContext` | Agent hierarchy context. Contains `AgentId` (unique run ID), `AgentChain` (list of parent names), `Depth` (nesting level), and `ParentAgentId`. Useful for correlating events when a workflow is invoked as a harness capability inside a parent agent. |
