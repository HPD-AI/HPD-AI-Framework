...
All workflow events and each node's agent events flow into the same `RunAsync` loop — no separate stream. Use `Metadata` on `AgentEvent` to identify which agent emitted what:
...
await foreach (var evt in agent.RunAsync("Research quantum computing", sessionId: sessionId))
{
    switch (evt)
    {
        ...
        // Workflow lifecycle events — no Metadata, match directly
...
- [06.5 As a ToolHarness Capability](../Multi-Agent/06.5%20As%20a%20ToolHarness%20Capability.md) — full reference including event filtering details
