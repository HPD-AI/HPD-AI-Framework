...
Use `[Collapse("description")]` to make a toolharness collapsible. This is the primary attribute for hierarchical collapsing:

```csharp
[Collapse("File operations for reading, writing, and managing files")]
public class FileSystemToolHarness
{
    [AIFunction]
...
**Note:** A toolharness without `[Collapse]` always has its functions visible directly.
...
├── Agent calls FileToolHarness container
├── FunctionResult returned: "FileToolHarness expanded. Available functions: ReadFile, WriteFile..."
...
var config = new AgentConfig
{
    Collapsing = new CollapsingConfig
    {
        ...
        // Runtime override: these toolharnesses never collapse even if they have descriptions
        NeverCollapse = new HashSet<string> { "CoreToolHarness", "DebugToolHarness" },
...
#### Enabled
```csharp
Enabled = true  // ToolHarnesses with [Collapse] are collapsible
...
This setting does not affect C# toolharnesses with `[Collapse]` — their description is always the string you provide.
...
#### NeverCollapse
Runtime override to prevent specific toolharnesses from collapsing, even if they have descriptions:
```csharp
NeverCollapse = new HashSet<string> { "CoreToolHarness", "DebugToolHarness" }
```
...
- **Debug/development** toolharnesses that you don't want collapsed during testing
...
- At runtime, `ToolVisibilityManager` treats toolharnesses in `NeverCollapse` as non-containers
...
// But at runtime, if "FileToolHarness" is in NeverCollapse:
// - Container is hidden
...
public class GitToolHarness { /* commit, push, pull, status, diff */ }

// Bad: Unrelated functions forced together
[Collapse("Misc utilities")]
public class MiscToolHarness { /* sendEmail, readFile, getCurrentTime, translateText */ }
```
...
```
Tools: DatabaseToolHarness (Query, Insert, Update, Delete, ...)
```
...
| Concept | Purpose |
|---------|---------|
| `[Collapse("description")]` | Group toolharness functions into expandable container |
...
| `CollapsingConfig.NeverCollapse` | Runtime override to prevent specific toolharnesses from collapsing |
