...
Use `[Collapse("description")]` to make a harness collapsible. This is the primary attribute for hierarchical collapsing:
...
**Note:** A harness without `[Collapse]` always has its functions visible directly.
...
├── Agent calls FileHarness container
├── FunctionResult returned: "FileHarness expanded. Available functions: ReadFile, WriteFile..."
...
var config = new AgentConfig
{
    Collapsing = new CollapsingConfig
    {
        ...
        // Runtime override: these harnesses never collapse even if they have descriptions
        NeverCollapse = new HashSet<string> { "CoreHarness", "DebugHarness" },
...
#### Enabled
```csharp
Enabled = true  // Harneses with [Collapse] are collapsible
...
This setting does not affect C# harnesses with `[Collapse]` — their description is always the string you provide.
...
#### NeverCollapse
Runtime override to prevent specific harnesses from collapsing, even if they have descriptions:
```csharp
NeverCollapse = new HashSet<string> { "CoreHarness", "DebugHarness" }
```
...
- **Debug/development** harnesses that you don't want collapsed during testing
...
- At runtime, `ToolVisibilityManager` treats harnesses in `NeverCollapse` as non-containers
...
// But at runtime, if "FileHarness" is in NeverCollapse:
// - Container is hidden
...
public class GitHarness { /* commit, push, pull, status, diff */ }

// Bad: Unrelated functions forced together
[Collapse("Misc utilities")]
public class MiscHarness { /* sendEmail, readFile, getCurrentTime, translateText */ }
```
...
```
Tools: DatabaseHarness (Query, Insert, Update, Delete, ...)
```
...
| Concept | Purpose |
|---------|---------|
| `[Collapse("description")]` | Group harness functions into expandable container |
...
| `CollapsingConfig.NeverCollapse` | Runtime override to prevent specific harnesses from collapsing |
