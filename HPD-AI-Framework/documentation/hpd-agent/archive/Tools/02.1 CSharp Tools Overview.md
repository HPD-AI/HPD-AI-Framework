...
ToolHarnesses are the primary way to extend HPD-Agent with custom capabilities. A tool container is a **class** that contains one or more **capabilities** that the agent can use to accomplish tasks.
...
A child agent that can be delegated tasks. SubAgents have their own configuration, ToolHarnesses, tool set, and branch-native execution policy.
...
```csharp
var agent = await new AgentBuilder()
    .WithToolHarness<SearchTools>(new SearchMetadata
    {
        ProviderName = "Tavily",
...
When you have many tools, the agent's context window can become cluttered. HPD-Agent supports **collapsing** ToolHarnesses into containers that expand on demand.
...
// ToolHarness instructions - same dual-context
[Collapse(
    "Database operations",
