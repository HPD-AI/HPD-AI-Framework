...
```csharp
.WithHarness<SearchHarness>(new SearchMetadata
{
    ProviderName = "Tavily",
    MaxResults = 20
})
...
- `description` in `[Harness("...")]` attribute
...
var agent = await new AgentBuilder()
    .WithHarness<SearchHarness>(metadata)
    .BuildAsync();
```
...
// Harness using metadata
[Harness("Web search operations using {metadata.ProviderName}")]
public class WebSearchHarness
{
    ...
    public Skill ImageResearchSkill()
    {
        return SkillFactory.Create(
            ...
            "WebSearchHarness.WebSearch",
            "WebSearchHarness.ImageSearch"
        );
    }
}
...
// Registration
var agent = await new AgentBuilder()
    .WithHarness<WebSearchHarness>(new WebSearchMetadata
    {
        ProviderName = "Google",
        MaxResults = 20,
        HasImageSearch = true,
