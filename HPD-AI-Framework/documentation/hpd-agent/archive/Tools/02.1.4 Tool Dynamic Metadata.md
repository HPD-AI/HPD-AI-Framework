...
```csharp
.WithToolHarness<SearchToolHarness>(new SearchMetadata
{
    ProviderName = "Tavily",
    MaxResults = 20
})
...
- `description` in `[ToolHarness("...")]` attribute
- `description` in `SkillFactory.Create()`
- `description` in `SubAgent.FromConfig()` and `SubAgent.FromAgentId()`
...
var agent = await new AgentBuilder()
    .WithToolHarness<SearchToolHarness>(metadata)
    .BuildAsync();
```
...
// ToolHarness using metadata
[ToolHarness("Web search operations using {metadata.ProviderName}")]
public class WebSearchToolHarness
{
    ...
    public Skill ImageResearchSkill()
    {
        return SkillFactory.Create(
            ...
            "WebSearchToolHarness.WebSearch",
            "WebSearchToolHarness.ImageSearch"
        );
    }
}
...
// Registration
var agent = await new AgentBuilder()
    .WithToolHarness<WebSearchToolHarness>(new WebSearchMetadata
    {
        ProviderName = "Google",
        MaxResults = 20,
        HasImageSearch = true,
