...
The per-run `clientHarnesses` option adds tool groups for that run. The `onClientToolInvoke` handler on the client still executes the tool request.

### 3. Dynamically at runtime

```typescript
// Add a group after construction
client.registerHarness(myHarness);

// Add multiple
client.registerHarnesses([groupA, groupB]);

// Remove it later
client.unregisterHarness('IDE');
...
```typescript
const ideGroup: clientHarnessDefinition = {
    name: 'IDE',
    description: 'VS Code interaction tools',
...
```typescript
import { createCollapsedHarness, createExpandedHarness } from '@hpd/hpd-agent-client';

// Starts hidden, agent must expand before using
const group = createCollapsedHarness('IDE', 'VS Code tools', tools, { systemPrompt: '...' });

// Always visible
const group = createExpandedHarness('IDE', tools);
```
...
onClientToolInvoke: async (request) => {
    if (request.toolName === 'Login') {
        ...
        return createSuccessResponse(request.requestId, 'Logged in as admin', {
            injectHarnesses: [adminHarness],   // Add new group
            removeHarnesses: ['GuestTools'],      // Remove old group
            expandHarnesses: ['AdminTools'],      // Pre-expand new group
            collapseHarnesses: ['PublicTools'],   // Collapse a group
...
|-------|------|-------------|
| `injectHarnesses` | `clientHarnessDefinition[]` | Add new tool groups |
| `removeHarnesses` | `string[]` | Remove groups by name |
| `expandHarnesses` | `string[]` | Pre-expand groups |
| `collapseHarnesses` | `string[]` | Collapse groups |
...
await client.run({
    ...
    clientHarnesses: [ideGroup],          // Tool groups for this run
...
> **Note:** Skill references use `toolName` (and optionally `HarnessName`), not `name`.
...
client.on(EventTypes.CLIENT_TOOL_GROUPS_REGISTERED, event => {
    console.log(`Registered ${event.totalTools} tools: ${event.registeredHarnesses.join(', ')}`);
});
...
2. **Group registered after the run started** — use `clientHarnesses` in run options, or register before calling `run(...)`
...
| Tool not found after login | Need to inject group dynamically | Use `augmentation.injectHarnesses` in the response |
...
| Tools disappear between turns | Per-run registration without persistence | Register at client level or use `client.registerHarness()` before calling `run(...)` |
