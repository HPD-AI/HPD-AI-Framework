...
When a toolharness has multiple functions, you can group them under a collapsible container using `[Collapse("description")]`:
...
**Note:** Providing a description to `[Collapse]` enables collapsing. Without `[Collapse]`, the toolharness's functions are always visible.
...
public class MyToolHarness
{
    ...
    [ConditionalFunction("FeatureEnabled")]
    public SubAgent FeatureAgent() => SubAgent.FromConfig(...);
}
```
