...
### Via ToolHarness Class (Recommended)

Add `[OpenApi]` to a method inside a toolharness class that returns `OpenApiConfig`:
...
var agent = await new AgentBuilder()
    .WithToolHarness<StripeToolHarness>()
    .BuildAsync();
```
...
The toolharness approach also gives you collapsing, permissions, and conditional visibility — the same as any other capability.
...
| `CollapseWithinToolHarness` | `bool` | `false` | Group all generated functions behind their own nested container inside the parent toolharness. |
...
### Collapse inside a toolharness

`ISecretResolver` is injected automatically here too:

```csharp
[Collapse("Stripe billing tools")]
public class BillingToolHarness(ISecretResolver secrets)
{
    [OpenApi(Prefix = "stripe")]
    public OpenApiConfig Stripe() => new()
    {
        SpecPath = "stripe.json",
        CollapseWithinToolHarness = true,  // stripe_* grouped behind own sub-container
        AuthCallback = async (req, ct) =>
        {
            var key = await secrets.RequireAsync("stripe:ApiKey", "Stripe", ct: ct);
            req.Headers.Authorization = new("Bearer", key);
...
```
Before expanding BillingToolHarness:
┌──────────────────────┐
│ BillingToolHarness       │
│ (Stripe billing...)  │
└──────────────────────┘

After expanding BillingToolHarness:
┌──────────────────────┐
│ OpenApi_stripe       │  ← sub-container (CollapseWithinToolHarness)
...
```csharp
public class PaymentsToolHarness
{
    [OpenApi(Prefix = "stripe")]
    [RequiresPermission]
    public OpenApiConfig Stripe() => new()
