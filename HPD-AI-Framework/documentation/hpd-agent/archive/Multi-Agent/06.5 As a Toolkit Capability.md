...
```csharp
public class ResearchToolHarness
{
...
public class PipelineToolHarness
{
...
var agent = await new AgentBuilder()
    .WithToolHarness<PipelineToolHarness>(new WorkflowMetadata { HasResearchCapability = true })
    .BuildAsync();
```
...
await foreach (var evt in agent.RunAsync("Research quantum computing", sessionId: sessionId))
{
    switch (evt)
    {
        ...
        case TextDeltaEvent delta when delta.Metadata?.Depth == 0:
        ...
        case TextDeltaEvent delta when delta.Metadata?.AgentName == "writer":
            Console.Write(delta.Text);
            break;
...
Every `AgentEvent` carries `Metadata` with two properties useful for filtering:
...
Workflow lifecycle events (`WorkflowStartedEvent`, `WorkflowNodeStartedEvent`, etc.) do **not** have `Metadata` — they come from the graph orchestrator, not an agent. Pattern-match on them directly without checking `Metadata`.
...
await foreach (var evt in agent.RunAsync(input, sessionId: sessionId))
{
    ...
    {
        var depth = agentEvt.Metadata?.Depth ?? 0;
        var name  = agentEvt.Metadata?.AgentName ?? "unknown";
    ...
    // Workflow events have no Metadata — match directly
