...
export interface GatewaySchemaValidationResult { readonly diagnostics:readonly GatewayJsonDiagnostic[]; readonly truncatedDiagnostics:number; }
export function validateGatewaySchema(root: GatewayJsonObject): GatewaySchemaValidationResult {
  const diagnostics = new DiagnosticSink();
  if (rootSchema === undefined) { diagnostics.push(diagnostic('schema-authority-unavailable', '')); return diagnostics.result(); }
...
function resolveBranch(node: GatewayJsonNode, schema: Schema, path: string, output: DiagnosticSink): Schema | null {
...
class DiagnosticSink{private readonly kept:GatewayJsonDiagnostic[]=[];private total=0;push(value:GatewayJsonDiagnostic):number{this.total++;if(this.kept.length<1024)this.kept.push(value);return this.total;}result():GatewaySchemaValidationResult{return Object.freeze({diagnostics:Object.freeze([...this.kept]),truncatedDiagnostics:Math.max(0,this.total-this.kept.length)});}}
