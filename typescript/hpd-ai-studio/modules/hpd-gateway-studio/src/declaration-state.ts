...
export interface GatewayDeclarationController {
  ...
  appendAtPointer(pointer:string,value:GatewayJsonNode):boolean;
  reset(): void;
  clearPrincipal(): void;
  dispose(): void;
}
...
export type GatewayProposalCommitter=(baseGeneration:bigint,baseSourceSha256:string,proposedUtf8:string,evidence:GatewayServerValidationEvidence,proposalId:string)=>boolean;
const proposalCommitters=new WeakMap<object,GatewayProposalCommitter>();
export function getInternalGatewayProposalCommitter(controller:GatewayDeclarationController):GatewayProposalCommitter{const value=proposalCommitters.get(controller);if(value===undefined)throw new TypeError('Unknown Gateway declaration controller.');return value;}
...
export function createGatewayDeclarationController(options: GatewayDeclarationControllerOptions = {}): GatewayDeclarationController {
  ...
  const controller:GatewayDeclarationController=Object.freeze({
    ...
    appendAtPointer(pointer:string,value:GatewayJsonNode){if(disposed||document.compatibleGraph===null)return false;const updated=appendNode(document.compatibleGraph,pointer,value);if(updated===null)return false;const source=serializeGatewayJson(updated);const candidate=materialize(source,document.editGeneration+1n);if(candidate.compatibleGraph===null)return false;validationGeneration++;validationController?.abort();validationController=null;history=document;document=candidate;emit();return true;},
  ...
  });proposalCommitters.set(controller,commitProposal);return controller;
}
...
function appendNode(root:GatewayJsonObject,pointerValue:string,value:GatewayJsonNode):GatewayJsonObject|null{if(!pointerValue.startsWith('/'))return null;const segments=pointerValue.split('/').slice(1).map(value=>value.replaceAll('~1','/').replaceAll('~0','~'));const recurse=(node:GatewayJsonNode,index:number):GatewayJsonNode|null=>{if(index===segments.length){if(node.kind!=='array'||node.items.length>=10_000)return null;return Object.freeze({kind:'array',items:Object.freeze([...node.items,value])});}const segment=segments[index]!;if(node.kind==='object'){const position=node.entries.findIndex(entry=>entry.name===segment);if(position<0)return null;const child=recurse(node.entries[position]!.value,index+1);if(child===null)return null;return Object.freeze({kind:'object',entries:Object.freeze(node.entries.map((entry,i)=>i===position?Object.freeze({name:entry.name,value:child}):entry))});}if(node.kind==='array'){const position=Number(segment);if(!Number.isInteger(position)||position<0||position>=node.items.length)return null;const child=recurse(node.items[position]!,index+1);if(child===null)return null;return Object.freeze({kind:'array',items:Object.freeze(node.items.map((item,i)=>i===position?child:item))});}return null;};const result=recurse(root,0);return result?.kind==='object'?result:null;}
