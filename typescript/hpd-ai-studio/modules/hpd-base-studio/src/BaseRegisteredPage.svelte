<script lang="ts">
  ...
  function displayObservation(value:StudioObservation<Value>):StudioDisplayObservation { switch(value.state){
    ...
    case 'loading':return{state:'loading',hasPrevious:value.previous!==null};case'unobserved':return{state:'unobserved'};default:return{state:value.state,code:value.code};} }
  function currentValue(value:StudioObservation<Value>):Value|null{return value.state==='value'||value.state==='stale'?value.value:value.state==='loading'?value.previous:null}
</script>
<StudioRegisteredWorkspace eyebrow="HPD BASE Studio" {page} {resource} observation={observed} views={payload?.views??{}}
  links={payload?.links??[]} {commands} onnavigate={(link:StudioLinkProjection)=>navigation.navigate({link})}/>
