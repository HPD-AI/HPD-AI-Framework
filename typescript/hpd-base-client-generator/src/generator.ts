...
export function validate(snapshot: GenerationSnapshot, expectedAudience?: "application" | "controlPlane" | "service" | "system"): void {
  exactKeys(snapshot as unknown as Record<string, unknown>, ["protocol", "application", "schema", "endpoints", "capabilities", "registeredReads", "dependencyTemplates", "vectorIndexes", "textIndexes", "selectionMutations", "moduleMutations", "subjectLifecycleConsumers", "semanticActivations", "errors", "digest"]);
  ...
  if (snapshot.schema.collections.length > 256 || snapshot.schema.types.length > 512 || snapshot.endpoints.length > 256 || snapshot.registeredReads.length > 256 || snapshot.vectorIndexes.length > 256 || snapshot.textIndexes.length > 256 || snapshot.selectionMutations.length > 256 || snapshot.moduleMutations.length > 256 || snapshot.subjectLifecycleConsumers.length > 32 || snapshot.semanticActivations.length > 4096 || snapshot.dependencyTemplates.length > 512) throw new Error("base.client.snapshotTooLarge");
  if (snapshot.application.audience !== "controlPlane" && snapshot.semanticActivations.length !== 0) throw new Error("base.client.audienceMismatch");
  const semanticNames = unique(snapshot.semanticActivations.map(value => value.generatedName), "base.client.nameCollision");
  if (semanticNames.size !== snapshot.semanticActivations.length) throw new Error("base.client.nameCollision");
  for (const definition of snapshot.semanticActivations) {
    exactKeys(definition as unknown as Record<string, unknown>, ["id", "version", "checksum", "generatedName", "compactable", "removable"]);
    if (!stableId(definition.id) || !stableProperty(definition.generatedName) || !safeBound(definition.version, 1, 2_147_483_647)
      || !base64Bytes(definition.checksum, 32) || typeof definition.compactable !== "boolean" || typeof definition.removable !== "boolean") throw new Error("base.client.snapshotInvalid");
  }
  ...
  for (const index of snapshot.textIndexes) {
    exactKeys(index as unknown as Record<string, unknown>, ["collectionId", "id", "version", "generatedName", "analyzerId", "scoringId", "audience", "fields", "filterFields", "maximumResults", "maximumQueryNodes", "maximumQueryDepth", "maximumPhraseTerms", "maximumQueryBytes", "maximumFilterNodes", "maximumFilterDepth", "maximumFilterLiterals", "maximumInValues", "maximumSecondaryOrderFields", "maximumCursorBytes"]);
    if (!stableId(index.collectionId) || !stableId(index.id) || !safeBound(index.version, 1, 2_147_483_647) || !safeBound(index.maximumResults, 1, 256) || !safeBound(index.maximumQueryNodes, 1, 256) || !safeBound(index.maximumQueryDepth, 1, 32) || !safeBound(index.maximumPhraseTerms, 1, 64) || !safeBound(index.maximumQueryBytes, 1, 65_536) || !safeBound(index.maximumFilterNodes, 1, 256) || !safeBound(index.maximumFilterDepth, 1, 32) || !safeBound(index.maximumFilterLiterals, 1, 256) || !safeBound(index.maximumInValues, 1, 256) || !safeBound(index.maximumSecondaryOrderFields, 0, 16) || !safeBound(index.maximumCursorBytes, 1, 16_384) || index.fields.length === 0 || index.fields.length > 16 || index.filterFields.length > 16) throw new Error("base.client.snapshotInvalid");
...
const wellKnownWireTypeIds = new Set([
  ...
  "base.semanticActivation.inspect.request", "base.semanticActivation.inspect.page", "base.semanticActivation.control.read.request", "base.semanticActivation.control.descriptor", "base.semanticActivation.control.command", "base.semanticActivation.control.resolve.request", "base.semanticActivation.control.result",
...
function base64Bytes(value: unknown, length: number): boolean { if (typeof value !== "string" || !/^(?:[A-Za-z0-9+/]{4})*(?:[A-Za-z0-9+/]{2}==|[A-Za-z0-9+/]{3}=)?$/u.test(value)) return false; try { const bytes = Buffer.from(value, "base64"); return bytes.length === length && bytes.toString("base64") === value; } catch { return false; } }
...
function render(snapshot: GenerationSnapshot): Record<string, string> {
  ...
  const textIndexes = snapshot.textIndexes.map(item => `export const ${safe(item.generatedName)} = ${JSON.stringify(textIndexValue(item))} as const;`).join("\n");
  ...
  const semanticValues = snapshot.semanticActivations.map(item => `  ${safe(item.generatedName)}: semanticActivationDefinition(${JSON.stringify({ id: item.id, version: item.version, checksum: item.checksum, compactable: item.compactable, removable: item.removable })})`).join(",\n");
  const features = { files: snapshot.endpoints.some(endpoint => endpoint.operation.startsWith("File")), realtime: snapshot.endpoints.some(endpoint => endpoint.operation === "RealtimeSubscribe"), batch: snapshot.schema.collections.some(collection => collection.operations.includes("batch")), controlOperations: snapshot.application.audience === "controlPlane" ? snapshot.endpoints.map(endpoint => endpoint.id).filter(id => id.startsWith("base.admin.") || id.startsWith("base.semanticActivation.") || id.startsWith("hpd.base.vector.")).sort() : [] };
  ...
  if (snapshot.application.audience === "controlPlane") {
    files["semantic-activations.ts"] = `import { semanticActivationDefinition } from "@hpd/base-client";\nexport const semanticActivations = {\n${semanticValues}\n} as const;\n`;
    files["module-mutations.ts"] = `import { moduleMutation } from "@hpd/base-client";\nimport { typeGraph } from "./types.js";\nimport type * as GeneratedTypes from "./types.js";\n${moduleTypes}\nexport const moduleMutations = {\n${moduleValues}\n} as const;\n`;
    files["schema.ts"] = `import { collections } from "./collections.js";\nimport { reads } from "./reads.js";\nimport { selectionMutations } from "./selection-mutations.js";\nimport { moduleMutations } from "./module-mutations.js";\nimport { semanticActivations } from "./semantic-activations.js";\nimport { protocol } from "./protocol.js";\nimport { typeGraph } from "./types.js";\nexport const schema = Object.freeze({ ...protocol, collections, reads, selectionMutations, moduleMutations, semanticActivations, typeGraph });\n`;
    files["index.ts"] = files["index.ts"]!.replace('export * from "./selection-mutations.js";\n', 'export * from "./selection-mutations.js";\nexport * from "./module-mutations.js";\nexport * from "./semantic-activations.js";\n');
  }
...
function renderCollectionValue(collection: CollectionDescriptor, typeNames: ReadonlyMap<string, string>, vectors: readonly VectorDescriptor[], textIndexes: readonly TextIndexDescriptor[]): string {
  ...
  const textValues = textIndexes.map(item => `${safe(item.generatedName)}: ${JSON.stringify(textIndexValue(item))}`).join(", ");
...
function textIndexValue(item: TextIndexDescriptor): object {
  return { id: item.id, version: item.version, maximumResults: item.maximumResults, maximumQueryNodes: item.maximumQueryNodes, maximumQueryDepth: item.maximumQueryDepth, maximumPhraseTerms: item.maximumPhraseTerms, maximumQueryBytes: item.maximumQueryBytes, maximumFilterNodes: item.maximumFilterNodes, maximumFilterDepth: item.maximumFilterDepth, maximumFilterLiterals: item.maximumFilterLiterals, maximumInValues: item.maximumInValues, maximumSecondaryOrderFields: item.maximumSecondaryOrderFields, maximumCursorBytes: item.maximumCursorBytes, fields: Object.fromEntries(item.fields.map(field => [field.generatedName, { id: field.id, wireName: field.wireName }])), filterFields: Object.fromEntries(item.filterFields.map(field => [field.generatedName, { id: field.id, wireName: field.wireName, valueKind: field.valueKind }])) };
}
