...
import { generate, parseSnapshot, structuralDigest } from "../dist/index.js";
...
test("generation validates its digest, emits the complete surface, and typechecks with TypeScript 7", async () => {
  ...
  for (const name of ["index.ts", "protocol.ts", "schema.ts", "collections.ts", "fields.ts", "reads.ts", "vectors.ts", "dependencies.ts", "errors.ts"]) await access(`${output}/${name}`);
  ...
  for (const name of ["index.ts", "protocol.ts", "schema.ts", "collections.ts", "fields.ts", "reads.ts", "vectors.ts", "dependencies.ts", "errors.ts"]) assert.equal(await readFile(`${output}/${name}`, "utf8"), await readFile(`${second}/${name}`, "utf8"));
