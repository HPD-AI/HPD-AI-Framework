  ...
  try {
    ...
    for (const [name, engine] of Object.entries({ chromium, firefox, webkit })) {
      ...
      try {
        ...
        const codec = await page.evaluate(async () => { const base = await import("/dist/index.js"); const graph = { f32: { kind: "floating", precision: "binary32", finiteOnly: true }, array: { kind: "array", elementTypeId: "f32", minItems: 0, maxItems: 2 }, record: { kind: "object", additionalProperties: false, properties: [{ name: "score", wireName: "score", typeId: "f32", required: true, nullable: false, redactionOptional: false }] }, create: { kind: "object", additionalProperties: false, properties: [] }, replace: { kind: "object", additionalProperties: false, properties: [] }, patch: { kind: "object", additionalProperties: false, properties: [] } }; const schema = { protocolMajor: 2, schemaGeneration: "1", digest: `sha256:${"0".repeat(64)}`, audience: "application", features: { files: false, realtime: false, batch: false, controlOperations: [] }, typeGraph: graph, reads: {}, collections: { documents: base.collection({ id: "documents", recordTypeId: "record", createTypeId: "create", replaceTypeId: "replace", patchTypeId: "patch", fields: { score: base.field("score", "score", ["equal"], "f32") }, operations: ["get"], pagination: "seek", maxPageSize: 10, vectorIndexes: {} }) } }; const client = base.createBaseClient({ schema, url: "http://base.invalid/base/", fetch: async () => new Response('{"collectionId":"documents","id":"d1","payload":{"kind":"json","json":{"score":-0}},"metadata":{}}', { headers: { "X-Correlation-ID": "browser" } }) }); const response = await client.documents.get("d1"); return { encoded: base.encodeBaseJson([Math.fround(0.1), -0], "array", graph), decoded: base.decodeBaseJson("[0.1,-0]", "array", graph), transportPositiveZero: response.ok && response.value.payload.json.score === 0 && !Object.is(response.value.payload.json.score, -0) }; });
        assert.deepEqual(codec, { encoded: "[0.1,0]", decoded: [Math.fround(0.1), 0], transportPositiveZero: true });
