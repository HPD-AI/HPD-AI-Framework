...
declare const semanticCompactBrand: unique symbol; declare const semanticRemoveBrand: unique symbol; declare const semanticResumeBrand: unique symbol; declare const semanticResolutionBrand: unique symbol;
export type BaseSemanticActivationCompactToken = string & { readonly [semanticCompactBrand]: true };
export type BaseSemanticActivationRemoveToken = string & { readonly [semanticRemoveBrand]: true };
export type BaseSemanticActivationResumeToken = string & { readonly [semanticResumeBrand]: true };
export type BaseSemanticActivationResolutionToken = string & { readonly [semanticResolutionBrand]: true };
export interface BaseSemanticActivationControlDescriptor { readonly definitionId: string; readonly definitionVersion: number; readonly authorityGeneration: string | null; readonly liveCount: string | null; readonly retiredCount: string | null; readonly absenceCount: string | null; readonly ready: boolean; readonly quarantined: boolean; readonly compactToken: BaseSemanticActivationCompactToken | null; readonly removeToken: BaseSemanticActivationRemoveToken | null; }
export interface BaseSemanticActivationControlResult { readonly disposition: "Completed" | "Duplicate" | "InProgress" | "ConfirmedRolledBack" | "Indeterminate"; readonly authorityGeneration: string; readonly examinedRows: string; readonly changedRows: string; readonly canonicalBytes: string; readonly receiptDisposition: "Committed" | "Duplicate" | null; readonly resumeToken: BaseSemanticActivationResumeToken | null; readonly resolutionToken: BaseSemanticActivationResolutionToken | null; readonly sanitizedChecksum: string; }
export interface BaseSemanticActivationDefinitionShape { readonly id: string; readonly version: number; readonly checksum: string; readonly compactable: boolean; readonly removable: boolean; }
type SemanticHandle<T extends BaseSemanticActivationDefinitionShape> = {
  read(storeId: string, signal?: AbortSignal): Promise<BaseResult<BaseSemanticActivationControlDescriptor>>;
  resume(storeId: string, token: BaseSemanticActivationResumeToken, idempotencyKey: string, signal?: AbortSignal): Promise<BaseResult<BaseSemanticActivationControlResult>>;
  resolve(storeId: string, token: BaseSemanticActivationResolutionToken, signal?: AbortSignal): Promise<BaseResult<BaseSemanticActivationControlResult>>;
} & (T["compactable"] extends true ? { compact(storeId: string, token: BaseSemanticActivationCompactToken, idempotencyKey: string, signal?: AbortSignal): Promise<BaseResult<BaseSemanticActivationControlResult>> } : {})
  & (T["removable"] extends true ? { remove(storeId: string, token: BaseSemanticActivationRemoveToken, idempotencyKey: string, signal?: AbortSignal): Promise<BaseResult<BaseSemanticActivationControlResult>> } : {});
export type BaseSemanticActivationControlSurface<T extends Readonly<Record<string, BaseSemanticActivationDefinitionShape>>> = { readonly [K in keyof T]: SemanticHandle<T[K]> };
...
export type BaseControlPlaneClient<TOperations extends readonly string[], TSemantic extends Readonly<Record<string, BaseSemanticActivationDefinitionShape>> = {}> = Pick<BaseControlPlaneSurface, InstalledControlMethod<TOperations>> & { readonly semanticActivations: BaseSemanticActivationControlSurface<TSemantic> };

export function createControlPlaneClient<const TOperations extends readonly string[], const TSemantic extends Readonly<Record<string, BaseSemanticActivationDefinitionShape>>>(transport: BaseHttpTransport, operations: TOperations, semanticDefinitions: TSemantic): BaseControlPlaneClient<TOperations, TSemantic> {
  ...
  result["semanticActivations"] = createSemanticControls(transport, operations, semanticDefinitions);
  return Object.freeze(result) as BaseControlPlaneClient<TOperations, TSemantic>;
}
...
function createSemanticControls<const T extends Readonly<Record<string, BaseSemanticActivationDefinitionShape>>>(
  transport: BaseHttpTransport, operations: readonly string[], definitions: T): BaseSemanticActivationControlSurface<T> {
  const installed = new Set(operations); const result: Record<string, unknown> = {};
  const required = ["base.semanticActivation.control.read", "base.semanticActivation.maintenance.resume", "base.semanticActivation.maintenance.resolve"];
  if (Object.keys(definitions).length !== 0 && required.some(value => !installed.has(value))) throw new TypeError("base.client.endpointMismatch");
  for (const [name, definition] of Object.entries(definitions)) {
    if (!validSemanticDefinition(definition)) throw new TypeError("base.client.contractInvalid");
    const handle: Record<string, unknown> = {
      read: (storeId: string, signal?: AbortSignal) => readSemanticControl(transport, definition, storeId, signal),
      resume: (storeId: string, token: BaseSemanticActivationResumeToken, idempotencyKey: string, signal?: AbortSignal) => executeSemanticControl(transport, "maintenance:resume", "resume-semantic-maintenance", storeId, token, idempotencyKey, signal),
      resolve: (storeId: string, token: BaseSemanticActivationResolutionToken, signal?: AbortSignal) => resolveSemanticControl(transport, storeId, token, signal),
    };
    if (definition.compactable) { if (!installed.has("base.semanticActivation.compact")) throw new TypeError("base.client.endpointMismatch"); handle["compact"] = (storeId: string, token: BaseSemanticActivationCompactToken, idempotencyKey: string, signal?: AbortSignal) => executeSemanticControl(transport, ":compact", "compact-retired-semantic-authority", storeId, token, idempotencyKey, signal); }
    if (definition.removable) { if (!installed.has("base.semanticActivation.remove")) throw new TypeError("base.client.endpointMismatch"); handle["remove"] = (storeId: string, token: BaseSemanticActivationRemoveToken, idempotencyKey: string, signal?: AbortSignal) => executeSemanticControl(transport, ":remove", "remove-semantic-definition", storeId, token, idempotencyKey, signal); }
    result[name] = Object.freeze(handle);
  }
  return Object.freeze(result) as BaseSemanticActivationControlSurface<T>;
}

async function readSemanticControl(transport: BaseHttpTransport, definition: BaseSemanticActivationDefinitionShape, storeId: string, signal?: AbortSignal): Promise<BaseResult<BaseSemanticActivationControlDescriptor>> {
  if (!boundedIdentifier(storeId, 256)) throw new TypeError("base.client.requestInvalid");
  const response = await transport.json("POST", `control/stores/${encodeURIComponent(storeId)}/semantic-activations/${encodeURIComponent(definition.id)}/control`, new Uint8Array(encode({ definitionVersion: definition.version, definitionChecksum: definition.checksum })), signal);
  return response.ok ? isSemanticControlDescriptor(response.value, definition) ? success(freezeSemanticControlDescriptor(response.value), response.correlationId) : invalid(response.correlationId) : response;
}

async function executeSemanticControl(transport: BaseHttpTransport, route: string, confirmation: string, storeId: string, token: string, idempotencyKey: string, signal?: AbortSignal): Promise<BaseResult<BaseSemanticActivationControlResult>> {
  if(!boundedIdentifier(storeId,256)||!semanticControlToken(token)||!rawIdentity(idempotencyKey))throw new TypeError("base.client.requestInvalid");
  const response = await transport.json("POST", `control/stores/${encodeURIComponent(storeId)}/semantic-activations${route}`, new Uint8Array(encode({ commandToken: token, idempotencyKey, confirmation })), signal);
  return response.ok ? isSemanticControlResult(response.value) ? success(Object.freeze(response.value), response.correlationId) : invalid(response.correlationId) : response;
}

async function resolveSemanticControl(transport: BaseHttpTransport, storeId: string, token: string, signal?: AbortSignal): Promise<BaseResult<BaseSemanticActivationControlResult>> {
  if (!boundedIdentifier(storeId, 256) || !semanticControlToken(token)) throw new TypeError("base.client.requestInvalid");
  const response = await transport.json("POST", `control/stores/${encodeURIComponent(storeId)}/semantic-activations/maintenance:resolve`, new Uint8Array(encode({ resolutionToken: token })), signal);
  return response.ok ? isSemanticControlResult(response.value) ? success(Object.freeze(response.value), response.correlationId) : invalid(response.correlationId) : response;
}
...
function validSemanticDefinition(value: BaseSemanticActivationDefinitionShape): boolean { return exactObject(value, ["id","version","checksum","compactable","removable"])&&boundedIdentifier(value.id,256)&&Number.isInteger(value.version)&&value.version>0&&value.version<=2_147_483_647&&base64Bytes(value.checksum,32)&&typeof value.compactable==="boolean"&&typeof value.removable==="boolean"; }
function isSemanticControlDescriptor(value: unknown, definition: BaseSemanticActivationDefinitionShape): value is BaseSemanticActivationControlDescriptor { if(!exactObject(value,["definitionId","definitionVersion","authorityGeneration","liveCount","retiredCount","absenceCount","ready","quarantined","compactToken","removeToken"]))return false;const item=value as unknown as BaseSemanticActivationControlDescriptor;const unavailable=item.ready===false&&item.authorityGeneration===null&&item.liveCount===null&&item.retiredCount===null&&item.absenceCount===null&&item.compactToken===null&&item.removeToken===null;const available=item.ready===true&&item.quarantined===false&&positiveDecimal(item.authorityGeneration)&&nonnegativeDecimal(item.liveCount)&&nonnegativeDecimal(item.retiredCount)&&nonnegativeDecimal(item.absenceCount)&&(item.compactToken===null||semanticControlToken(item.compactToken))&&(item.removeToken===null||semanticControlToken(item.removeToken));return item.definitionId===definition.id&&item.definitionVersion===definition.version&&typeof item.quarantined==="boolean"&&(unavailable||available); }
function freezeSemanticControlDescriptor(value: BaseSemanticActivationControlDescriptor): BaseSemanticActivationControlDescriptor { return Object.freeze({...value,compactToken:value.compactToken as BaseSemanticActivationCompactToken|null,removeToken:value.removeToken as BaseSemanticActivationRemoveToken|null}); }
function isSemanticControlResult(value: unknown): value is BaseSemanticActivationControlResult { if(!exactObject(value,["disposition","authorityGeneration","examinedRows","changedRows","canonicalBytes","receiptDisposition","resumeToken","resolutionToken","sanitizedChecksum"]))return false;const item=value as unknown as BaseSemanticActivationControlResult;if(!positiveDecimal(item.authorityGeneration)||!nonnegativeDecimal(item.examinedRows)||!nonnegativeDecimal(item.changedRows)||!nonnegativeDecimal(item.canonicalBytes)||!base64Bytes(item.sanitizedChecksum,32))return false;switch(item.disposition){case "Completed":return item.receiptDisposition==="Committed"&&item.resumeToken===null&&item.resolutionToken===null;case "Duplicate":return item.receiptDisposition==="Duplicate"&&item.resumeToken===null&&item.resolutionToken===null;case "InProgress":return item.receiptDisposition==="Committed"&&semanticControlToken(item.resumeToken)&&item.resolutionToken===null;case "ConfirmedRolledBack":return item.receiptDisposition===null&&item.resumeToken===null&&item.resolutionToken===null;case "Indeterminate":return item.receiptDisposition===null&&item.resumeToken===null&&semanticControlToken(item.resolutionToken);default:return false;} }
function semanticControlToken(value: unknown): value is string { return typeof value==="string"&&value.length>=1&&value.length<=4096&&/^[A-Za-z0-9_-]+$/u.test(value); }
...
function rawIdentity(value: unknown): value is string { return typeof value==="string"&&value.length!==0; }
