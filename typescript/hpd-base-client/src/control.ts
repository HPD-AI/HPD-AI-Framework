...
class BaseControlPlaneSurfaceImplementation implements BaseControlPlaneSurface {
  ...
  public async purge(request: BasePurgeRequest, signal?: AbortSignal): Promise<BaseResult<BasePurgeResult>> { const result = await this.transport.json<unknown>("POST", "administration/purge", new Uint8Array(encode(request)), signal); return result.ok ? (isPurgeResult(result.value) ? success(result.value, result.correlationId) : invalid(result.correlationId)) : result; }
