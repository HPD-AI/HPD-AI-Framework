...
describe("Gateway declaration editor contract", () => {
  it("correlates all 365 fields and pins deterministic source identity", () => {
    const contract = createEditorContract(parseSnapshot(snapshotBytes), parseEditorLedger(ledgerBytes));
    expect(contract.fields).toHaveLength(365);
    expect(contract.sourceSha256).toBe("20337124f7ec5bfcba1677cd9d7cc93d677c3d872e81def279c9c601ffa12c81");
  ...
  it("rejects missing, duplicate, reordered, and moved occurrences", () => {
    ...
    expect(() => createEditorContract(snapshot, parseLedger(missing))).toThrow(/365 records/u);
