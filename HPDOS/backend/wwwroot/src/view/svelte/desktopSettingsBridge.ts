export type DesktopSettingsRequest<Method extends string, Params> = {
  source: string;
  type: "request";
  id: number;
  method: Method;
...
export function requestDesktopSettings<Method extends string, Params>(
  source: string,
  method: Method,
  params: Params
): Promise<unknown> {
  ...
  return new Promise((resolve, reject) => {
    ...
    const packet: DesktopSettingsRequest<Method, Params> = {
      source,
      type: "request",
      id,
      method,
...
function installDesktopResponseHandler(): void {
  ...
  window.addEventListener("hpdos-desktop-settings-response", (event) => {
...
function handleDesktopResponse(message: unknown): boolean {
  ...
  const packet = message as Partial<DesktopSettingsResponse>;
  if (
