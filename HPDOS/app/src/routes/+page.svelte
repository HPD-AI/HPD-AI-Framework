<script lang="ts">
	...
	import type { RunConfig as AgentRunConfig } from '@hpd-research/hpd-agent-client';
	import type {
	...
	function createWorkspaceInstance(sessionId?: string, branchId?: string) {
		const agentApiBaseUrl = runtimeDetails.agentApi
			? `${backendUrl}${runtimeDetails.agentApi}`
			: `${backendUrl}/api/hpd-agent`;
...
{#snippet Icon(name: IconName, size = 'md')}
	<svg class="icon" data-size={size} viewBox="0 0 24 24" aria-hidden="true">
		{#if name === 'arrow-up'}
			<path d="M12 19V5" />
		...
		{:else if name === 'folder-search'}
			<path d="M3 7a2 2 0 0 1 2-2h5l2 2h7a2 2 0 0 1 2 2v4.5" />
			<path d="M3 9v8a2 2 0 0 0 2 2h8" />
			<circle cx="17" cy="17" r="3" />
			<path d="m21 21-2-2" />
		{:else if name === 'folders'}
			<path d="M3 7a2 2 0 0 1 2-2h4l2 2h6a2 2 0 0 1 2 2v1" />
			<path d="M5 11a2 2 0 0 1 2-2h4l2 2h6a2 2 0 0 1 2 2v5a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2Z" />
		{:else if name === 'hammer'}
		...
		<aside class="sidebar" class:collapsed={sidebarCollapsed}>
				...
				<div class="section-header">
					...
					<div class="icon-row">
						...
						<button class="icon-button" type="button" aria-label={sidebarCollapsed ? 'Expand sidebar' : 'Collapse sidebar'} title={sidebarCollapsed ? 'Expand sidebar' : 'Collapse sidebar'} onclick={() => (sidebarCollapsed = !sidebarCollapsed)}>
							{@render Icon(sidebarCollapsed ? 'panel-left-open' : 'panel-left-close')}
						</button>
					</div>
				</div>
						...
						{#each workspaceProfiles as profile (profile.key)}
							...
							>
								<button type="button" class="workspace-select" onclick={() => selectWorkspace(profile)}>
									<span class="workspace-icon">{@render Icon(profile.roots && profile.roots.length > 1 ? 'folders' : 'folder', 'sm')}</span>
									{#if !sidebarCollapsed}
								...
								{#if !sidebarCollapsed}
									<button class="mini-button" type="button" aria-label="Edit workspace" onclick={() => openWorkspaceModal('edit', profile)}>{@render Icon('edit', 'xs')}</button>
									<button class="mini-button danger" type="button" aria-label="Delete workspace" onclick={() => deleteWorkspace(profile)}>{@render Icon('trash', 'xs')}</button>
								{/if}
							</div>
						{/each}
					</div>
			...
			<section class="sidebar-section sessions-section">
				<div class="section-header">
					...
					<button class="icon-button" type="button" aria-label="Create new session" title="Create new session" onclick={createSessionForActiveWorkspace}>{@render Icon('plus')}</button>
				</div>
			...
			<section class="sidebar-footer">
				<button class="settings-button" type="button" onclick={() => (settingsOpen = true)}>
					<span class="settings-icon">{@render Icon('settings', 'sm')}</span>
					{#if !sidebarCollapsed}
						<span>
																			...
																			<div class="exploration-body">
																				...
																				{#each explorationCalls as toolCall (toolCall.callId)}
																					<details class="exploration-op">
																						<summary>
																							<span>
																								{@render Icon(toolCall.status === 'error' ? 'shield-alert' : toolCall.status === 'complete' ? 'check' : 'loader-circle', 'xs')}
																								{summarizeExplorationTool(toolCall)}
																							</span>
																			...
																			{#snippet children(toolState)}
																				<ToolExecution.Trigger class="tool-trigger">
																					{#snippet children(triggerState)}
																						{@render Icon(toolState.status === 'error' ? 'shield-alert' : toolState.status === 'complete' ? 'check' : 'hammer', 'sm')}
																						<span>{toolState.name}</span>
																						<small>{toolState.status}</small>
																						<span class="tool-chevron" data-expanded={triggerState.expanded}>{@render Icon('chevron-down', 'xs')}</span>
																					{/snippet}
																...
																{#snippet children(actions)}
																	<div class="message-actions" data-role={message.role}>
																		{#if message.role === 'user'}
																			<button type="button" class="action-button" onclick={editState.startEdit}>{@render Icon('edit', 'xs')}<span>Edit</span></button>
																		{/if}
																		<MessageActions.RetryButton>
																			{#snippet child(retry)}
																				<button {...retry.props} type="button" class="action-button" onclick={retry.retry}>{@render Icon('retry', 'xs')}<span>Retry</span></button>
																			{/snippet}
																		</MessageActions.RetryButton>
																		<MessageActions.CopyButton content={message.content}>
																			{#snippet child(copy)}
																				<button {...copy.props} type="button" class="action-button" onclick={copy.copy}>{@render Icon(copy.copied ? 'check' : 'copy', 'xs')}<span>{copy.copied ? 'Copied' : 'Copy'}</span></button>
																			{/snippet}
				...
				<div class="workspace-surface">
					<header>
						<div class="surface-title">
							<span class="surface-icon">{@render Icon('cpu')}</span>
							<span>
							<h2>HPD-OS</h2>
			...
			<footer class="composer-area">
								...
								<ChatInput.Bottom>
									{#snippet children(inputState)}
										<div class="model-strip">
											<span class="model-icon">{@render Icon('plus', 'xs')}</span>
											<span>{runConfig.modelId ?? 'Default model'}</span>
											{#if isStreaming}
												<button type="button" class="stop-button" onclick={() => workspace?.abort()}>{@render Icon('square', 'xs')}<span>Stop</span></button>
											{/if}
										</div>
										<button class="send-button" type="button" aria-label="Send message" disabled={!inputState.canSubmit} onclick={inputState.submit}>{@render Icon('arrow-up', 'sm')}</button>
									{/snippet}
								</ChatInput.Bottom>
							</div>
						{/snippet}
				...
				<div class="monitor-card">
					<button class="icon-button monitor-toggle" type="button" aria-label={transcriptCollapsed ? 'Expand transcript' : 'Collapse transcript'} onclick={() => (transcriptCollapsed = !transcriptCollapsed)}>
						{@render Icon(transcriptCollapsed ? 'panel-left-open' : 'panel-left-close')}
					</button>
			...
			<section class="settings-modal workspace-modal">
				<header class="modal-header">
					<div class="modal-title">
						<span class="modal-icon">{@render Icon('folders')}</span>
						<span>
					...
					<button class="icon-button" type="button" aria-label="Close workspace modal" onclick={closeWorkspaceModal}>{@render Icon('x')}</button>
				</header>
	...
	{#if settingsOpen}
		<div class="modal-backdrop">
			<section class="settings-modal">
				<header class="modal-header">
					<div class="modal-title">
						<span class="modal-icon">{@render Icon('sliders')}</span>
						<span>
						<h2>Workspace Execution Parameters</h2>
						<p>Configure provider, model, credentials, and run behavior.</p>
						</span>
					</div>
					<button class="icon-button" type="button" aria-label="Close settings" onclick={() => (settingsOpen = false)}>{@render Icon('x')}</button>
				</header>
							...
							<PermissionDialog.Header>
								{#snippet child(header)}
									<div class="permission-title">
										<span class="modal-icon warning">{@render Icon('shield-alert')}</span>
...
<style>
	...
	.exploration-op > summary > span:first-child {
		min-width: 0;
		display: inline-flex;
		align-items: center;
		gap: 0.4rem;
	...
	.tool-chevron[data-expanded='true'] {
