...
{
    internal static IAuthorityJournalV1 Wrap(IAuthorityJournalV1 inner,Action<int> observed)
    {
        var scripted=new ScriptedAuthorityJournal();scripted.ReadHandler=async (request,token)=>{var result=await inner.ReadAsync(request,token);observed(scripted.ReadCalls);return result;};
        return scripted;
    }
    [Fact]
    ...
    public async Task Scripted_journal_is_a_borrowed_neutral_port()
    {
        ...
        var result=await journal.ReadAsync(default);
    ...
    private sealed class ScriptedAuthorityJournal : IAuthorityJournalV1
    {
        ...
        public ValueTask<AppendAuthorityResultV1> AppendAsync(AppendAuthorityBatchV1 request,CancellationToken cancellationToken=default)=>throw new InvalidOperationException();
        public ValueTask<ReadAuthorityRangeResultV1> ReadAsync(ReadAuthorityRangeV1 request,CancellationToken cancellationToken=default){ReadCalls++;return ReadHandler?.Invoke(request,cancellationToken)??ValueTask.FromResult<ReadAuthorityRangeResultV1>(new ReadAuthorityRangeResultV1.StoreUnavailable(new BoundedAscii("test-unavailable")));}
    }
}
