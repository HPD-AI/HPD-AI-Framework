...
public sealed class LanguageServerMiddlewareTests
{
    ...
    private static string? ResolveTypeScriptSmokeRoot()
    {
        ...
        {
            Path.Combine(Directory.GetCurrentDirectory(), "typescript", "hpd-agent-acp"),
            Path.Combine(Directory.GetCurrentDirectory(), "typescript", "hpd-agent-client"),
            Path.Combine(Directory.GetCurrentDirectory(), "typescript", "hpd-agent-headless-ui")
        };
