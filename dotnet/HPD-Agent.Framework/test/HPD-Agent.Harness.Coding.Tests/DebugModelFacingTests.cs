...
public sealed class DebugModelFacingTests
{
    ...
    public void EveryPublicAction_HasAPermissionClassification()
    {
        ...
        actions.Select(GetPermissionClass).Should().HaveCount(49);
    }
    ...
    public void PermissionClassification_DistinguishesRiskFamilies(
        ...
        => GetPermissionClass(action).Should().Be(expected);
    ...
    public void GeneratedDebugActions_DeclareDistinctStaticPermissionScopes()
    {
        ...
        var mutation = typeof(WriteDebugMemoryOperation)
            .GetCustomAttributes(typeof(AIFunctionActionAttribute), false)
            .Cast<AIFunctionActionAttribute>().Single();
    ...
    public void DebuggingRegistration_IncludesTheModelFacingBoundary()
    {
        ...
        services.Should().NotContain(descriptor => descriptor.ServiceType.Name == "DebugPermissionMiddleware");
    ...
    private static AIFunctionActionAttribute GetActionAttribute(string action)
        => typeof(DebugOperation).GetCustomAttributes<JsonDerivedTypeAttribute>()
    ...
    private static DebugPermissionClass GetPermissionClass(string action)
        => GetActionAttribute(action).PermissionScope switch
        {
            ...
            _ => throw new InvalidOperationException($"Unknown generated debug scope for '{action}'.")
        };
