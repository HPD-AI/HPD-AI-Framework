...
int main(void){
  ...
  const uint8_t advance_req[]={0xa2,0x01,0x01,0x02,0x45,0xa2,0x01,0x01,0x02,0x05},step_req[]={0xa2,0x01,0x02,0x02,0x43,0xa1,0x01,0x07},explore_req[]={0xa2,0x01,0x03,0x02,0x43,0xa1,0x01,0x03};
  ...
  if(explore(h,explore_req,sizeof explore_req,&r)||r.payload.len!=104)return 3;free(&r.payload);
  if(complete(h,&r)||r.payload.len!=104)return 4;free(&r.payload);
