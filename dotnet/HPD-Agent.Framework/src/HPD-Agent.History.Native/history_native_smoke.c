...
int main(void){
  ...
  if(hpd_history_query_next(h,&out,&e)||out.written!=sizeof req||memcmp(out_bytes,req,sizeof req))return 3;
  ...
  if(hpd_history_export_start(&in,&h,&e)||hpd_history_export_status(h,&out,&e)||out.written!=80)return 7;
  ...
  if(hpd_privacy_delete_start(&in,&h,&e)||hpd_privacy_status(h,&out,&e)||hpd_privacy_cancel(h,&e))return 9;
  if(hpd_privacy_hold_start(&in,&h,&e)||hpd_privacy_hold_release(h,&in,&e))return 10;
