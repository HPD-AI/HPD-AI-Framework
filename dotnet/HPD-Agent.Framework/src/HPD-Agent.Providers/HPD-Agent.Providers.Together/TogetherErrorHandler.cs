...
using TryAgiTogether = Together;

namespace HPD.Agent.Providers.Together;

internal class TogetherErrorHandler : IProviderErrorHandler
{
    public ProviderErrorDetails? ParseError(Exception exception)
    {
        if (exception is TryAgiTogether.ApiException apiException)
        {
    ...
    {
        var match = Regex.Match(message, @"""(?:code|error_code)""\s*:\s*""([^""]+)""", RegexOptions.IgnoreCase);
        if (match.Success)
        {
