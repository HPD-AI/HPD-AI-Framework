...
internal sealed class MultiAgentConversationRuntime : IMultiAgentConversationRuntime
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var existing = await _store.ProjectThreadAsync(_sessionId, threadId, ThreadProjectionPurpose.ModelContext, cancellationToken).ConfigureAwait(false);
        if (existing != null)
        {
            return threadId;
        }
        ...
        if (forkFromRoot)
        {
            ...
            var root = await _store.ProjectThreadAsync(_sessionId, rootThreadId, ThreadProjectionPurpose.ModelContext, cancellationToken).ConfigureAwait(false)
        ...
        var thread = await _store.ProjectThreadAsync(_sessionId, threadId, ThreadProjectionPurpose.ModelContext, cancellationToken).ConfigureAwait(false);
        if (thread != null)
        {
