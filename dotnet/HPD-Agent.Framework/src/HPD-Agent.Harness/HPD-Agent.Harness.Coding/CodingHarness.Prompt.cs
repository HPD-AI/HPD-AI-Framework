/// <summary>
/// System prompts and instructions for the CodingToolHarness.
/// Separated for maintainability and clarity.
/// </summary>
internal static class CodingToolHarnessPrompts
{
...
- ExecuteCommand accepts one closed request shape. For a normal command, use `{""request"":{""action"":""run"",""command"":""git status -sb""}}`.
- For background control, select the matching request branch, for example `{""request"":{""action"":""readOutput"",""operationId"":""cmd_..."",""delayMilliseconds"":1000}}`.
...
- Use executionMode: ""Background"" on the run request for long-running servers or watchers.
- When asked to run, open, view, or preview a static HTML file, do not use the macOS `open` command by default. Start a local static server from the file's directory, for example `python3 -m http.server 8000` with `executionMode: ""Background""`, then verify the page with curl before giving the URL.
...
- Verify a just-started background server or watcher with ExecuteCommand using the readOutput request branch, the returned operationId, and delayMilliseconds instead of running a separate sleep command.
