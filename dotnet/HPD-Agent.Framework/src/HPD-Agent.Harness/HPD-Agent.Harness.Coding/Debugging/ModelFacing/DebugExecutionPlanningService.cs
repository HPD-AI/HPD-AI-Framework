    ...
    IDebugChildSessionPlanFactory? childSessionPlanFactory = null)
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var workspacePath = string.IsNullOrWhiteSpace(operation.WorkspacePath)
            ? workspace.GetOwningRoot(canonicalTarget).Path
            : workspace.ResolveDirectory(operation.WorkspacePath);
        ...
        var evidence = await markerResolver.ResolveAsync(
            workspace, canonicalTarget, markers, cancellationToken).ConfigureAwait(false);
    ...
    private static string ResolveTarget(AgentWorkspace workspace, DebugTarget target)
        => target switch
        {
            SourceFileDebugTarget source => workspace.ResolvePath(source.Path),
            ApplicationProjectDebugTarget project =>
                ResolveProjectLike(workspace, project.Path),
            ExecutableDebugTarget executable => workspace.ResolvePath(executable.Path),
    ...
    {
        var candidate = workspace.ResolvePath(path);
