...
internal sealed class CodingExplorationStore
{
    ...
    public IReadOnlyList<CodingExplorationGroup> CaptureActiveGroups()
    {
        lock (_gate)
        {
            return _groups.Values
                .Where(static group => group.CaptureIsActive())
                .OrderByDescending(static group => group.LastUpdatedAt)
                .ToArray();
        }
    }

    public IReadOnlyList<CodingExplorationGroup> CaptureRecentGroups()
    {
        lock (_gate)
        {
            return _groups.Values
    ...
    public bool TryGetOperation(string callId, out CodingExplorationGroup group, out CodingExplorationOperation operation)
    {
        lock (_gate)
        {
            ...
            {
                operation = group.CaptureOperations().FirstOrDefault(item => string.Equals(item.CallId, callId, StringComparison.Ordinal))!;
