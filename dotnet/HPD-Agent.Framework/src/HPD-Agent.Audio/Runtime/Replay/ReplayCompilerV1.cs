...
internal static class ReplayCompilerV1
{
    ...
    {using var hash=IncrementalHash.CreateHash(HashAlgorithmName.SHA256);hash.AppendData("hpd-s10-replay-compilation-v1\0"u8);hash.AppendData(SessionAuthorityStampV1Codec.Encode(session));Span<byte>n=stackalloc byte[8];BinaryPrimitives.WriteInt64BigEndian(n,through);hash.AppendData(n);foreach(var fact in facts){BinaryPrimitives.WriteInt64BigEndian(n,fact.Position.Sequence);hash.AppendData(n);Span<byte>id=stackalloc byte[16];fact.FactId.TryWriteBytes(id);hash.AppendData(id);Span<byte>h=stackalloc byte[32];fact.PayloadHash.TryWriteBytes(h);hash.AppendData(h);BinaryPrimitives.WriteUInt16BigEndian(n,(ushort)fact.Owner);hash.AppendData(n[..2]);BinaryPrimitives.WriteInt32BigEndian(n, fact.Payload.Count);hash.AppendData(n[..4]);hash.AppendData(fact.PayloadMemory.Span);}return Hash256.FromBytes(hash.GetHashAndReset());}
}
