...
internal sealed class RouteSelectionCommandV1
{
    private readonly byte[] _body;

    internal RouteSelectionCommandV1(
        SessionAuthorityStampV1 session,
        ExpectedAuthorityVectorV1 expectedAuthority,
        ReadOnlySpan<byte> body)
    {
        RouteSelectionAuthorityPayloadCodecV1.Validate(session, expectedAuthority, body);
        Session = session;
...
internal static class RouteSelectionAuthorityPayloadRegistrationV1
{
    internal const ushort Discriminator = 23;

    internal static readonly AuthorityPayloadRegistrationV1 Command =
        AuthorityPayloadRegistrationV1.CreateOwnerRegistration(
            new BoundedAscii(RouteSelectionAuthorityPayloadCodecV1.SchemaId),
            RouteSelectionAuthorityPayloadCodecV1.Major,
            RouteSelectionAuthorityPayloadCodecV1.Minor,
            OwnerSliceId.S8,
            RouteSelectionAuthorityPayloadCodecV1.MaximumEncodedBytes,
            static (payload, session) =>
                RouteSelectionAuthorityPayloadCodecV1.TryDecode(payload, out var value) && value!.Session == session);
}

internal static class RouteSelectionAuthorityPayloadCodecV1
{
    internal const string SchemaId = "hpd.authority-payload-route-selection-command.v1";
    ...
    internal static byte[] Encode(RouteSelectionCommandV1 value)
    {
    ...
    internal static bool TryDecode(ReadOnlyMemory<byte> encoded, out RouteSelectionCommandV1? value)
    {
        value = null;
        ...
        try
        {
            ...
            value = new RouteSelectionCommandV1(session, authority, rented.AsSpan(0, bodyLength));
    ...
    internal static Hash256 ComputeHash(RouteSelectionCommandV1 value) =>
    ...
    internal static void Validate(SessionAuthorityStampV1 session, ExpectedAuthorityVectorV1 expectedAuthority, ReadOnlySpan<byte> body)
    {
        if (!session.IsValid || expectedAuthority is null || expectedAuthority.Session != session || body.Length > MaximumBodyBytes)
            throw new ArgumentException("Invalid route-selection authority payload.");
    }
}
