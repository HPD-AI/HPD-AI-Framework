...
internal sealed record GraphParticipantReservationRequestV1
{
    private readonly byte[] _bytes;
    internal GraphParticipantReservationRequestV1(ReadOnlyMemory<byte> exactCanonicalCommandBytes, CorrelationEnvelopeV1 correlation, UtcInstant observedAt, ulong maximumSessionRecords, ulong maximumSessionCanonicalBytes)
    {
        ...
        if (!GraphParticipantBindingCodecsV1.TryDecodeReservationCommand(exactCanonicalCommandBytes, out var outer) || outer is null || !GraphParticipantBindingCodecsV1.TryDecodeReservationCommandBody(outer.BodyBytes.ToArray(), out var body) || body is null || !correlation.IsValid || correlation.OperationId != body.OperationId || correlation.ParticipantId is not null) throw new ArgumentException("Invalid command proposal metadata.");
...
internal abstract record GraphParticipantReservationResultV1
{
    private GraphParticipantReservationResultV1() { }
    internal sealed record Applied : GraphParticipantReservationResultV1
    {
        private readonly byte[] _bytes;
    ...
    internal sealed record Rejected : GraphParticipantReservationResultV1
    {
        private readonly byte[] _bytes;
    ...
    internal sealed record RetryRequired(BoundedAscii SafeCode) : GraphParticipantReservationResultV1;
    internal sealed record StoreUnavailable(BoundedAscii SafeCode) : GraphParticipantReservationResultV1;
    internal sealed record OutcomeUnknown(JournalFactId FactId, BoundedAscii SafeCode) : GraphParticipantReservationResultV1;
    internal sealed record RealmFenced(ulong CurrentFenceEpoch) : GraphParticipantReservationResultV1;
    internal sealed record Quarantined(BoundedAscii SafeCode) : GraphParticipantReservationResultV1;
}

internal sealed class GraphParticipantReservationCoordinatorV1
{
    ...
    internal async ValueTask<GraphParticipantReservationResultV1> ReserveAsync(GraphParticipantReservationRequestV1 request, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (!GraphParticipantBindingCodecsV1.TryDecodeReservationCommand(bytes, out var outer) || outer is null || !GraphParticipantBindingCodecsV1.TryDecodeReservationCommandBody(outer.BodyBytes.ToArray(), out var body) || body is null)
                return new GraphParticipantReservationResultV1.Quarantined(new("invalid-payload"));
            ...
            if (query is GraphParticipantReservationFoldV1.AppliedReservation applied) return await ExistingAppliedAsync(applied, cancellationToken).ConfigureAwait(false);
            if (query is GraphParticipantReservationFoldV1.RejectedReservation rejected) return Existing(rejected);
            AuthorityFactEnvelopeV1? command;
            if (query is GraphParticipantReservationFoldV1.CommandOnly commandOnly)
            { if (!commandOnly.Command.PayloadBytes.SequenceEqual(bytes)) return Q("contradictory-duplicate"); command = commandOnly.Command; }
            else
            {
                var registration = GraphParticipantBindingPayloadRegistrationsV1.ReservationCommand;
                var factId = GraphParticipantBindingFactIdsV1.ReservationCommand(outer.Session, body.OperationId);
            ...
            for (var attempt = 0; attempt < 3; attempt++)
            {
                ...
                if (snapshotResult is GlobalParticipantAllocatorDurableSnapshotResultV1.RealmFenced fenced) return new GraphParticipantReservationResultV1.RealmFenced(fenced.CurrentFenceEpoch);
                ...
                for (var candidateAttempt = 0; candidateAttempt < 3; candidateAttempt++)
                {
                    ...
                    if (claim is GlobalParticipantAllocatorDurableClaimResultV1.OutcomeUnknown)
                    {
                        ...
                        catch (Exception) { return new GraphParticipantReservationResultV1.OutcomeUnknown(claimFact, new("allocator-outcome-unknown")); }
                        if (reconciliation is GlobalParticipantAllocatorReconcileResultV1.NotFound)
                        {
                            if (cancellationToken.IsCancellationRequested) return new GraphParticipantReservationResultV1.OutcomeUnknown(claimFact, new("allocator-outcome-unknown"));
                            continue;
                        }
                        ...
                        if (reconciliation is GlobalParticipantAllocatorReconcileResultV1.RealmFenced rf) return new GraphParticipantReservationResultV1.RealmFenced(rf.CurrentFenceEpoch);
                        if (reconciliation is GlobalParticipantAllocatorReconcileResultV1.StoreUnavailable && claimThrew) return new GraphParticipantReservationResultV1.OutcomeUnknown(claimFact, new("allocator-outcome-unknown"));
                        ...
                        return new GraphParticipantReservationResultV1.OutcomeUnknown(claimFact, new("allocator-outcome-unknown"));
                    }
                    ...
                    if (claim is GlobalParticipantAllocatorDurableClaimResultV1.RealmFenced rf2) return new GraphParticipantReservationResultV1.RealmFenced(rf2.CurrentFenceEpoch);
            ...
            return new GraphParticipantReservationResultV1.RetryRequired(new("allocator-head-advanced"));
        }
        finally { _mutex.Release(); }
    }

    private async ValueTask<(GraphParticipantReservationFoldV1? Fold, long Through, ulong Count, ulong Bytes, JournalPositionV1? AppliedFact, GraphParticipantReservationResultV1? Result)> ReadAsync(SessionAuthorityStampV1 session, GraphParticipantReservationRequestV1 request, CancellationToken token)
    {
        var fold = GraphParticipantReservationFoldV1.Create(session); long cursor = 0; long? through = null; ulong count = 0, total = 0;
        while (through is null || cursor < through)
        {
            ...
            foreach (var e in b.Facts) { count++; var length = AuthorityCanonicalCborV1.GetEnvelopeEncodedLength(e); total += length; if (count > request.MaximumSessionRecords || total > request.MaximumSessionCanonicalBytes || fold.Apply(e) is GraphParticipantReservationFoldV1.InvalidHistory) return (null, 0, 0, 0, null, Q("session-history-invalid")); }
            cursor = b.Facts.Count == 0 ? b.SnapshotThrough : b.Facts[^1].Position.Sequence;
        }
        var completed = fold.Complete(); return (fold, through ?? 0, completed.RecordCount, completed.TotalCanonicalBytes, completed.AppliedReservationFact, null);
    }
    private async ValueTask<(AuthorityFactEnvelopeV1? Envelope, GraphParticipantReservationResultV1? Result)> AppendCommandAsync(SessionAuthorityStampV1 session, OperationId operation, ProposedAuthorityFactV1 proposal, GraphParticipantReservationRequestV1 request, long head, CancellationToken token)
    {
        ...
        for (var i = 0; i < 3; i++)
        {
            ...
            if (history.Result is not null)
                return (null, history.Result is GraphParticipantReservationResultV1.StoreUnavailable ? new GraphParticipantReservationResultV1.OutcomeUnknown(factId, new("session-journal-outcome-unknown")) : history.Result);
            if (history.Fold!.Query(operation) is GraphParticipantReservationFoldV1.CommandOnly foundCommand)
            ...
            if (token.IsCancellationRequested)
                return (null, new GraphParticipantReservationResultV1.OutcomeUnknown(factId, new("session-journal-outcome-unknown")));
            head = history.Through;
        }
        return (null, new GraphParticipantReservationResultV1.OutcomeUnknown(factId, new("session-journal-outcome-unknown")));
    }
    private async ValueTask<GraphParticipantReservationResultV1> AppendResultAsync(AuthorityFactEnvelopeV1 command, GraphParticipantReservationCommandBodyV1 body, ParticipantId participant, ParticipantIdClaimOutcomeV1 outcome, GlobalParticipantAuthorityHeadV1 allocatorHead, JournalFactId allocatorClaimFact, GraphParticipantReservationRequestV1 request, CancellationToken token)
    {
        ...
        var code = outcome.Kind == 2 ? new BoundedAscii("participant-id-collision") : (BoundedAscii?)null;
        var factBody = new GraphParticipantReservationFactBodyV1(body.OperationId, command.Position, body.ExpectedReservationFact, outcome.Kind == 1 ? (ushort)1 : (ushort)2, body.RuntimeGeneration, body.ParticipantPlanFingerprint, body.TopologyFingerprint, body.ExecutablePlanFingerprint, reservation, code, body.ObservedAt);
        var bodyBytes = GraphParticipantBindingCodecsV1.Encode(factBody);
        var outer = GraphParticipantBindingCodecsV1.TryDecodeReservationCommand(command.PayloadMemory, out var commandOuter) && commandOuter is not null ? new GraphParticipantReservationFactV1(commandOuter.Session, commandOuter.ExpectedAuthority, bodyBytes) : throw new InvalidOperationException();
        var payload = GraphParticipantBindingCodecsV1.Encode(outer);
        var registration = GraphParticipantBindingPayloadRegistrationsV1.ReservationFact;
        var factId = GraphParticipantBindingFactIdsV1.ReservationFact(command.Position);
        ...
        for (var i = 0; i < 3; i++)
        {
            if (token.IsCancellationRequested) return new GraphParticipantReservationResultV1.OutcomeUnknown(allocatorClaimFact, new("allocator-outcome-unknown"));
            ...
            if (q is GraphParticipantReservationFoldV1.AppliedReservation appliedFound) return new GraphParticipantReservationResultV1.Applied(command.Position, appliedFound.Fact.Position, participant, allocatorHead, appliedFound.Fact.PayloadMemory);
            if (q is GraphParticipantReservationFoldV1.RejectedReservation rejectedFound) return new GraphParticipantReservationResultV1.Rejected(command.Position, rejectedFound.Fact.Position, rejectedFound.SafeCode, rejectedFound.Fact.PayloadMemory);
            if (!Nullable.Equals(history.AppliedFact, body.ExpectedReservationFact)) return Q("session-singleton-changed");
            ...
            try
            {
                ...
                if (append is AppendAuthorityResultV1.Committed or AppendAuthorityResultV1.AlreadyCommitted)
                {
                    ...
                    return outcome.Kind == 1 ? new GraphParticipantReservationResultV1.Applied(command.Position, accepted.Position, participant, allocatorHead, accepted.PayloadMemory) : new GraphParticipantReservationResultV1.Rejected(command.Position, accepted.Position, code!.Value, accepted.PayloadMemory);
                }
            ...
            catch (OperationCanceledException) when (token.IsCancellationRequested)
            {
                ...
                if (h.Result is not null) return new GraphParticipantReservationResultV1.OutcomeUnknown(factId, new("session-journal-outcome-unknown"));
                var found = h.Fold!.Query(body.OperationId);
                if (found is GraphParticipantReservationFoldV1.AppliedReservation canceledApplied) return new GraphParticipantReservationResultV1.Applied(command.Position, canceledApplied.Fact.Position, participant, allocatorHead, canceledApplied.Fact.PayloadMemory);
                if (found is GraphParticipantReservationFoldV1.RejectedReservation canceledRejected) return new GraphParticipantReservationResultV1.Rejected(command.Position, canceledRejected.Fact.Position, canceledRejected.SafeCode, canceledRejected.Fact.PayloadMemory);
                return new GraphParticipantReservationResultV1.OutcomeUnknown(factId, new("session-journal-outcome-unknown"));
            }
            catch (Exception) { }
        }
        return new GraphParticipantReservationResultV1.OutcomeUnknown(factId, new("session-journal-outcome-unknown"));
    }
    private async ValueTask<GraphParticipantReservationResultV1> ExistingAppliedAsync(GraphParticipantReservationFoldV1.AppliedReservation x, CancellationToken token)
    {
        ...
        if (result is GlobalParticipantAllocatorDurableSnapshotResultV1.RealmFenced f) return new GraphParticipantReservationResultV1.RealmFenced(f.CurrentFenceEpoch);
        ...
        if (!GraphParticipantBindingCodecsV1.TryDecodeReservationCommand(x.Command.PayloadMemory, out var commandOuter) || commandOuter is null || !GraphParticipantBindingCodecsV1.TryDecodeReservationCommandBody(commandOuter.BodyBytes.ToArray(), out var commandBody) || commandBody is null) return Q("allocator-snapshot-invalid");
        ...
        return new GraphParticipantReservationResultV1.Applied(x.Command.Position, x.Fact.Position, x.Reservation.ParticipantId, owner.ClaimHead, x.Fact.PayloadMemory);
    }
    private static GraphParticipantReservationResultV1 Existing(GraphParticipantReservationFoldV1.RejectedReservation x) => new GraphParticipantReservationResultV1.Rejected(x.Command.Position, x.Fact.Position, x.SafeCode, x.Fact.PayloadMemory);
    ...
    private static bool ReturnedCandidateMatches(GlobalParticipantAuthorityHeadV1 returnedHead, ulong returnedSequence, ReadOnlyMemory<byte> returnedBytes, GlobalParticipantAllocatorExactRecordSnapshotV1 snapshot, ReadOnlyMemory<byte> candidateBytes, JournalFactId factId, GraphParticipantReservationCommandV1 sourceOuter, GlobalParticipantClaimRecordBodyV1 candidateBody)
    {
    ...
    private static GraphParticipantReservationResultV1 Q(string code) => new GraphParticipantReservationResultV1.Quarantined(new(code));
    private static GraphParticipantReservationResultV1 S(string code) => new GraphParticipantReservationResultV1.StoreUnavailable(new(code));
}
