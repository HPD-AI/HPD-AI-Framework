...
var dotnetRoot = Path.Combine(repoRoot, "dotnet");
var textExtractRoot = Path.Combine(dotnetRoot, "HPD-Extract.Framework", "src", "HPD-Extract");
...
var targetFramework = options.TargetFramework ?? Environment.GetEnvironmentVariable("HPD_EXTRACT_AOT_TFM") ?? "net10.0";
var runtimeIdentifier = options.RuntimeIdentifier ?? Environment.GetEnvironmentVariable("HPD_EXTRACT_AOT_RID") ?? InferRuntimeIdentifier();
...
Console.WriteLine("==================================");
Console.WriteLine(" HPD Extract AOT PDF Smoke ");
Console.WriteLine("==================================");
...
sealed record AotSmokeOptions(string? ProjectPath, string? TargetFramework, string? RuntimeIdentifier)
{
    ...
    private static void PrintUsage()
    {
        ...
        Console.WriteLine("  HPD_EXTRACT_AOT_TFM overrides the default TFM.");
        Console.WriteLine("  HPD_EXTRACT_AOT_RID overrides runtime identifier inference.");
    }
}
