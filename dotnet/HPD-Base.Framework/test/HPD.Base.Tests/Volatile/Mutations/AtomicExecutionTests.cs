...
public sealed class AtomicExecutionTests
{
    ...
    public async Task SingleAndAtomicUseTheSameRestrictedSessionExecutor(bool atomic)
    {
        ...
        var read = await store.GetAsync(
            collection,
            new RecordId("one"),
            VolatileTestData.Operation(BaseOperationKind.Get));
    ...
    {
        var store = new VolatileRecordStore();
        var firstCollection = VolatileTestData.Collection("first");
        var secondCollection = VolatileTestData.Collection("second");
        var processor = new CallbackProcessor(async (session, token) =>
        {
            ...
            var observed = await session.GetAsync(
                firstCollection,
                new RecordId("shared"),
                VolatileTestData.Operation(BaseOperationKind.Get, "first"),
                token);
            ...
            var patched = await session.PatchAsync(
                firstCollection,
                new RecordId("shared"),
                new RecordPatchRequest { Patch = VolatileTestData.Patch("title", "after") },
        ...
        var second = await store.GetAsync(
            secondCollection,
            new RecordId("other"),
            VolatileTestData.Operation(BaseOperationKind.Get, "second"));
    ...
    public async Task FailedAtomicExecutionRollsBackRecordsAndCounters()
    {
        ...
        var failed = new CallbackProcessor(async (session, token) =>
        {
            var create = await session.CreateAsync(
                ...
                {
                    Payload = VolatileTestData.Payload(("title", "discard"))
                },
        ...
        var committed = await VolatileMutationTestDriver.CreateAsync(
            store,
            collection,
            new RecordCreateRequest
            {
                Payload = VolatileTestData.Payload(("title", "kept"))
            },
    ...
    public async Task ConcurrentCommitCausesConfirmedConflictWithoutRetryOrLostWrite()
    {
        ...
        await staged.Task;
        var winning = await VolatileMutationTestDriver.CreateAsync(
            store,
            collection,
            Create("winner", "keep"),
            VolatileTestData.Operation(BaseOperationKind.Create));
        ...
        (await store.GetAsync(
            collection,
            new RecordId("winner"),
            VolatileTestData.Operation(BaseOperationKind.Get))).Status.Should().Be(OperationStatus.Ok);
        (await store.GetAsync(
            collection,
            new RecordId("loser"),
            VolatileTestData.Operation(BaseOperationKind.Get))).Status.Should().Be(OperationStatus.NotFound);
    }
    ...
    {
        var capabilities = new VolatileRecordStore().Capabilities;
    ...
    {
        var capabilities = new VolatileRecordStore(
            new HPDBaseVolatileStoreOptions { AllowClientRequestedIds = false }).Capabilities;
    ...
    private static RecordCreateRequest Create(string id, string title) => new()
    {
        ...
        Payload = VolatileTestData.Payload(("title", title))
    };
        ...
        string eventId) => new()
    {
        ...
        EventId = eventId,
        Operation = VolatileTestData.Operation(
            mutation switch
            {
