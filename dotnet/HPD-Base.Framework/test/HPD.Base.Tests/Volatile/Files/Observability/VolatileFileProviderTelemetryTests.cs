...
namespace HPD.Base.Tests.Volatile.Files.Observability;

public sealed class VolatileFileProviderTelemetryTests
{
    [Fact]
    public async Task ProviderSpansAndMetricsDoNotLeakFileIdentityOrContentMarkers()
    {
        using var activities = new ActivityCollector(HPDBaseActivitySourceNames.FilesVolatile);
        using var metrics = new MeterCollector(HPDBaseMeterNames.FilesVolatile);
        var provider = new VolatileFileStorageProvider(Options.Create(new HPDBaseVolatileFileStoreOptions()));
        var bucket = Bucket();
