...
public sealed class FileRouteTests
{
    ...
    public async Task DescriptorRoutesAppearOnlyAfterFilesApiIsMappedAndRegistryRebuilt()
    {
        ...
        await unmapped.Services.GetRequiredService<HPD.Base.Runtime.Descriptors.IBaseDescriptorRegistry>().RebuildAsync();
        unmapped.Services.GetRequiredService<HPD.Base.Runtime.Descriptors.IBaseDescriptorRegistry>().Current.Manifest.Projections
        ...
        await mapped.Services.GetRequiredService<HPD.Base.Runtime.Descriptors.IBaseDescriptorRegistry>().RebuildAsync();
        var projection = mapped.Services.GetRequiredService<HPD.Base.Runtime.Descriptors.IBaseDescriptorRegistry>().Current.Manifest.Projections
    ...
    public async Task ListRouteBindsLimitAndCustomPrefixFlowsToLocationAndDescriptors()
    {
        ...
        await app.Services.GetRequiredService<HPD.Base.Runtime.Descriptors.IBaseDescriptorRegistry>().RebuildAsync();
        var projection = app.Services.GetRequiredService<HPD.Base.Runtime.Descriptors.IBaseDescriptorRegistry>().Current.Manifest.Projections
            .Should().ContainSingle(candidate => candidate.Id == FileModuleIds.AspNetCoreModule)
            .Subject;
    ...
    private static async Task<WebApplication> CreateAsync(bool mapFiles, bool useInMemory = false, bool mapOpenApi = false, string filesRoutePrefix = "/base/files")
    {
        ...
        if (mapOpenApi)
            HPD.Base.AspNetCore.OpenApi.HPDBaseOpenApiServiceCollectionExtensions.AddHPDBaseOpenApi(builder.Services);
        ...
        if (mapOpenApi)
            HPD.Base.AspNetCore.OpenApi.HPDBaseOpenApiEndpointRouteBuilderExtensions.MapHPDBaseOpenApi(app);
