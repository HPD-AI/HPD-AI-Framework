...
internal static class Program
{
    private static async Task<int> Main()
    {
        ...
        try
        {
            ...
            if (!(await session.Collection(AotVectorRecord.Collection).CreateAsync(new RecordId("one"), new AotVectorRecord { Label = "one", Tenant = "a", Active = true, Embedding = BaseVector.Create([1, 0]) })).Status.IsSuccess()) return 4;
            if (!(await session.Collection(AotVectorRecord.Collection).CreateAsync(new RecordId("two"), new AotVectorRecord { Label = "two", Tenant = "b", Active = false, Embedding = BaseVector.Create([0, 1]) })).Status.IsSuccess()) return 4;
