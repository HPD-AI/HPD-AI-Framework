...
public sealed class BaseStudioRetirementHttpTests
{
    ...
    private static async Task<BarrierFixture> CreateBarrierAsync(WebApplication app)
    {
        ...
        Assert.True(tombstoned.TryGetValue(out BaseSubjectTombstoneResult<RetirementHttpSubject>? tombstoneResult), tombstoned is BaseFailure<BaseSubjectTombstoneResult<RetirementHttpSubject>> failure ? failure.Error.Code : "missing tombstone value");
        BaseSubjectLifecycleFact<RetirementHttpSubject> tombstoneFact = tombstoneResult!.Fact;
