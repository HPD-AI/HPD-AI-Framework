...
internal sealed class SqliteTextMutationProjection : ISqliteAtomicMutationProjection, ISqliteAtomicMutationProjectionCatalog
{
    ...
    {
        _model = model;
        _statements = model.Indexes.SelectMany(index => new[] { Upsert(index), Delete(index), DeleteFts(index), InsertFts(index), StageUpsert(index), StageDelete(index), StageApplied(index), Advance(index), AdvancePurge(index) }).ToArray();
    }
    ...
    IReadOnlyList<string> ISqliteAtomicMutationProjectionCatalog.RequiredSchemaTables => [SqliteTextModel.StateTable, SqliteTextModel.RebuildReceiptTable, SqliteTextModel.RebuildProgressTable, SqliteTextModel.RebuildStageTable, SqliteTextModel.RebuildAppliedTable, .. _model.Indexes.SelectMany(static value => new[] { value.Table, value.FtsTable })];
    ...
    [
        ...
        [new("scope", "TEXT", true, true), new("operation", "TEXT", true, true), new("idempotency_key", "TEXT", true, true), new("fingerprint", "BLOB", true, false), new("collection_id", "TEXT", true, false), new("index_id", "TEXT", true, false), new("expected_generation", "INTEGER", true, false), new("staging_generation", "INTEGER", true, false), new("source_head", "INTEGER", true, false), new("publication_head", "INTEGER", false, false), new("applied_through", "INTEGER", false, false), new("applied_mutation_count", "INTEGER", true, false), new("applied_mutation_checksum", "BLOB", false, false), new("phase", "TEXT", true, false), new("last_record_id", "TEXT", false, false), new("record_count", "INTEGER", true, false), new("canonical_bytes", "INTEGER", true, false), new("rolling_checksum", "BLOB", true, false), new("scan_complete", "INTEGER", true, false)]),
        ...
        new(SqliteTextModel.RebuildAppliedTable,
        [new("scope", "TEXT", true, true), new("operation", "TEXT", true, true), new("idempotency_key", "TEXT", true, true), new("journal_position", "INTEGER", true, true), new("record_id", "TEXT", true, true)]),
    ...
    public async ValueTask<OperationResult> ApplyAsync(ISqliteAtomicProjectionContext context, BaseAtomicMutationProjectionRequest request, CancellationToken cancellationToken = default)
    {
        foreach (BaseAtomicMutationProjectionFact mutation in request.Mutations)
        {
            foreach (SqliteTextModel.IndexModel index in _model.Indexes.Where(value => value.Definition.CollectionId == mutation.CollectionId))
            {
                ...
                OperationResult<int> covered = await context.ExecuteAsync(StageAppliedId(index), [SqliteProjectionValue.Text("record", (mutation.After ?? mutation.Before!).Id.Value), SqliteProjectionValue.Integer("position", mutation.JournalPosition.Value)], cancellationToken).ConfigureAwait(false);
                if (!covered.Status.IsSuccess()) return Copy(covered);
    ...
    private static SqliteProjectionStatement StageApplied(SqliteTextModel.IndexModel index) => new(StageAppliedId(index), $"INSERT INTO {SqliteTextModel.RebuildAppliedTable}(scope,operation,idempotency_key,journal_position,record_id) SELECT scope,operation,idempotency_key,$position,$record FROM {SqliteTextModel.RebuildProgressTable} WHERE collection_id='{Escape(index.Definition.CollectionId)}' AND index_id='{Escape(index.Definition.Id)}' AND phase IN ('scan','catchup');", ["record", "position"], 64);
    ...
    private static string StageAppliedId(SqliteTextModel.IndexModel index) => index.Definition.Id + ".text.rebuild.stage.applied";
