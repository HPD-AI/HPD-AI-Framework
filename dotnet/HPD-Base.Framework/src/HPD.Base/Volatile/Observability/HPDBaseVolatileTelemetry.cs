...
internal static class HPDBaseVolatileTelemetry
{
    private static readonly Counter<long> Operations = HPDBaseVolatileObservability.Meter.CreateCounter<long>(
        HPDBaseTelemetryInstruments.StoreOperations,
        unit: "{operation}",
        description: "Counts HPD.BASE Volatile store operations.");

    private static readonly Histogram<double> OperationDuration = HPDBaseVolatileObservability.Meter.CreateHistogram<double>(
    ...
    {
        using var activity = HPDBaseVolatileObservability.ActivitySource.StartActivity(spanName, ActivityKind.Internal);
    ...
    private static void SetStartTags(Activity? activity, BaseOperationKind operation, string storeId, string collectionId)
    {
        ...
        activity.SetTag(HPDBaseTelemetryTags.ModuleId, HPDBaseTelemetryValues.ModuleVolatile);
        activity.SetTag(HPDBaseTelemetryTags.ProviderKind, HPDBaseTelemetryValues.ProviderVolatile);
    ...
    private static void Finish(Activity? activity, OperationStatus status, BaseError? error, BaseOperationKind operation, string storeId, string collectionId, long startedAt)
    {
        ...
        {
            { HPDBaseTelemetryTags.ModuleId, HPDBaseTelemetryValues.ModuleVolatile },
            { HPDBaseTelemetryTags.ProviderKind, HPDBaseTelemetryValues.ProviderVolatile },
