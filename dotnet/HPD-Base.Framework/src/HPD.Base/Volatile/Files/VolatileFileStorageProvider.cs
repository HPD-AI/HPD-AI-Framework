...
internal sealed class VolatileFileStorageProvider : IFileStorageProvider
{
    private readonly ConcurrentDictionary<string, VolatileFileObject> _objects = new(StringComparer.Ordinal);
    ...
    public ValueTask<OperationResult<FileObjectUploadResult>> UploadAsync(
        ...
        HPDBaseFilesVolatileTelemetry.TraceAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var stored = new VolatileFileObject
        {
    ...
    private bool TryGet(FileBucketId bucketId, FileObjectId objectId, out VolatileFileObject stored) =>
    ...
    private static OperationResult<T> Validation<T>(string target, string message) =>
        OperationResults.ValidationFailed<T>(Error("files.volatile.validation", message, ErrorCategory.Validation, target));
