...
internal sealed partial class VolatileRecordStore : IAtomicRecordStore, IStreamingRecordStore, IRelationalReadStore, IConsistentRecordIncludeStore
{
    ...
    private VolatileStoreState _publishedState = new();
    private long _generation;

    /// <summary>
    /// Initializes a new store using configured options.
    /// </summary>
    /// <param name="options">The configured Volatile options.</param>
    public VolatileRecordStore(IOptions<HPDBaseVolatileStoreOptions> options)
        : this(options.Value)
    {
    }
    ...
    /// </summary>
    /// <param name="options">The Volatile options.</param>
    public VolatileRecordStore(HPDBaseVolatileStoreOptions? options = null)
    {
    ...
    public ValueTask<OperationResult<RecordPage>> ListAsync(
        ...
        HPDBaseVolatileTelemetry.TraceAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (VolatileValidation.ValidateCollectionId<RecordPage>(collection.Id) is { } collectionError)
        {
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (VolatileValidation.ValidateCollectionId<RecordEnvelope>(collection.Id) is { } collectionError)
        {
            return ValueTask.FromResult(collectionError);
        }

        if (VolatileValidation.ValidateRecordId<RecordEnvelope>(id.Value) is { } idError)
        {
        ...
        if (record is null)
        {
            return ValueTask.FromResult(VolatileResultFactory.NotFound<RecordEnvelope>(id.Value));
        }

        var envelope = RecordCloneHelpers.CloneEnvelope(record);
        return ValueTask.FromResult(VolatileResultFactory.WithRevision(OperationResults.Ok(envelope), record.Metadata));
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        catch
        {
            await session.CloseAsync().ConfigureAwait(false);
            return Rollback(
                VolatileErrorCodes.MutationProcessorFailed,
            ...
            if (_generation != capturedGeneration)
            {
                return new RecordMutationExecutionResult(
                    ...
                    Error(
                        ...
                        "The Volatile mutation snapshot was superseded by a concurrent commit."));
            }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            return ValueTask.FromResult(VolatileResultFactory.Unsupported<RecordEnvelope>(
                VolatileErrorCodes.IdempotencyUnsupported,
                "Idempotency keys are not supported by HPD.BASE Volatile.",
                collection.Id));
        }
        ...
        {
            if (VolatileValidation.ValidateFieldName<RecordEnvelope>(field.Key) is { } fieldError)
            {
        ...
        {
            return ValueTask.FromResult(VolatileResultFactory.Unsupported<RecordEnvelope>(
                VolatileErrorCodes.RequestedIdUnsupported,
                "Client-requested ids are disabled for this Volatile store.",
                id.Value));
        }
        ...
        if (state.RecordsById.ContainsKey(id.Value))
            return ValueTask.FromResult(VolatileResultFactory.DuplicateId<RecordEnvelope>(id.Value));
        ...
        return ValueTask.FromResult(VolatileResultFactory.WithRevision(
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (VolatileValidation.ValidateCollectionId<DeleteResult>(collection.Id) is { } collectionError)
        {
            return ValueTask.FromResult(collectionError);
        }

        if (VolatileValidation.ValidateRecordId<DeleteResult>(id.Value) is { } idError)
        {
            return ValueTask.FromResult(idError);
        }
        ...
        if (state is null || !state.RecordsById.TryGetValue(id.Value, out var current))
            return ValueTask.FromResult(VolatileResultFactory.NotFound<DeleteResult>(id.Value));
        ...
        return ValueTask.FromResult(VolatileResultFactory.WithRevision(result, current.Metadata));
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            return ValueTask.FromResult(VolatileResultFactory.Unsupported<AsyncStream<RecordEnvelope>>(
                VolatileErrorCodes.UnsupportedQuery,
                "Streaming is disabled for this HPD.BASE Volatile store.",
                collection.Id));
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            return ValueTask.FromResult(VolatileResultFactory.Unsupported<RecordEnvelope>(
                VolatileErrorCodes.PatchUnsupportedShape,
                "Portable Volatile patch requires a field-map payload.",
                id.Value));
        }

        if (request.Patch.Fields is null || request.Patch.Fields.Count == 0)
        {
            return ValueTask.FromResult(VolatileResultFactory.Validation<RecordEnvelope>(
                VolatileErrorCodes.EmptyPatch,
        ...
        if (request.ExpectedRevision is { } expected && !RevisionEquals(current.Metadata.Revision, expected))
        {
            return ValueTask.FromResult(VolatileResultFactory.RevisionConflict<RecordEnvelope>(expected, current.Metadata.Revision, id.Value));
        }
    ...
    private static VolatileCollectionState GetOrCreateCollection(VolatileStoreState state, string collectionId)
    {
        ...
        collection = new VolatileCollectionState();
    ...
    private static RevisionToken NextRevision(VolatileStoreState state) => new($"mem:{++state.NextRevision:x16}");
    ...
    private static PayloadNormalizeResult<T> NormalizeObjectPayload<T>(RecordPayload payload)
    {
        if (payload is null)
        {
            return PayloadNormalizeResult<T>.Failure(VolatileResultFactory.Validation<T>(
                VolatileErrorCodes.PayloadRequired,
        ...
        {
            return PayloadNormalizeResult<T>.Failure(VolatileResultFactory.Validation<T>(
                VolatileErrorCodes.ObjectPayloadRequired,
        ...
        QuerySort[]? sort)
    {
        ...
        foreach (var sortField in sort)
        {
            foreach (var record in records)
            {
                ...
                {
                    return QueryResult<List<StoredRecord>, T>.Failure(VolatileResultFactory.Validation<T>(
                        VolatileErrorCodes.InvalidQuery,
        ...
        out OperationResult<T>? error)
    {
        ...
        error = VolatileResultFactory.Validation<T>(
            VolatileErrorCodes.InvalidQuery,
    ...
    private OperationResult<T>? ValidateUnsupportedQuery<T>(RecordQuery query, bool allowCount)
    {
        ...
        {
            return VolatileResultFactory.Unsupported<T>(VolatileErrorCodes.UnsupportedQuery, "Includes are not supported by HPD.BASE Volatile.");
        }

        if (query.Extensions is { Length: > 0 })
        {
            return VolatileResultFactory.Unsupported<T>(VolatileErrorCodes.UnsupportedQuery, "Query extensions are not supported by HPD.BASE Volatile.");
        }

        if (!allowCount && query.Count != QueryCountMode.None)
        {
            return VolatileResultFactory.Unsupported<T>(VolatileErrorCodes.UnsupportedQuery, "Streaming does not support count modes.");
        }

        if (query.Count is QueryCountMode.Estimated or QueryCountMode.Limited)
        {
            return VolatileResultFactory.Unsupported<T>(VolatileErrorCodes.UnsupportedQuery, "Estimated and limited count modes are not supported by HPD.BASE Volatile.");
        }

        if ((query.Sort?.Length ?? 0) > _options.MaxSortFields)
        {
            return VolatileResultFactory.Validation<T>(VolatileErrorCodes.InvalidQuery, "Query contains too many sort fields.");
        }

        if ((query.Select?.Length ?? 0) > _options.MaxSelectFields)
        {
            return VolatileResultFactory.Validation<T>(VolatileErrorCodes.InvalidQuery, "Query contains too many selected fields.");
        }
        ...
        foreach (var selectedField in query.Select ?? [])
        {
            ...
            if (segments.Length == 0
                ...
                || segments.Any(segment => VolatileValidation.ValidateFieldName<T>(segment) is not null))
            {
                return VolatileResultFactory.Validation<T>(VolatileErrorCodes.InvalidQuery, "Selected payload fields must be valid field paths.");
            }
        }

        if (query.Page?.Limit is < 0 || query.Page?.PerPage is < 0 || query.Page?.Offset is < 0)
        {
            return VolatileResultFactory.Validation<T>(VolatileErrorCodes.InvalidQuery, "Query pagination values must be non-negative.");
        }

        if (query.Page?.Page is <= 0)
        {
            return VolatileResultFactory.Validation<T>(VolatileErrorCodes.InvalidQuery, "Page mode is one-based.");
        }

        if ((query.Page?.Limit ?? query.Page?.PerPage) is { } requestedLimit && requestedLimit > _options.MaxPageSize)
        {
            return VolatileResultFactory.Validation<T>(VolatileErrorCodes.InvalidQuery, "Query page size exceeds the Volatile store limit.");
        }
    ...
    private OperationResult<T>? ValidateFilter<T>(FilterExpression filter, int depth, ref int nodeCount)
    {
        ...
        {
            return VolatileResultFactory.Validation<T>(VolatileErrorCodes.InvalidQuery, "Query filter exceeds Volatile limits.");
        }

        var error = filter.Kind switch
        {
            FilterNodeKind.Extension => VolatileResultFactory.Unsupported<T>(VolatileErrorCodes.UnsupportedQuery, "Filter extensions are not supported by HPD.BASE Volatile."),
            FilterNodeKind.Compare when filter.Operator is FilterOperator.Like or FilterOperator.NotLike => VolatileResultFactory.Unsupported<T>(VolatileErrorCodes.UnsupportedQuery, "Like and not-like filters are not supported by HPD.BASE Volatile."),
            FilterNodeKind.Compare when string.IsNullOrWhiteSpace(filter.Field) || filter.Value is null => VolatileResultFactory.Validation<T>(VolatileErrorCodes.InvalidQuery, "Compare filters require a field and value."),
            FilterNodeKind.In when string.IsNullOrWhiteSpace(filter.Field) || filter.Values is not { Length: > 0 } => VolatileResultFactory.Validation<T>(VolatileErrorCodes.InvalidQuery, "In filters require a field and values."),
            FilterNodeKind.In when filter.Values!.Length > _options.MaxInValues => VolatileResultFactory.Validation<T>(VolatileErrorCodes.InvalidQuery, "In filters contain too many values."),
            FilterNodeKind.Between when string.IsNullOrWhiteSpace(filter.Field) || filter.Values is not { Length: 2 } => VolatileResultFactory.Validation<T>(VolatileErrorCodes.InvalidQuery, "Between filters require a field and exactly two values."),
            FilterNodeKind.IsNull or FilterNodeKind.IsDefined when string.IsNullOrWhiteSpace(filter.Field) => VolatileResultFactory.Validation<T>(VolatileErrorCodes.InvalidQuery, "Field filter nodes require a field."),
            FilterNodeKind.Not when filter.Children is not { Length: 1 } => VolatileResultFactory.Validation<T>(VolatileErrorCodes.InvalidQuery, "Not filters require exactly one child."),
            FilterNodeKind.And or FilterNodeKind.Or when filter.Children is not { Length: > 0 } => VolatileResultFactory.Validation<T>(VolatileErrorCodes.InvalidQuery, "Boolean filters require children."),
            _ => null
        };
    ...
    private static void ValidateOptions(HPDBaseVolatileStoreOptions options)
    {
        ...
        if (options.CollectionIds.Any(id => !VolatileValidation.IsValidIdText(id)))
        {
    ...
    private static StoreCapabilityDescriptor CreateCapabilities(HPDBaseVolatileStoreOptions options) => new()
    {
        StoreId = options.StoreId,
        StoreKind = BaseStoreKinds.Volatile,
        ...
        Batch = new StoreBatchCapability
        {
            ...
            MaxOperations = HPDBaseVolatileDefaults.MaximumBatchOperations,
            MaxCanonicalPayloadBytes = HPDBaseVolatileDefaults.MaximumBatchCanonicalPayloadBytes,
    ...
    private sealed class AtomicSession : IAtomicRecordSession
    {
        ...
        private readonly VolatileRecordStore _owner;
        private readonly VolatileStoreState _working;
        ...
        public ValueTask<OperationResult<RecordEnvelope>> GetAsync(
            ...
            ExecuteAsync(
                cancellationToken,
                token => HPDBaseVolatileTelemetry.TraceAsync(
                    HPDBaseTelemetrySpans.StoreGet,
                    BaseOperationKind.Get,
        ...
        public ValueTask<OperationResult<RecordMutationSessionResult>> CreateAsync(
            ...
            ExecuteAsync(
                ...
                {
                    ArgumentNullException.ThrowIfNull(context);
                    var result = await HPDBaseVolatileTelemetry.TraceAsync(
        ...
        private static OperationResult<T> SessionClosed<T>() =>
            VolatileResultFactory.StoreError<T>(
                VolatileErrorCodes.SessionClosed,
                "The Volatile mutation session is no longer active.");

        private static OperationResult<T> SessionOperationCancelled<T>() =>
            VolatileResultFactory.StoreError<T>(
                VolatileErrorCodes.SessionOperationCancelled,
                "The Volatile mutation session operation was cancelled.");
    }
