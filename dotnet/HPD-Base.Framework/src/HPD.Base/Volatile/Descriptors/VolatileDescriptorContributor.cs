...
internal sealed class VolatileDescriptorContributor : IBaseDescriptorContributor
{
    ...
    public void Contribute(IBaseDescriptorContributionBuilder builder)
    {
        ...
        {
            builder.AddCapabilities(VolatileCapabilityDescriptorFactory.Create(_options, _store.Capabilities));
        }
    }
    ...
    private CollectionDefinition BindCollection(CollectionDefinition collection)
    {
        ...
        {
            throw new InvalidOperationException("Volatile collection contributions must be unbound or bound to the configured Volatile store id.");
        }
