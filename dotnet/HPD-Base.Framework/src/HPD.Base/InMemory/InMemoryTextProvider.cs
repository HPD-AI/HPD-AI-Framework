    ...
    private sealed class Session : IBaseTextHydrationSession
    {
        ...
        public ValueTask<OperationResult<BaseTextProviderResult>> SearchAsync(BaseTextExecutionRequest request, CancellationToken cancellationToken = default)
        {
            ...
            foreach (InMemoryTextCarrier carrier in projection.Carriers.Values)
            {
                ...
                ImmutableArray<BaseTextOrderingValue> ordering = BaseTextOrderingContract.Values(record.Payload, _index, plan.Order); ImmutableArray<byte> boundary = BaseTextOrderingContract.Boundary(evaluated.Score, ordering, record.Id); proofBytes = checked(proofBytes + BaseTextSemanticEvaluator.ProofRetainedBytes(evaluated.Proof)); orderingBytes = checked(orderingBytes + boundary.Length); prefixCount = checked(prefixCount + BaseTextSemanticEvaluator.PrefixExpansionCount(evaluated.Proof)); prefixBytes = checked(prefixBytes + BaseTextSemanticEvaluator.PrefixExpansionBytes(evaluated.Proof));
                candidates.Add(new BaseTextCandidate { RecordId = record.Id, Revision = record.Metadata.Revision!.Value, IndexedPosition = Snapshot.SearchVisibleThrough, Score = evaluated.Score, SecondaryOrdering = ordering, CanonicalOrderingBoundary = boundary, ScoreProof = evaluated.Proof });
            ...
            if (request.AfterBoundary is { } after) { int found = candidates.FindIndex(value => value.CanonicalOrderingBoundary.AsSpan().SequenceEqual(after.AsSpan())); if (found < 0) return ValueTask.FromResult(Invalid<BaseTextProviderResult>()); start = found + 1; }
            BaseTextCandidate[] ordered = candidates.Skip(start).Take(request.TakePlusOne).ToArray();
