...
internal sealed partial class InMemoryRecordStore : IAtomicRecordStore, IStreamingRecordStore, IRelationalReadStore, IConsistentRecordIncludeStore, IInMemoryProjectionAuthority, ITransactionalMutationJournalStore, IBaseSubjectAdministration, IBaseSubjectPublicationStore, IBaseSubjectValidationPlanReceiptStore, IBaseSubjectLifecycleStore, IBaseSubjectRetirementStore, IBaseSubjectAuthorityMaintenanceStore, IBaseActivationProvider
{
        ...
        TimeProvider timeProvider)
    {
        ...
        {
            if (string.IsNullOrEmpty(_options.SemanticActivationApplicationId)
                || _options.SemanticActivationOwnerGeneration <= 0
                || _options.SemanticActivationDefinitionSetChecksum.Length != 32)
                throw new InvalidOperationException("base.semanticActivation.contractInvalid");
            _publishedState.SemanticActivationAuthority = new BaseSemanticActivationStoreAuthorityRequirement
            {
                ApplicationId = new string(_options.SemanticActivationApplicationId.AsSpan()),
                LogicalStoreId = new string(_options.StoreId.AsSpan()),
                StoreInstanceId = new string(_options.StoreId.AsSpan()),
                RestoreEpoch = 0,
                SchemaGeneration = 1,
                SemanticAuthorityGeneration = _options.SemanticActivationOwnerGeneration,
                DefinitionSetChecksum = _options.SemanticActivationDefinitionSetChecksum.ToImmutableArray(),
            };
        }
        ...
        BaseSubjectLifecycleOrderingBoundary? lifecycleCutoff = _publishedState.SubjectLifecycleFacts
            ...
            .Select(static row => row.Boundary)
            .LastOrDefault();
        foreach (BaseSubjectLifecycleConsumerDefinition consumer in _options.SubjectLifecycleConsumers)
        {
            ...
            BaseExportedSubjectDefinition subject = _options.ExportedSubjects.Single(value =>
                value.Id == normalized.ContractId && value.Version == normalized.ContractVersion);
            ...
            _publishedState.SubjectLifecycleConsumers.Add($"{normalized.Id}\n{normalized.Version}", new(
                ...
                normalized.Limits.MaximumCheckpointLag));
        }
        ...
        CancellationToken cancellationToken = default)
    {
        if (_lifecycleMaintenance is not null)
            return ValueTask.FromResult(LifecycleMaintenanceRequired<RecordPage>());
    ...
    {
        if (_lifecycleMaintenance is not null)
            return ValueTask.FromResult(LifecycleMaintenanceRequired<RecordEnvelope>());
        var published = Volatile.Read(ref _publishedState);
    ...
    private sealed class AtomicSession : IAtomicRecordSession
    {
        ...
        private sealed class InMemorySemanticPreparedPlan : BaseSemanticActivationPreparedPlan
        {
            ...
            internal required long ResultingGeneration { get; init; }
            internal string? ActivationId { get; init; }
        }
            ...
            CancellationToken cancellationToken)
        {
            ...
            OperationResult<BaseCapturedSemanticActivationEvidence?> semanticResult = CaptureSemanticActivation(request.SemanticActivation, intent, digest, intervals);
        ...
        private OperationResult<BaseCapturedSemanticActivationEvidence?> CaptureSemanticActivation(
            ...
            ImmutableArray<BaseAtomicReadIntervalEvidence>.Builder enclosingIntervals)
        {
            ...
            BaseSemanticActivationOperation operation = extension.Operation;
            ...
            ImmutableArray<byte> canonicalKey = capture.CanonicalKey;
            BaseOwnedSubjectScopeEvidence scope = capture.Scope;
            BaseSemanticActivationOperationKind operationKind = operation is BaseSemanticActivationEnsureIntent
                ? BaseSemanticActivationOperationKind.Ensure : operation is BaseSemanticActivationRetireIntent
                    ? BaseSemanticActivationOperationKind.Retire : 0;
            BaseSemanticActivationDefinitionIdentity? operationDefinition = operation switch
            {
                BaseSemanticActivationEnsureIntent value => value.Definition,
                BaseSemanticActivationRetireIntent value => value.Definition,
                _ => null,
            };
            ImmutableArray<byte> operationKey = operation switch
            {
                BaseSemanticActivationEnsureIntent value => value.CanonicalKey,
                BaseSemanticActivationRetireIntent value => value.CanonicalKey,
                _ => [],
            };
            BaseOwnedSubjectScopeEvidence? operationScope = operation switch
            {
                BaseSemanticActivationEnsureIntent value => value.Scope,
                BaseSemanticActivationRetireIntent value => value.Scope,
                _ => null,
            };
            if (extension.StructuralDigest.Length != 32 || definition.Checksum.Length != 32
                || definition.OwnerGeneration < 1 || canonicalKey.IsDefaultOrEmpty
                ...
                || capture.ProposedScopeBindingId.Length != 32 || capture.Operation != operationKind
                || capture.StoreAuthority.DefinitionSetChecksum.Length != 32
                || capture.StoreAuthority.RestoreEpoch != intent.Authority.RestoreEpoch
                || capture.StoreAuthority.SchemaGeneration != intent.Authority.SchemaGeneration
                || capture.StoreAuthority.SemanticAuthorityGeneration < 1
                || !_owner.AcceptActivationTime(capture.AcceptedTime)
                || !string.Equals(capture.StoreAuthority.ApplicationId, intent.Authority.ApplicationId, StringComparison.Ordinal)
                || !string.Equals(capture.StoreAuthority.StoreInstanceId, intent.Authority.StoreInstanceId, StringComparison.Ordinal)
                || !string.Equals(capture.StoreAuthority.LogicalStoreId, _owner._options.StoreId, StringComparison.Ordinal)
                || operationDefinition is null || operationScope is null
                || operationDefinition.Id != definition.Id || operationDefinition.Version != definition.Version
                || operationDefinition.OwnerGeneration != definition.OwnerGeneration
                || !CryptographicOperations.FixedTimeEquals(operationDefinition.Checksum.AsSpan(), definition.Checksum.AsSpan())
                || operationDefinition.RetirementOperation != definition.RetirementOperation
                || !operationKey.AsSpan().SequenceEqual(canonicalKey.AsSpan())
                || operationScope.Kind != scope.Kind || !string.Equals(operationScope.Value, scope.Value, StringComparison.Ordinal)
                || capture.Limits.MaximumOperations < 1)
            ...
            if (proposed.Length != 32)
                return SubjectFailure<BaseCapturedSemanticActivationEvidence?>(BaseSubjectErrorCodes.ProviderContractInvalid);
            bool scopeExists = _working.SemanticActivationScopes.TryGetValue(scopeKey, out BaseSemanticActivationScopeBinding? binding);
            binding ??= SemanticScopeBinding(scope.Kind, protectedScope, proposed);
            byte[] keyBytes = SemanticKeyDigest(definition.Id, binding.BindingId.AsSpan(), canonicalKey.AsSpan());
            ...
            ImmutableArray<BaseAtomicReadIntervalEvidence> semanticIntervals = [scopeInterval, slotInterval];
            ...
            var scopeCapture = new BaseSemanticActivationScopeDirectoryCapture
            {
                ...
                ResultingBinding = binding, ReadIntervals = [scopeInterval], CanonicalBytes = binding.BindingId.Length + binding.ProtectedCanonicalScope.Length + binding.SeekDigest.Length,
                Checksum = SHA256.HashData(binding.Checksum.AsSpan()).ToImmutableArray(),
            };
            BaseSemanticActivationCapturedState state;
            BaseSemanticActivationMissingAuthority? missing = null;
            ...
            if (slot?.Live is not null)
            {
                ...
                if (!_working.Activations.TryGetValue(live.ActivationId, out InMemoryActivationRow? mapped))
                    return SubjectFailure<BaseCapturedSemanticActivationEvidence?>(BaseSubjectErrorCodes.ProviderContractInvalid);
            ...
            var capturedResult = new BaseCapturedSemanticActivationEvidence
            {
        ...
        {
            var requirement = source with
            {
                ApplicationId = new string(source.ApplicationId.AsSpan()), LogicalStoreId = new string(source.LogicalStoreId.AsSpan()),
        ...
        private static bool SemanticAuthorityEquals(
            BaseSemanticActivationStoreAuthorityRequirement installed,
            BaseSemanticActivationStoreAuthorityRequirement requested) =>
            string.Equals(installed.ApplicationId, requested.ApplicationId, StringComparison.Ordinal)
            && string.Equals(installed.LogicalStoreId, requested.LogicalStoreId, StringComparison.Ordinal)
            && string.Equals(installed.StoreInstanceId, requested.StoreInstanceId, StringComparison.Ordinal)
            && installed.RestoreEpoch == requested.RestoreEpoch
            && installed.SchemaGeneration == requested.SchemaGeneration
        ...
        private static byte[] SemanticKeyDigest(string definitionId, ReadOnlySpan<byte> bindingId, ReadOnlySpan<byte> canonicalKey)
        {
            ...
            hash.AppendData("base.semanticActivation.key.v1\0"u8);
            byte[] length = new byte[4];
            byte[] definition = Encoding.UTF8.GetBytes(definitionId);
            System.Buffers.Binary.BinaryPrimitives.WriteInt32BigEndian(length, definition.Length); hash.AppendData(length); hash.AppendData(definition);
            System.Buffers.Binary.BinaryPrimitives.WriteInt32BigEndian(length, bindingId.Length); hash.AppendData(length); hash.AppendData(bindingId);
            System.Buffers.Binary.BinaryPrimitives.WriteInt32BigEndian(length, canonicalKey.Length); hash.AppendData(length); hash.AppendData(canonicalKey);
            return hash.GetHashAndReset();
        }

        private static BaseSemanticActivationAccounting SemanticAccounting(long keyBytes, long evidenceBytes, int intervals) => new()
        {
            Operations = 1, ScopeDirectoryReads = 1, SlotReads = 1, ActivationReads = 0, ReadIntervals = intervals,
            IndexOperations = 0, KeyBytes = keyBytes, ScopeDirectoryBytes = 96, ActivationBytes = 0,
            EvidenceBytes = evidenceBytes, ReceiptBytes = 0, TransientBytes = checked(keyBytes + evidenceBytes + 96),
        ...
        private BasePreparedSemanticActivation? PrepareSemantic(
            BaseAtomicSemanticActivationExtension? extension,
            ...
            BaseAtomicMutationExecutionLimits enclosingLimits)
        {
            ...
            if (!CryptographicOperations.FixedTimeEquals(capturedKey.ToArray(), finalizedKey.ToArray()))
                throw new InvalidOperationException(BaseSubjectErrorCodes.ProviderContractInvalid);
            if (extension.Operation is BaseSemanticActivationEnsureIntent finalizedEnsure)
            {
                Span<byte> keyBytes = stackalloc byte[32]; finalizedKey.CopyTo(keyBytes);
                byte[] expectedActivationId = SemanticBoundHash("base.semanticActivation.activation.v1\0",
                    ...
                    Encoding.UTF8.GetBytes(finalizedEnsure.Definition.OwningModuleId),
                    Encoding.UTF8.GetBytes(finalizedEnsure.Definition.Id),
                    captured.ScopeDirectory.ResultingBinding.BindingId.ToArray(), finalizedEnsure.CanonicalKey.ToArray());
                byte[] expectedCreationChecksum = SemanticBoundHash("base.semanticActivation.creation.v1\0",
                    finalizedEnsure.Definition.Checksum.ToArray(), keyBytes.ToArray(),
                    captured.ScopeDirectory.ResultingBinding.BindingId.ToArray(), expectedActivationId);
                if (!finalizedEnsure.Activation.Identity.ScopeBindingId.AsSpan().SequenceEqual(captured.ScopeDirectory.ResultingBinding.BindingId.AsSpan())
                    || !finalizedEnsure.Activation.Identity.DerivedActivationIdBytes.AsSpan().SequenceEqual(expectedActivationId)
                    || !CryptographicOperations.FixedTimeEquals(finalizedEnsure.Activation.Identity.Checksum.AsSpan(), expectedCreationChecksum))
                    throw new InvalidOperationException(BaseSubjectErrorCodes.ProviderContractInvalid);
            }
            ...
            switch (kind, captured.State)
            {
                case (BaseSemanticActivationOperationKind.Ensure, BaseSemanticActivationCapturedState.Missing):
                    ...
                    BaseActivationProviderCapability capability = _owner.Descriptor.Capability;
                    BaseActivationLimits installedLimits = ensure.Activation.Limits;
                    ...
                    long activationEvidenceBytes = checked(activationBytes + 32 + sizeof(long) * 2);
                    if (!capability.AtomicCreationSupported || capability.MaximumActivationsPerTransaction < 1
                        || ensure.Activation.CanonicalInput.Length > Math.Min(capability.MaximumInputBytes, installedLimits.MaximumInputBytes)
                        || 1 > Math.Min(capability.MaximumDueCandidates, installedLimits.Provider.MaximumCandidates)
                        || 1 > Math.Min(capability.MaximumReadIntervals, installedLimits.Provider.MaximumReadIntervals)
                        || 2 > capability.MaximumIndexOperations
                        || activationEvidenceBytes > Math.Min(capability.MaximumEvidenceBytes, installedLimits.Provider.MaximumEvidenceBytes)
                        || activationEvidenceBytes > Math.Min(capability.MaximumTransientBytes, installedLimits.Provider.MaximumTransientBytes)
                        || 1 > installedLimits.Provider.MaximumCandidates
                        || 1 > installedLimits.Provider.MaximumReadIntervals
                        || 2 > installedLimits.Provider.MaximumIndexOperations
                        || 1 > Math.Min(installedLimits.AtomicCreation.MaximumProducedMutations, enclosingLimits.MaximumProducedMutations)
                        || 1 > Math.Min(installedLimits.AtomicCreation.MaximumReadIntervals, enclosingLimits.MaximumReadIntervals)
                        || activationEvidenceBytes > Math.Min(installedLimits.AtomicCreation.MaximumEvidenceBytes, enclosingLimits.MaximumEvidenceBytes)
                        || activationEvidenceBytes > Math.Min(installedLimits.AtomicCreation.MaximumTransientBytes, enclosingLimits.MaximumTransientBytes)
                        || activationEvidenceBytes > Math.Min(installedLimits.AtomicCreation.MaximumJournalBytes, enclosingLimits.MaximumJournalBytes)
                        || activationEvidenceBytes > Math.Min(installedLimits.AtomicCreation.MaximumFactBytes, enclosingLimits.MaximumFactBytes)
                        || activationEvidenceBytes > Math.Min(installedLimits.AtomicCreation.MaximumReceiptBytes, enclosingLimits.MaximumReceiptBytes))
                    ...
                    activationAccounting = new BaseActivationAccounting
                    {
                        ...
                        EvidenceBytes = activationEvidenceBytes, TransientBytes = activationEvidenceBytes,
                    };
                ...
                case (BaseSemanticActivationOperationKind.Ensure, BaseSemanticActivationCapturedState.Live):
                    ...
                    resulting = BaseSemanticActivationSlotState.Live; generation = captured.Live.SlotGeneration; break;
                ...
                case (BaseSemanticActivationOperationKind.Retire, BaseSemanticActivationCapturedState.Live):
                    ...
                    resulting = BaseSemanticActivationSlotState.Retired; generation = checked(captured.Live.SlotGeneration + 1); break;
                case (BaseSemanticActivationOperationKind.Retire, BaseSemanticActivationCapturedState.Retired):
                    activationId = captured.Retired!.ActivationId;
                    resulting = BaseSemanticActivationSlotState.Retired; generation = captured.Retired.SlotGeneration; break;
            ...
            var providerPlan = new InMemorySemanticPreparedPlan
            {
                ScopeKey = _capturedSemanticScopeKey, SlotKey = _capturedSemanticSlotKey,
                Binding = captured.ScopeDirectory.ResultingBinding, InsertScope = captured.ScopeDirectory.State == BaseSemanticActivationScopeDirectoryState.Missing,
                Extension = extension, PriorState = captured.State, ResultingState = resulting, ResultingGeneration = generation, ActivationId = activationId,
            };
            BaseSemanticActivationAccounting accounting = captured.Accounting with
            {
                ...
                ActivationReads = kind == BaseSemanticActivationOperationKind.Retire ? 1 : 0,
            ...
            var writeInterval = new BaseSemanticActivationWriteIntervalEvidence
            {
                ...
                LowerInclusive = true, Upper = Encoding.UTF8.GetBytes(_capturedSemanticSlotKey).ToImmutableArray(), UpperInclusive = true,
                Checksum = [],
            };
            writeInterval = writeInterval with { Checksum = BaseSemanticActivationEvidenceContract.WriteIntervalChecksum(writeInterval) };
            var preparedResult = new BasePreparedSemanticActivation
            {
                ...
                ReadIntervals = captured.ReadIntervals, WriteIntervals = [writeInterval],
                Accounting = accounting,
                Checksum = [],
            };
            return preparedResult with { Checksum = BaseSemanticActivationEvidenceContract.PreparedChecksum(extension, preparedResult) };
        }

        private static byte[] SemanticBoundHash(string marker, params byte[][] parts)
        {
            using IncrementalHash hash = IncrementalHash.CreateHash(HashAlgorithmName.SHA256);
            hash.AppendData(Encoding.UTF8.GetBytes(marker));
            byte[] length = new byte[4];
            ...
            CancellationToken cancellationToken = default) => ExecuteAsync(cancellationToken, async token =>
        {
            ...
            try { provisionalSemantic = ApplySemantic(prepared.SemanticActivation); }
        ...
        private BaseProvisionalSemanticActivation? ApplySemantic(BasePreparedSemanticActivation? prepared)
        {
            ...
            if (prepared.SessionPlan is not InMemorySemanticPreparedPlan semantic
                || !ReferenceEquals(prepared.SessionPlan, semantic))
                throw new InvalidOperationException(BaseSubjectErrorCodes.ProviderContractInvalid);
            if (semantic.InsertScope)
            {
                if (_working.SemanticActivationScopes.ContainsKey(semantic.ScopeKey))
                    throw new InvalidOperationException("base.semanticActivation.conflict");
                _working.SemanticActivationScopes.Add(semantic.ScopeKey, semantic.Binding);
            }
            ...
            if (operation is BaseSemanticActivationEnsureIntent ensure && semantic.PriorState == BaseSemanticActivationCapturedState.Missing)
            {
                ...
                var live = new BaseSemanticActivationLiveAuthority
                {
                    ...
                    ScopeBinding = semantic.Binding, SubjectLifetime = ensure.SubjectLifetime,
                    ActivationId = activationId, ActivationDefinition = ensure.Activation.Definition,
                    InputChecksum = ensure.Activation.InputChecksum, Due = ensure.Due, SlotGeneration = 1,
                ...
                _working.GlobalMutationPosition = checked(_working.GlobalMutationPosition + 1);
                activationGeneration = 1; activationChecksum = controlChecksum.ToImmutableArray();
            }
            else if (operation is BaseSemanticActivationEnsureIntent && semantic.PriorState == BaseSemanticActivationCapturedState.Live)
            {
                InMemoryActivationRow row = _working.Activations[semantic.ActivationId!];
                activationGeneration = row.Generation; activationChecksum = row.ControlChecksum.ToImmutableArray();
            }
            else if (operation is BaseSemanticActivationRetireIntent retire && semantic.PriorState == BaseSemanticActivationCapturedState.Live)
            {
                InMemoryActivationRow row = _working.Activations[semantic.ActivationId!];
                BaseSemanticActivationLiveAuthority prior = _working.SemanticActivationSlots[semantic.SlotKey].Live!;
                long position = checked(++_working.GlobalMutationPosition);
                byte[] receiptChecksum = row.TerminalReceiptChecksum?.ToArray()
                    ?? throw new InvalidOperationException("base.semanticActivation.activationNotTerminal");
            ...
            else if (semantic.ActivationId is not null && _working.Activations.TryGetValue(semantic.ActivationId, out InMemoryActivationRow? existing))
            {
                activationGeneration = existing.Generation; activationChecksum = existing.ControlChecksum.ToImmutableArray();
            }
        ...
        public ValueTask<OperationResult<BaseSubjectRetirementTimeoutResult>> ApplySubjectRetirementTimeoutAsync(
            BaseSubjectRetirementProviderTimeoutRequest request, CancellationToken cancellationToken = default) => ExecuteAsync(cancellationToken, _ =>
        {
            ArgumentNullException.ThrowIfNull(request);
            ...
            BaseProtectedSubjectScope scope = _owner._subjectScopes.Protect(request.Scope, _owner._subjectScopeProtectionKey);
            ...
            if (request.ObservedAtUtc <= stored.Barrier.DeadlineUtc)
                return ValueTask.FromResult(RetirementFailure<BaseSubjectRetirementTimeoutResult>("base.subjectRetirement.barrierPending", OperationStatus.Conflict, ErrorCategory.Conflict));
            BaseSubjectRetirementPolicy policy = _owner._options.SubjectRetirementPolicies.Single(value => value.ContractId == command.ContractId && value.ContractVersion == command.ContractVersion);
            BaseSubjectRetirementBarrierState timeoutState = policy.TimeoutBehavior == BaseSubjectRetirementTimeoutBehavior.Quarantine
                ? BaseSubjectRetirementBarrierState.Quarantined : BaseSubjectRetirementBarrierState.TimedOut;
            BaseSubjectRetirementBarrier resulting = RecomputeBarrier(stored, timeoutState);
            _working.SubjectRetirementBarriers[key] = stored with { Barrier = resulting };
            long position = checked(++_working.SubjectRetirementPosition);
            AddRetirementBarrierPublication(scope, timeoutState == BaseSubjectRetirementBarrierState.Quarantined
                ? BaseSubjectRetirementPublicationKind.BarrierQuarantined : BaseSubjectRetirementPublicationKind.BarrierTimedOut,
                resulting, stored.Barrier.Generation, null, position);
            return ValueTask.FromResult(OperationResults.Ok(new BaseSubjectRetirementTimeoutResult { Outcome = BaseSubjectRetirementMutationOutcome.Applied, State = resulting.State, Generation = resulting.Generation, BarrierChecksum = resulting.BarrierChecksum }));
        });

        public ValueTask<OperationResult<BaseSubjectRetirementOverrideResult>> ApplySubjectRetirementOverrideAsync(
            BaseSubjectRetirementProviderOverrideRequest request, CancellationToken cancellationToken = default) => ExecuteAsync(cancellationToken, _ =>
        {
            ArgumentNullException.ThrowIfNull(request);
            ...
            string key = RetirementKey(scope, command.ContractId, command.ContractVersion, command.SubjectId, command.AuthorityEpoch, command.Incarnation);
            if (!_working.SubjectRetirementBarriers.TryGetValue(key, out InMemorySubjectRetirementBarrierState? stored)
                || stored.Barrier.TombstoneSequence != command.ExpectedTombstoneSequence || stored.Barrier.Generation != command.ExpectedBarrierGeneration
                || stored.Barrier.BarrierChecksum != command.ExpectedBarrierChecksum)
                return ValueTask.FromResult(RetirementFailure<BaseSubjectRetirementOverrideResult>("base.subjectRetirement.overrideConflict", OperationStatus.Conflict, ErrorCategory.Conflict));
            if (stored.Barrier.State == BaseSubjectRetirementBarrierState.Overridden)
                return ValueTask.FromResult(OperationResults.Ok(new BaseSubjectRetirementOverrideResult { Outcome = BaseSubjectRetirementMutationOutcome.Obsolete, Generation = stored.Barrier.Generation, BarrierChecksum = stored.Barrier.BarrierChecksum }));
            if (stored.Barrier.State is not (BaseSubjectRetirementBarrierState.TimedOut or BaseSubjectRetirementBarrierState.Quarantined))
                return ValueTask.FromResult(RetirementFailure<BaseSubjectRetirementOverrideResult>("base.subjectRetirement.overrideConflict", OperationStatus.Conflict, ErrorCategory.Conflict));
            BaseSubjectRetirementBarrier resulting = RecomputeBarrier(stored, BaseSubjectRetirementBarrierState.Overridden);
            _working.SubjectRetirementBarriers[key] = stored with { Barrier = resulting };
            long position = checked(++_working.SubjectRetirementPosition);
            AddRetirementBarrierPublication(scope, BaseSubjectRetirementPublicationKind.BarrierOverridden, resulting, stored.Barrier.Generation, null, position);
            return ValueTask.FromResult(OperationResults.Ok(new BaseSubjectRetirementOverrideResult { Outcome = BaseSubjectRetirementMutationOutcome.Applied, Generation = resulting.Generation, BarrierChecksum = resulting.BarrierChecksum }));
        });

        public ValueTask<OperationResult<BaseSubjectRetirementPurgeApplied>> ApplySubjectRetirementPurgeAsync(
            BaseSubjectRetirementProviderPurgeRequest request, CancellationToken cancellationToken = default) => ExecuteAsync(cancellationToken, async token =>
        {
            ArgumentNullException.ThrowIfNull(request);
            ...
            if (policy is null || policy.PolicyChecksum != request.RetirementPolicyChecksum || contract is null
                || contract.ContractChecksum != request.ContractChecksum || policy.PurgeRetention.MinimumTombstoneAge != request.MinimumTombstoneAge)
                return RetirementFailure<BaseSubjectRetirementPurgeApplied>(BaseSubjectRetirementErrorCodes.ProviderContractInvalid, OperationStatus.CapabilityUnavailable, ErrorCategory.Capability);
            BaseProtectedSubjectScope protectedScope = _owner._subjectScopes.Protect(request.Scope, _owner._subjectScopeProtectionKey);
            string barrierKey = RetirementKey(protectedScope, command.ContractId, command.ContractVersion, command.SubjectId, command.AuthorityEpoch, command.Incarnation);
            if (!_working.SubjectRetirementBarriers.TryGetValue(barrierKey, out InMemorySubjectRetirementBarrierState? stored)
                || stored.Barrier.TombstoneSequence != command.ExpectedTombstoneSequence || stored.Barrier.Generation != command.ExpectedBarrierGeneration
                || stored.Barrier.BarrierChecksum != command.ExpectedBarrierChecksum
                || stored.Barrier.State is not (BaseSubjectRetirementBarrierState.Satisfied or BaseSubjectRetirementBarrierState.Overridden)
                || stored.Barrier.RequiredConsumerSetChecksum != BaseSubjectRetirementRegistry.AcceptedSetChecksum(policy.AcceptedConsumers)
                || request.ObservedAtUtc < checked(stored.Barrier.CreatedAtUtc + request.MinimumTombstoneAge))
                return RetirementFailure<BaseSubjectRetirementPurgeApplied>("base.subjectRetirement.purgeConflict", OperationStatus.Conflict, ErrorCategory.Conflict);
            KeyValuePair<string, InMemorySubjectLifetimeState> located = _working.SubjectLifetimes.SingleOrDefault(pair =>
                pair.Value.ContractId == command.ContractId && pair.Value.ContractVersion == command.ContractVersion
                && pair.Value.SubjectId.Equals(command.SubjectId) && pair.Value.Incarnation.Equals(command.Incarnation)
                && pair.Value.LifecycleState == BaseSubjectLifecycleState.Tombstoned
                && ProtectedScopeEquals(_owner._subjectScopes.Protect(pair.Value.Scope, _owner._subjectScopeProtectionKey), protectedScope));
            if (located.Value is null || !contract.AuthorityEpoch.Equals(command.AuthorityEpoch))
                return RetirementFailure<BaseSubjectRetirementPurgeApplied>("base.subjectRetirement.purgeConflict", OperationStatus.Conflict, ErrorCategory.Conflict);
            InMemorySubjectLifetimeState lifetime = located.Value;
            CollectionDefinition? collection = (_owner._options.Collections ?? []).SingleOrDefault(value => value.Id == lifetime.PrivateCollectionId);
            RecordEnvelope? before = collection is null ? null : SnapshotRecord(collection, lifetime.PrivateRecordId);
            if (collection is null || before is null || before.Metadata.Revision != command.ExpectedPrivateRevision)
                return RetirementFailure<BaseSubjectRetirementPurgeApplied>("base.subjectRetirement.purgeConflict", OperationStatus.Conflict, ErrorCategory.Conflict);
            OperationResult<DeleteResult> deleted = await _owner.DeleteCoreAsync(_working, collection, lifetime.PrivateRecordId,
                new() { ExpectedRevision = command.ExpectedPrivateRevision, ReturnPrevious = true }, request.Operation, token,
                () => _relationChecks = checked(_relationChecks + 1)).ConfigureAwait(false);
            if (!deleted.IsSuccess() || deleted.Value is null) return new() { Status = deleted.Status, Error = deleted.Error };
            long journalPosition = checked(++_working.GlobalMutationPosition);
            long retiredSequence = checked(lifetime.SubjectSequence + 1);
            var lifecycle = new BaseSubjectLifecycleCommitEvidence { ContractId = command.ContractId, ContractVersion = command.ContractVersion, SubjectId = command.SubjectId.Value, Kind = BaseSubjectLifecycleMutationKind.Retire, AuthorityEpoch = command.AuthorityEpoch, Incarnation = command.Incarnation, SubjectSequence = retiredSequence, ContractStateGeneration = contract.StateGeneration, DeliveryEpoch = _working.SubjectLifecycleDeliveryEpoch, Scope = lifetime.Scope, PreviousState = BaseSubjectLifecycleState.Tombstoned, ResultingState = BaseSubjectLifecycleState.Retired, CommitPosition = new(journalPosition) };
            var fact = new BaseRecordMutationFact { ItemId = "subject-retirement-purge", RequestedOperation = BaseRecordMutationKind.Delete, CommittedOperation = BaseCommittedRecordMutationKind.Delete, Collection = collection, Event = EventReference(Guid.NewGuid().ToString("N"), BaseCommittedRecordMutationKind.Delete) with { PublishedAt = Now(request.Operation), Stream = "base.mutations", Guarantee = EventDeliveryGuarantee.Transactional }, JournalPosition = new(journalPosition), Before = before, After = null, Delete = deleted.Value, ChangedFields = null, SubjectLifecycle = lifecycle };
            _working.MutationJournal.Add(journalPosition, CreateJournalEntry(fact, request.Operation.TenantId));
            _working.SubjectLifetimes.Remove(located.Key);
            string l47Checksum = BaseSubjectTerminalIntegrity.Compute(command.ContractId, command.ContractVersion, command.SubjectId, lifetime.Scope, command.AuthorityEpoch, command.Incarnation, lifetime.LifetimeGeneration, retiredSequence, new(journalPosition), contract.StateGeneration, contract.RestoreEpoch);
            _working.SubjectTerminals[_owner.SubjectKey(lifetime.Scope, command.ContractId, command.ContractVersion, command.SubjectId)] = new(command.ContractId, command.ContractVersion, command.SubjectId, lifetime.Scope, command.AuthorityEpoch, command.Incarnation, lifetime.LifetimeGeneration, retiredSequence, journalPosition, contract.StateGeneration, contract.RestoreEpoch, l47Checksum);
            var boundary = new BaseSubjectLifecycleOrderingBoundary { CommitPosition = new(journalPosition), SubjectId = command.SubjectId, AuthorityEpoch = command.AuthorityEpoch, Incarnation = command.Incarnation, SubjectSequence = retiredSequence };
            var lifecycleFact = new BaseSubjectLifecycleFact { CommitPosition = new(journalPosition), ContractId = command.ContractId, ContractVersion = command.ContractVersion, SubjectId = command.SubjectId, AuthorityEpoch = command.AuthorityEpoch, Incarnation = command.Incarnation, SubjectSequence = retiredSequence, ContractStateGeneration = contract.StateGeneration, DeliveryEpoch = _working.SubjectLifecycleDeliveryEpoch, Kind = BaseSubjectLifecycleFactKind.Retired, Retired = new() { PreviousState = BaseSubjectLifecycleState.Tombstoned } };
            int factIndex = _working.SubjectLifecycleFacts.Count;
            _working.SubjectLifecycleFacts.Add(new(protectedScope, boundary, lifecycleFact));
            foreach (BaseSubjectLifecycleConsumerDefinition consumer in _owner._options.SubjectLifecycleConsumers.Where(value => value.ContractId == command.ContractId && value.ContractVersion == command.ContractVersion && value.ObservedStates.Contains(BaseSubjectLifecycleState.Retired)))
            {
                InMemorySubjectLifecycleConsumerProjection? projection = _working.SubjectLifecycleConsumers.GetValueOrDefault($"{consumer.Id}\n{consumer.Version}");
                if (projection is null) continue;
                int membershipIndex = _working.SubjectLifecycleMemberships.Count;
                _working.SubjectLifecycleMemberships.Add(new(consumer.Id, consumer.Version, projection.ConsumerChecksum, projection.ProjectionGeneration, BaseSubjectLifecycleState.Retired, protectedScope, factIndex));
                string membershipKey = ProtectedScopeKey(consumer.Id, consumer.Version, protectedScope);
                if (!_working.SubjectLifecycleMembershipIndex.TryGetValue(membershipKey, out List<int>? indexes)) _working.SubjectLifecycleMembershipIndex.Add(membershipKey, indexes = []);
                indexes.Add(membershipIndex);
            }
            ImmutableArray<BaseSubjectTerminalAcknowledgement> acknowledgements = [.. stored.Acknowledgements.Values.OrderBy(static value => value.ConsumerId, StringComparer.Ordinal).ThenBy(static value => value.ConsumerVersion).Select(static value => new BaseSubjectTerminalAcknowledgement { ConsumerId = value.ConsumerId, ConsumerVersion = value.ConsumerVersion, ConsumerChecksum = value.ConsumerChecksum, ThroughSubjectSequence = value.ThroughSequence, Disposition = value.Disposition, AcknowledgedPosition = new(value.Position) })];
            var terminal = new BaseSubjectRetirementTerminalReceipt { ContractId = command.ContractId, ContractVersion = command.ContractVersion, SubjectId = command.SubjectId, Scope = protectedScope, AuthorityEpoch = command.AuthorityEpoch, Incarnation = command.Incarnation, TombstoneSequence = stored.Barrier.TombstoneSequence, AuthorizingState = stored.Barrier.State, FinalBarrierGeneration = stored.Barrier.Generation, FinalBarrierChecksum = stored.Barrier.BarrierChecksum, RequiredConsumerSetChecksum = stored.Barrier.RequiredConsumerSetChecksum, Acknowledgements = acknowledgements, RetiredPosition = new(journalPosition), PurgedAtUtc = request.ObservedAtUtc, ReceiptChecksum = string.Empty };
            terminal = terminal with { ReceiptChecksum = BaseSubjectRetirementRegistry.TerminalChecksum(terminal) };
            _working.SubjectRetirementBarriers.Remove(barrierKey);
            _working.SubjectRetirementTerminals[barrierKey] = new(terminal);
            long publicationPosition = checked(++_working.SubjectRetirementPosition);
            AddRetirementPublication(new() { Scope = protectedScope, Fact = new() { Position = new(publicationPosition), Kind = BaseSubjectRetirementPublicationKind.SubjectPurged, Purged = new() { ContractId = command.ContractId, ContractVersion = command.ContractVersion, SubjectId = command.SubjectId, AuthorityEpoch = command.AuthorityEpoch, Incarnation = command.Incarnation, TombstoneSequence = stored.Barrier.TombstoneSequence, FinalBarrierGeneration = stored.Barrier.Generation, FinalBarrierChecksum = stored.Barrier.BarrierChecksum, TerminalReceiptChecksum = terminal.ReceiptChecksum, RetiredLifecyclePosition = new(journalPosition) } } });
            return OperationResults.Ok(new BaseSubjectRetirementPurgeApplied { Result = new() { Outcome = BaseSubjectRetirementMutationOutcome.Applied, RetiredSubjectSequence = retiredSequence, RetiredPosition = new(journalPosition), TerminalReceiptChecksum = terminal.ReceiptChecksum }, Mutation = fact, Terminal = terminal });
        });
