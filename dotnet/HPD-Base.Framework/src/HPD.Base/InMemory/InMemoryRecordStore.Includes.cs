...
internal sealed partial class InMemoryRecordStore
{
    ...
    private static Dictionary<string, string[]> InMemoryRelatedIdsBatch(RecordEnvelope[] parents, RelationDefinition relation, bool inverse, InMemoryIncludeState state)
    {
        ...
        if (!inverse)
            return parents.ToDictionary(static parent => parent.Id.Value, parent => PayloadIds(parent.Payload, field.WireName), StringComparer.Ordinal);
        ...
        foreach (StoredRecord record in records.OrderBy(static record => record.AppendPosition).ThenBy(static record => record.Id.Value, StringComparer.Ordinal))
            foreach (string targetId in PayloadIds(record.Payload, field.WireName))
    ...
    private static string LowerInMemoryField(CollectionDefinition collection, string id) => id is "id" or "createdAt" or "updatedAt" or "revision" ? id : (collection.Fields ?? []).Single(field => field.Id == id || field.WireName == id).WireName;
