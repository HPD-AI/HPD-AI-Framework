...
public sealed class BaseSchemaGenerator : IIncrementalGenerator
{
    ...
    private const string ActivationDtoAuthorityAttribute = "HPD.Base.BaseActivationDtoAuthorityAttribute";
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        IncrementalValuesProvider<INamedTypeSymbol> moduleMutations =
            context.SyntaxProvider.ForAttributeWithMetadataName(
                ...
                static (node, _) => node is TypeDeclarationSyntax,
                static (attributeContext, _) => (INamedTypeSymbol)attributeContext.TargetSymbol);
        IncrementalValuesProvider<INamedTypeSymbol> activations =
            context.SyntaxProvider.ForAttributeWithMetadataName(
                ActivationDtoAuthorityAttribute,
                static (node, _) => node is TypeDeclarationSyntax,
                static (attributeContext, _) => (INamedTypeSymbol)attributeContext.TargetSymbol);
        IncrementalValuesProvider<INamedTypeSymbol> closedEnums = context.SyntaxProvider.CreateSyntaxProvider(
            ...
            .Where(static symbol => symbol is not null && UsesBaseClosedEnumConverter(symbol))!;

        context.RegisterSourceOutput(collections.Collect().Combine(reads.Collect()).Combine(subjects.Collect()).Combine(moduleMutations.Collect()).Combine(activations.Collect()).Combine(context.CompilationProvider),
            static (productionContext, input) => Generate(
                productionContext, input.Left.Left.Left.Left.Left, input.Left.Left.Left.Left.Right,
                input.Left.Left.Left.Right, input.Left.Left.Right, input.Left.Right, input.Right));
        context.RegisterSourceOutput(closedEnums.Collect(), static (productionContext, values) =>
            GenerateClosedEnumAuthorities(productionContext, values));
        BaseCollectionGenerator.RegisterForbiddenReferences(context);
    }

    private static bool UsesBaseClosedEnumConverter(INamedTypeSymbol symbol) => symbol.GetAttributes().Any(attribute =>
        ...
        && SymbolEqualityComparer.Default.Equals(converter.TypeArguments[0], symbol));

    private static void GenerateClosedEnumAuthorities(
        SourceProductionContext context,
        ImmutableArray<INamedTypeSymbol> values)
    {
        INamedTypeSymbol[] enums = values
            .GroupBy(static value => value.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat), StringComparer.Ordinal)
            .Select(static group => group.First())
            .OrderBy(static value => value.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat), StringComparer.Ordinal)
            .ToArray();
        if (enums.Length == 0) return;
        var source = new StringBuilder("// <auto-generated />\n#nullable enable\nnamespace HPD.Base.Generated;\ninternal static class HPDBaseClosedEnumAuthorities\n{\n");
        for (int index = 0; index < enums.Length; index++)
        {
            INamedTypeSymbol enumType = enums[index];
            string type = enumType.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat);
            (string Member, string Wire)[] members = enumType.GetMembers().OfType<IFieldSymbol>()
                .Where(static field => field.HasConstantValue)
                .Select(field => (field.Name, field.GetAttributes().FirstOrDefault(attribute =>
                    attribute.AttributeClass?.ToDisplayString() == "System.Text.Json.Serialization.JsonStringEnumMemberNameAttribute")
                    ?.ConstructorArguments[0].Value as string ?? field.Name))
                .ToArray();
            source.AppendLine("    [global::System.Runtime.CompilerServices.ModuleInitializer]");
            source.Append("    internal static void Register_").Append(index.ToString(CultureInfo.InvariantCulture))
                .Append("() => global::HPD.Base.BaseClosedEnumGeneratedContract.Register<").Append(type).Append(">(")
                .Append("new ").Append(type).Append("[] { ")
                .Append(string.Join(", ", members.Select(member => type + "." + member.Member)))
                .Append(" }, new string[] { ")
                .Append(string.Join(", ", members.Select(member => Microsoft.CodeAnalysis.CSharp.SymbolDisplay.FormatLiteral(member.Wire, true))))
                .AppendLine(" });");
        }
        source.AppendLine("}");
        context.AddSource("HPDBaseClosedEnumAuthorities.g.cs", source.ToString());
    }
    ...
    private static void Generate(
        ...
        ImmutableArray<INamedTypeSymbol> activations,
        Compilation compilation)
    {
        ...
        AddRoots(rootsByContext, activations, ActivationDtoAuthorityAttribute, 5, 6, 7);
        ...
        BaseModuleMutationGenerator.GenerateActivationCombined(context, activations, contextResults);
    }
        ...
        int resultIndex)
    {
        foreach (INamedTypeSymbol declaration in declarations.Distinct(SymbolEqualityComparer.Default))
        {
            AttributeData attribute = declaration.GetAttributes().FirstOrDefault(value => value.AttributeClass?.ToDisplayString() == attributeName);
            if (attribute is null || attribute.ConstructorArguments.Length <= resultIndex
                || attribute.ConstructorArguments[contextIndex].Value is not INamedTypeSymbol context
                || attribute.ConstructorArguments[inputIndex].Value is not INamedTypeSymbol input
                || attribute.ConstructorArguments[resultIndex].Value is not INamedTypeSymbol result)
                continue;
            if (!rootsByContext.TryGetValue(context, out HashSet<INamedTypeSymbol> roots))
                rootsByContext.Add(context, roots = new HashSet<INamedTypeSymbol>(SymbolEqualityComparer.Default));
            roots.Add(input);
            roots.Add(result);
        }
    }
        ...
        bool omitOptionalNonNullableNulls)
    {
        ...
        bool Visit(ITypeSymbol input, int depth, int wrappers, ISymbol origin)
        {
            ...
            if (type is INamedTypeSymbol convertedType && Find(convertedType, JsonConverterAttribute) is not null &&
                !SerializerScalar(convertedType) && !BaseOwnedScalarConverter(convertedType))
                return rejectedTypes.Add(type)
    ...
    private static bool BaseOwnedScalarConverter(INamedTypeSymbol type) =>
        ...
        type.Name is "BaseCanonicalJson" or "BaseModuleGeneration" or "RevisionToken" or
            "BaseSubjectIncarnation" or "BaseSubjectReference";
