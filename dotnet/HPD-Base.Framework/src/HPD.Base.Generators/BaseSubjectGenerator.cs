...
internal static class BaseSubjectGenerator
{
    ...
    internal static void Generate(SourceProductionContext context, ImmutableArray<INamedTypeSymbol> subjects)
    {
        ...
        foreach (INamedTypeSymbol subject in subjects.OrderBy(static value => value.ToDisplayString(), StringComparer.Ordinal))
        {
            ...
            string? tombstoneInstantField = NamedString(attribute, "TombstoneInstantFieldId");
            string? tombstoneSequenceField = NamedString(attribute, "TombstoneSequenceFieldId");
            int finalRetirementExecutionMode = NamedInt(attribute, "FinalRetirementExecutionMode", 0);
            ...
            string? error = !IsPartial(subject) || subject.TypeKind != TypeKind.Class || subject.IsGenericType ? "the marker must be a partial non-generic class" :
                ...
                activeField is null || !ValidId(activeField) || !ValidId(tombstoneField) || scopeField is not null && !ValidId(scopeField) ||
                tombstoneInstantField is not null && !ValidId(tombstoneInstantField) || tombstoneSequenceField is not null && !ValidId(tombstoneSequenceField) ? "a bound stable lifecycle or scope field ID is invalid" :
                finalRetirementExecutionMode is < 0 or > 1 ? "the final-retirement execution mode is invalid" :
                tombstoneInstantField is not null && (tombstoneInstantField == tombstoneField || tombstoneInstantField == activeField || tombstoneInstantField == scopeField) ||
                tombstoneSequenceField is not null && (tombstoneSequenceField == tombstoneField || tombstoneSequenceField == activeField || tombstoneSequenceField == scopeField || tombstoneSequenceField == tombstoneInstantField) ? "the tombstone metadata fields must be distinct from lifecycle and scope fields" :
            ...
            context.AddSource(subject.Name + ".HPDBaseSubject.g.cs", SourceText.From(Render(subject, id!, version, module!, collectionId!,
                ...
                tombstoneField!, tombstoneInstantField, tombstoneSequenceField, finalRetirementExecutionMode, coordinatedRetirement, scopeField, audiences), Encoding.UTF8));
        }
    }
    ...
    private static string Render(INamedTypeSymbol subject, string id, int version, string module, string collectionId,
        ...
        int scope, string? activeField, bool activeValue, string tombstoneField, string? tombstoneInstantField,
        string? tombstoneSequenceField, int finalRetirementExecutionMode, bool coordinatedRetirement, string? scopeField, int[] audiences)
    {
        ...
        source.AppendLine("        TombstoneMetadata = new global::HPD.Base.BaseSubjectTombstoneMetadataDefinition");
        source.AppendLine("        {");
        source.Append("            Instant = new global::HPD.Base.BaseSubjectTombstoneInstantBinding { Kind = global::HPD.Base.BaseSubjectTombstoneMetadataBindingKind.")
            .Append(tombstoneInstantField is null ? "NotStored" : "RequiredField").Append(", FieldId = ").Append(tombstoneInstantField is null ? "null" : Literal(tombstoneInstantField)).AppendLine(" },");
        source.Append("            Sequence = new global::HPD.Base.BaseSubjectTombstoneSequenceBinding { Kind = global::HPD.Base.BaseSubjectTombstoneMetadataBindingKind.")
            .Append(tombstoneSequenceField is null ? "NotStored" : "RequiredField").Append(", FieldId = ").Append(tombstoneSequenceField is null ? "null" : Literal(tombstoneSequenceField)).AppendLine(" },");
        source.AppendLine("        },");
        source.Append("        FinalRetirementExecutionMode = (global::HPD.Base.BaseSubjectFinalExecutionMode)").Append(finalRetirementExecutionMode).AppendLine(",");
