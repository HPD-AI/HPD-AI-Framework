...
public static class HPDBaseEndpointRouteBuilderExtensions
{
        ...
        Action<IEndpointConventionBuilder, HPDBaseEndpointDescriptor> convention)
    {
        ...
        SemanticActivationAdministrationEndpoints.Map(group, convention);
        ...
        HPDBaseApplicationEndpointOptions options)
    {
        ...
        if (!options.MapRecords && !options.MapRegisteredReads && !options.MapFiles && !options.MapRealtime && !options.MapSubjectLifecycle && !options.MapActivations && !options.MapClientGeneration)
        ...
        if (options.MapActivations)
            ActivationWorkerEndpoints.Map(group);
