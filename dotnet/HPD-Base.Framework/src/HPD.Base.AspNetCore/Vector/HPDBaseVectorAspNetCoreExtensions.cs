...
public static class HPDBaseVectorAspNetCoreExtensions
{
    ...
    private static async Task Query(HttpContext context)
    {
        ...
        BaseVectorRuntimeResult value = result.Value; bool includeMeasures = body.MeasureDisclosure == BaseVectorHttpMeasureDisclosure.Include; var response = new BaseVectorHttpQueryResponse { Matches = value.Matches.Select(item => new BaseVectorHttpMatch { Record = item.Record, Rank = item.Rank, Measure = includeMeasures ? item.Measure : null }).ToArray(), VectorIndexId = value.VectorIndexId, VectorIndexGeneration = value.VectorIndexGeneration, ProviderId = value.ProviderId, Accuracy = value.Accuracy, ConsistencyToken = value.ConsistencyToken.Encode() };
