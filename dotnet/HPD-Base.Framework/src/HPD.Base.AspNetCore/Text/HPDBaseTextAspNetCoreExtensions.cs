...
public static class HPDBaseTextAspNetCoreExtensions
{
    ...
    private static async Task ListIndexes(HttpContext context)
    {
        ...
        if (!result.Status.IsSuccess() || result.Value is null) { await Error(context, 424, result.Error?.Code ?? BaseTextErrorCodes.IndexUnavailable); return; }
    ...
    {
        (string collectionId, string indexId) = RouteIds(context); OperationResult<BaseTextIndexStatus> result = await context.RequestServices.GetRequiredService<IBaseTextAdministration>().GetAsync(collectionId, indexId, context.RequestAborted).ConfigureAwait(false);
        if (!result.Status.IsSuccess() || result.Value is null) { await Error(context, result.Status == OperationStatus.NotFound ? 404 : 424, result.Error?.Code ?? BaseTextErrorCodes.IndexUnavailable); return; }
    ...
    {
        BaseTextHttpRebuildRequest? body; try { body = await JsonSerializer.DeserializeAsync(context.Request.Body, BaseTextHttpJsonContext.Default.BaseTextHttpRebuildRequest, context.RequestAborted).ConfigureAwait(false); } catch (JsonException) { await Error(context, 400, BaseTextErrorCodes.QueryInvalid); return; }
        ...
        if (!registry.Collections.TryGetValue(collectionId, out CollectionDefinition? collection) || (collection.TextIndexes ?? []).All(value => value.Id != indexId)) { await Error(context, 404, BaseTextErrorCodes.IndexNotFound); return; }
        ...
        if (!authorized.Status.IsSuccess()) { await Error(context, 403, BaseTextErrorCodes.Unauthorized); return; }
        ...
        if (!result.Status.IsSuccess() || result.Value is null) { await Error(context, result.Status == OperationStatus.Conflict ? 409 : result.Status == OperationStatus.NotFound ? 404 : 500, result.Error?.Code ?? BaseTextErrorCodes.CommitIndeterminate); return; }
    ...
    private static async Task Query(HttpContext context)
    {
        ...
        if (body is null || body.IndexId != indexId || !registry.Collections.TryGetValue(collectionId, out CollectionDefinition? collection) || (collection.TextIndexes ?? []).SingleOrDefault(value => value.Id == indexId) is not { } index) { await Error(context, 404, BaseTextErrorCodes.IndexNotFound); return; }
        try
        {
    ...
    private static int Status(OperationResult<BaseTextRuntimeResult> value) => value.Error?.Code switch
    {
        ...
        _ => value.Status switch { OperationStatus.PolicyDenied => 403, OperationStatus.NotFound => 404, OperationStatus.Conflict => 409, OperationStatus.Unsupported => 422, OperationStatus.CapabilityUnavailable => 424, OperationStatus.StoreError => 503, _ => 400 },
    };
