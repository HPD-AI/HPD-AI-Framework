...
public static class HPDBaseTextAspNetCoreExtensions
{
    ...
    private static async Task Query(HttpContext context)
    {
        ...
        try
        {
            ...
            ImmutableArray<BaseTextOrder> order = body.Order.Select(value => Order(value, index)).ToImmutableArray();
            OperationResult<BaseTextRuntimeResult> result = await context.RequestServices.GetRequiredService<IBaseTextRuntime>().ExecuteAsync(new() { Collection = collection, Index = index, Query = query, Constraint = filter, Order = order, Take = body.Take, After = cursor, Consistency = consistency, Principal = principal, Operation = operation }, context.RequestAborted).ConfigureAwait(false);
    ...
    private static BaseTextOrder Order(BaseTextHttpOrder value, BaseTextIndexDefinition index)
    { BaseTextIndexFilterFieldDefinition field = index.FilterFields.Single(item => item.StableFieldId == value.Field || item.WireName == value.Field || item.ApplicationName == value.Field); return new(field.StableFieldId, value.Direction == "desc" ? QuerySortDirection.Desc : value.Direction == "asc" ? QuerySortDirection.Asc : throw new ArgumentException(), value.NullOrder == "last" ? QueryNullOrder.Last : value.NullOrder == "first" ? QueryNullOrder.First : value.NullOrder == "unspecified" ? QueryNullOrder.Unspecified : throw new ArgumentException()); }
