...
public static class HPDBaseVectorAspNetCoreExtensions
{
    ...
    private static async Task Query(HttpContext context)
    {
        ...
        if (body?.Vector is null || !BaseVector.TryCreate(body.Vector, out BaseVector vector)) { await Error(context, 400, "base.vector.invalid", "The vector request body is invalid."); return; }
        ...
        BaseVectorRuntimeResult value = result.Value; var response = new BaseVectorHttpQueryResponse { Matches = value.Matches.Select(static item => new BaseVectorHttpMatch { Record = item.Record, Rank = item.Rank, Measure = item.Measure }).ToArray(), VectorIndexId = value.VectorIndexId, VectorIndexGeneration = value.VectorIndexGeneration, ProviderId = value.ProviderId, Accuracy = value.Accuracy, ConsistencyToken = value.ConsistencyToken.Encode() };
