    ...
    where TContext : class, IGraphContext
{
    ...
    private static bool IsControlNode(Abstractions.Graph.Node? node) =>
        node?.Type is Abstractions.Graph.NodeType.Start or Abstractions.Graph.NodeType.End;
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            foreach (var nodeId in GetExecutableNodeIds(context.Graph))
            {
                context.MarkNodeComplete(nodeId);
            }
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            nextDirtyNodes = ComputeNextDirtyNodes(context, triggeredNodes, backEdges);
    ...
    /// <summary>
    /// Compute next dirty set by following forward dependencies from triggered back-edge targets.
    /// A back-edge target re-execution can change any downstream input, so the affected
    /// forward subgraph must be re-run as a unit.
    /// </summary>
        ...
        IReadOnlyList<BackEdge> backEdges)
    {
        ...
        var dirtyNodes = GetAllForwardDependents(triggeredNodes, context.Graph, backEdges);
        foreach (var nodeId in alwaysDirty)
        {
            dirtyNodes.UnionWith(GetAllForwardDependents(new HashSet<string> { nodeId }, context.Graph, backEdges));
        }
        ...
        if (_checkpointStore != null && context is Context.GraphContext graphContext)
        {
            ...
            if (checkpoint != null)
            {
                ...
                foreach (var kvp in checkpoint.NodeOutputs)
                {
                    ...
                    var outputNode = context.Graph.GetNode(nodeId);
                    if (!IsControlNode(outputNode) && !checkpoint.NodeStateMetadata.ContainsKey(nodeId))
                    {
                        throw new InvalidOperationException(
    ...
    private HandlerInputs PrepareInputs(TContext context, Node node)
    {
        ...
        foreach (var sourceGroup in edgesBySource)
        {
            ...
            if (sourceNode is { Type: NodeType.Start })
            {
                AddWorkflowInputs(inputs, context);
                continue;
            }
    ...
    private static void AddWorkflowInputs(HandlerInputs inputs, TContext context)
    {
        foreach (var channelName in context.Channels.ChannelNames)
        {
            if (!channelName.StartsWith("input:", StringComparison.Ordinal))
            {
                continue;
            }

            var inputName = channelName["input:".Length..];
            try
            {
                var value = context.Channels[channelName].Get<object>();
            ...
            {
                // Skip uninitialized or incompatible input channels.
            }
        }
    }
        ...
        CancellationToken ct)
    {
        ...
        try
        {
            ...
            var currentPollingState = new HPD.Graph.Abstractions.Serialization.PollingState
            {
                StartTime = startTime,
                AttemptNumber = currentAttempt,
                SuspendToken = suspended.SuspendToken,
    ...
    private static Dictionary<string, object> ToPartitionInputValue(int index, PartitionKey partitionKey) => new()
    {
        ["index"] = index,
        ["key"] = partitionKey.ToString(),
        ["dimensions"] = partitionKey.Dimensions.ToArray()
    };
                    ...
                    async (index, ct) =>
                    {
                        ...
                        try
                        {
                            ...
                            if (mapContext is Context.GraphContext baseContext)
                            {
                                ...
                                // Write item to input channel. Partition map items carry runtime-only
                                // context; expose an AOT-safe value for hashing and handler inputs.
                                var inputItem = item is IPartitionMapItem partitionInput
                                    ? ToPartitionInputValue(index, partitionInput.PartitionKey)
                                    : item;
                                mapContextWithGraph.Channels["input:item"].Set(inputItem);
