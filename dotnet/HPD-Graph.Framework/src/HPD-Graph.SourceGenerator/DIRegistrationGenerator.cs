...
public class DIRegistrationGenerator : IIncrementalGenerator
{
    ...
    private string GenerateHandlerRegistrationCode(string contextType, List<INamedTypeSymbol> handlers)
    {
        ...
        sb.AppendLine("namespace HPD.Graph.Extensions;");
        sb.AppendLine();
        ...
        sb.AppendLine("    {");
        sb.AppendLine("        services.TryAddSingleton<HPD.Graph.Abstractions.Discovery.GeneratedHandlerCatalog>();");
        sb.AppendLine("        services.TryAddSingleton<HPD.Graph.Abstractions.Discovery.IGeneratedHandlerCatalog>(sp => sp.GetRequiredService<HPD.Graph.Abstractions.Discovery.GeneratedHandlerCatalog>());");
        ...
        foreach (var handler in handlers)
        {
            ...
            sb.AppendLine($"        services.AddScoped<HPD.Graph.Abstractions.Handlers.IGraphNodeHandler<{contextType}>>(sp => sp.GetRequiredService<{handlerFullName}>());");
            sb.AppendLine($"        services.AddScoped<HPD.Graph.Abstractions.Invocation.IGraphNodeHandlerInvoker, {invokerTypeName}>();");
        }
    ...
    private string GenerateHandlerCatalogCode(List<INamedTypeSymbol> handlers)
    {
        ...
        sb.AppendLine("#nullable enable");
        sb.AppendLine();
        sb.AppendLine("namespace HPD.Graph.Abstractions.Discovery;");
        sb.AppendLine();
    ...
    private static void AppendInvokerCode(StringBuilder sb, string contextType, INamedTypeSymbol handler)
    {
        ...
        sb.AppendLine();
        sb.AppendLine($"internal sealed class {invokerTypeName} : HPD.Graph.Abstractions.Invocation.IGraphNodeHandlerInvoker");
        sb.AppendLine("{");
        ...
        sb.AppendLine("    }");
        sb.AppendLine();
        sb.AppendLine($"    public string HandlerName => ((HPD.Graph.Abstractions.Handlers.IGraphNodeHandler<{contextType}>)_handler).HandlerName;");
        ...
        sb.AppendLine();
        sb.AppendLine("    public async System.Threading.Tasks.ValueTask<HPD.Graph.Abstractions.Execution.NodeExecutionResult> ExecuteAsync(");
        sb.AppendLine("        HPD.Graph.Abstractions.Context.IGraphContext context,");
        ...
        sb.AppendLine("        }");
        sb.AppendLine();
        sb.AppendLine($"        return await ((HPD.Graph.Abstractions.Handlers.IGraphNodeHandler<{contextType}>)_handler)");
    ...
    private string GenerateRouterRegistrationCode(List<INamedTypeSymbol> routers)
    {
        ...
        foreach (var router in routers)
        {
            ...
            sb.AppendLine($"        services.AddSingleton<HPD.Graph.Abstractions.Routing.IMapRouter, {routerFullName}>();");
        }
