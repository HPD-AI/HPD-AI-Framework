...
public sealed class PjrtDoubleTensor : Helium.Hardware.IHardwareTensor<double>
{
    private PjrtBuffer _buffer;
    private bool _disposed;

    internal PjrtDoubleTensor(PjrtDoubleBackend owner, PjrtBuffer buffer, int rows, int cols, PjrtElementType elementType)
    {
        Owner = owner;
        _buffer = buffer;
        Rows = rows;
    ...
    internal PjrtDoubleBackend Owner { get; }
    ...
    public void CopyToHost(Span<double> destination)
    {
        ThrowIfDisposed();
    ...
    public void UpdateFromSpan(ReadOnlySpan<double> hostData)
    {
        ThrowIfDisposed();
    ...
    {
        if (_disposed)
            throw new ObjectDisposedException(nameof(PjrtDoubleTensor));
