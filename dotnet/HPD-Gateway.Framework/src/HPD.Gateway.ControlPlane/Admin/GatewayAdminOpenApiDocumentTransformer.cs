...
internal sealed class GatewayAdminOpenApiSchemaTransformer(GatewayAdminOpenApiContract contract) : IOpenApiSchemaTransformer
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (schemaType.IsEnum)
        {
            schema.Type = Nullable.GetUnderlyingType(context.JsonTypeInfo.Type) is null
                ? JsonSchemaType.String
                : JsonSchemaType.String | JsonSchemaType.Null;
            bool camelCase = schemaType.GetCustomAttribute<JsonConverterAttribute>()?.ConverterType is { IsGenericType: true } converter &&
                converter.GetGenericTypeDefinition() == typeof(HPD.Gateway.StrictStringEnumJsonConverter<>);
            schema.Enum = Enum.GetNames(schemaType)
                .Select(value => JsonValue.Create(camelCase ? JsonNamingPolicy.CamelCase.ConvertName(value) : value))
                .ToList<JsonNode>();
        }
        ...
        if (context.JsonPropertyInfo?.PropertyType == typeof(ImmutableArray<ServiceDiscoveryScheme>))
        {
            schema.Items = new OpenApiSchema
            {
                Type = JsonSchemaType.String,
                Enum = Enum.GetNames<ServiceDiscoveryScheme>()
                    .Select(value => JsonValue.Create(JsonNamingPolicy.CamelCase.ConvertName(value)))
                    .ToList<JsonNode>(),
            };
        }
        ...
        if (GatewayAdminClientNullableSchemaLedger.Contains(declaringType, property))
    ...
    {
        schema.Type = schema.Type?.HasFlag(JsonSchemaType.Null) == true
            ? JsonSchemaType.String | JsonSchemaType.Null
            : JsonSchemaType.String;
...
internal static class GatewayAdminClientNullableSchemaLedger
{
    private static readonly HashSet<(Type Type, string Property)> V1 =
    [
    ...
    : IOpenApiDocumentTransformer
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        // The operation union branches own property-level semantic constraints.
        // Observe them before the base union is componentized so correlation is
        // generation-scoped even when ASP.NET subsequently reuses the branches.
        _ = await context.GetOrCreateSchemaAsync(
            typeof(GatewayCommandOperationProjection), null, cancellationToken).ConfigureAwait(false);
        _ = await context.GetOrCreateSchemaAsync(
            typeof(GatewayAdministrativeOperationProjection), null, cancellationToken).ConfigureAwait(false);
