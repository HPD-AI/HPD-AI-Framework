...
internal static partial class GatewayAdminEndpointMapper
{
    private static void MapAdditional(
        RouteGroupBuilder group, GatewayAdminApiOptions options,
    ...
    private static async Task Activations(HttpContext context)
    {
        ...
        if (!TryActivationPage(context, GatewayAdminClientSemanticLedger.For("activations"),
            out int maximum, out string? intentCursor, out string? outcomeCursor, out IResult? failure))
        { await Write(context, failure!); return; }
        IGatewayManagementReader reader = context.RequestServices.GetRequiredService<IGatewayManagementReader>();
        var intents = await reader.ListActivationsAsync(ns, target, maximum, intentCursor, context.RequestAborted).ConfigureAwait(false);
        var outcomes = await reader.ListOutcomesAsync(ns, target, maximum, outcomeCursor, context.RequestAborted).ConfigureAwait(false);
        ...
        var projectedOutcomes = new GatewayAdminPage<GatewayOutcomeProjection>(outcomes.Items.Select(static item =>
            new GatewayOutcomeProjection(item.Id, item.Value.ActivationIntentId,
                checked((ulong)item.Value.AuthorityVersion), item.Value.Kind, item.Value.Code,
                item.Value.ApplicationId,
                item.Value.SymbolicPlanAlgorithm is not null && item.Value.SymbolicPlanValue is not null
                    ? new HPD.Gateway.ContentHash(item.Value.SymbolicPlanAlgorithm, item.Value.SymbolicPlanValue)
                    : null,
                item.CreatedAt)).ToImmutableArray(),
    ...
    private static async Task Operation(HttpContext context)
    {
        ...
        IGatewayManagementReader reader = context.RequestServices.GetRequiredService<IGatewayManagementReader>();
        GatewayManagedRecord<GatewayCommandReceipt>? command = await reader
            .GetOperationAsync(ns, operation, context.RequestAborted).ConfigureAwait(false);
        GatewayAdministrativeOperationReadProjection? administration = await reader
            .GetAdministrativeOperationAsync(ns, operation, context.RequestAborted).ConfigureAwait(false);
        if (command is not null && administration is not null)
            throw new InvalidOperationException("The operation identity resolves to multiple authority graphs.");
        GatewayOperationProjection? projection = command is not null
            ? new GatewayCommandOperationProjection
            {
                OperationId = command.Id,
                Operation = command.Value.Operation,
                ResultCode = command.Value.StableResultCode,
                DesiredStateToken = command.Value.StableDesiredStateToken,
                AcceptedAt = command.CreatedAt,
            }
            : administration is not null
                ? new GatewayAdministrativeOperationProjection
                {
                    OperationId = administration.OperationId,
                    Operation = administration.Operation,
                    State = administration.State,
                    Code = administration.Code,
                    ArtifactReference = administration.ArtifactReference,
                    ObservedAt = administration.ObservedAt,
                }
                : null;
        await Write(context, projection is null ? NotFound(context) : TypedResults.Json(
            projection, GatewayAdminJsonContext.Default.GatewayOperationProjection)).ConfigureAwait(false);
    }
    ...
    private static async Task Backup(HttpContext context)
    {
        ...
        GatewayAdministrativeResult result = await context.RequestServices.GetRequiredService<IGatewayManagementAdministration>()
            .CreateBackupAsync(ns, key, attribution.ToActor(), request.SinkName, request.ArtifactLabel,
                context.RequestAborted).ConfigureAwait(false);
        await Write(context, ProjectAdministration(result, result.ArtifactReference)).ConfigureAwait(false);
    }
    ...
    private static async Task Effective(HttpContext context)
    {
        string ns = Route(context, "ns"), target = Route(context, "target");
        if (!await AdmitTarget(context, ns, target).ConfigureAwait(false)) return;
        GatewayAppliedRuntimeObservation? observation = context.RequestServices.GetRequiredService<IGatewayNodeAppliedRuntimeReader>().GetCurrent();
        ...
        await Write(context, !matches ? NotFound(context) : TypedResults.Json(observation!.Snapshot,
            GatewayAdminJsonContext.Default.GatewayAppliedRuntimeSnapshot)).ConfigureAwait(false);
    }
    ...
    internal static bool TryActivationPage(
        HttpContext context, GatewayAdminClientOperationSemantics semantics,
        out int maximum, out string? intentCursor, out string? outcomeCursor, out IResult? failure)
    {
        GatewayAdminClientPaginationSpecification specification = semantics.Pagination;
        specification.Validate();
        maximum = specification.DefaultMaximum!.Value;
        intentCursor = outcomeCursor = null;
        failure = null;
        if (context.Request.Query.Keys.Any(static key => key is not ("maximum" or "intentCursor" or "outcomeCursor")) ||
            context.Request.Query["maximum"].Count > 1 || context.Request.Query["intentCursor"].Count > 1 ||
            context.Request.Query["outcomeCursor"].Count > 1)
        { failure = Invalid(context); return false; }
        if (context.Request.Query["maximum"].Count == 1 &&
            (!int.TryParse(context.Request.Query["maximum"][0], out maximum) ||
             maximum < specification.MinimumMaximum!.Value || maximum > specification.MaximumMaximum!.Value))
        { failure = Invalid(context); return false; }
        intentCursor = context.Request.Query["intentCursor"].Count == 1 ? context.Request.Query["intentCursor"][0] : null;
        outcomeCursor = context.Request.Query["outcomeCursor"].Count == 1 ? context.Request.Query["outcomeCursor"][0] : null;
        foreach ((string Name, string? Value) cursor in new[] { ("intentCursor", intentCursor), ("outcomeCursor", outcomeCursor) })
        {
            if (cursor.Value is null) continue;
            GatewayAdminClientParameterConstraint constraint = semantics.ParameterConstraints.Single(item =>
                item.Location == GatewayAdminClientParameterLocation.Query && item.Name == cursor.Name);
            if (Encoding.UTF8.GetByteCount(cursor.Value) > constraint.Rules.MaximumUtf8Bytes!.Value)
            { failure = Invalid(context); return false; }
        }
        return true;
    }
