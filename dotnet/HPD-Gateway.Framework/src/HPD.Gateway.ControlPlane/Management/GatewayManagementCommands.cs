...
public sealed record GatewayActivateRevisionCommand(
    ...
    string CorrelationId,
    string? ExpectedDesiredStateToken = null,
    GatewayRevisionActivationKind Kind = GatewayRevisionActivationKind.ActivateExisting);

public enum GatewayRevisionActivationKind : byte
{
    ActivateExisting = 0,
    Rollback = 1,
}
...
public sealed record GatewayManagementCommandResult(
    ...
    string? ActivationIntentId = null,
    ...
    GatewayManagementRuntimeOptions options) : IGatewayManagementCommandCoordinator
{
        ...
        CancellationToken cancellationToken)
    {
            ...
            return await CommitAsync(batch, ownershipId.Value, null, null, cancellationToken).ConfigureAwait(false);
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (await TargetEpochReservationCapacityExhaustedAsync(session, cancellationToken).ConfigureAwait(false))
    ...
    private static async ValueTask<bool> TargetEpochReservationCapacityExhaustedAsync(
        BaseSession session,
        CancellationToken cancellationToken)
    {
        ...
        do
        {
            int remaining = MaximumTargetEpochReservations - observed;
            BaseQuery<GatewayTargetEpochReservation> query = session
                .Collection(GatewayTargetEpochReservation.Collection)
                .Query()
                .Take(Math.Min(pageSize, remaining));
            if (continuation is not null)
                query = query.ContinueFrom(continuation);
            BasePage<BaseRecord<GatewayTargetEpochReservation>> page = (await query
                .PageAsync(cancellationToken)
                .ConfigureAwait(false)).RequireValue();
            observed += page.Items.Length;
            if (observed >= MaximumTargetEpochReservations)
                return true;
            continuation = page.Page.HasMore
                ? page.Page.NextCursor ?? throw new InvalidOperationException(
                    "Gateway epoch-reservation pagination omitted its continuation token.")
                : null;
        }
        ...
        try
        {
            ...
            if (priorReceiptResult.TryGetValue(out BaseRecord<GatewayCommandReceipt>? priorReceipt))
            {
                ...
                return new(
                    ...
                    priorReceipt.Value.StableRevisionId,
                    priorReceipt.Value.StableActivationIntentId);
            }
            ...
            if (!command.Activate)
            {
                batch.Create(GatewayCommandReceipt.Collection, receiptId, Receipt(
                    ...
                    "accepted", receiptId.Value, revisionId.Value));
                return await CommitAsync(batch, receiptId.Value, revisionId.Value, null, cancellationToken).ConfigureAwait(false);
            }
            ...
            batch.Create(GatewayCommandReceipt.Collection, receiptId, Receipt(
                command.NamespaceId, command.TargetNodeId, operation, command.IdempotencyKey, fingerprint,
                "accepted", receiptId.Value, revisionId.Value, selectedIntentId, selectedDesiredToken));
            return await CommitDesiredAsync(
                batch, desiredItem, command.NamespaceId, command.TargetNodeId,
                desiredId, receiptId.Value, revisionId.Value, selectedIntentId,
                selectedDesiredToken, cancellationToken).ConfigureAwait(false);
        }
        finally { _commands.Release(); }
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (!Enum.IsDefined(command.Kind))
            return new(GatewayManagementCommandState.Invalid, "management.activation-kind.invalid");
        await authority.InitializeAsync(cancellationToken).ConfigureAwait(false);
        await _commands.WaitAsync(cancellationToken).ConfigureAwait(false);
        try
        {
            ...
            string operation = command.Kind switch
            {
                GatewayRevisionActivationKind.ActivateExisting => "activate-existing",
                GatewayRevisionActivationKind.Rollback => "rollback",
                _ => throw new InvalidOperationException("The validated activation kind is unavailable."),
            };
            byte[] fingerprint = Fingerprint(
            ...
            if (priorReceiptResult.TryGetValue(out BaseRecord<GatewayCommandReceipt>? priorReceipt))
            {
                ...
                return new(GatewayManagementCommandState.Duplicate, "management.duplicate",
                    ...
                    priorReceipt.Value.StableRevisionId, priorReceipt.Value.StableActivationIntentId);
            }
            ...
            batch.Create(GatewayCommandReceipt.Collection, receiptId, Receipt(
                command.NamespaceId, command.TargetNodeId, operation, command.IdempotencyKey, fingerprint,
                "accepted", receiptId.Value, command.RevisionId, intentId.Value, selectedDesiredToken));
            return await CommitDesiredAsync(batch, desiredItem, command.NamespaceId, command.TargetNodeId,
                desiredId, receiptId.Value, command.RevisionId, intentId.Value,
                selectedDesiredToken, cancellationToken).ConfigureAwait(false);
        }
        finally { _commands.Release(); }
    }
    ...
    private static GatewayCommandReceipt Receipt(
        ...
        string result, string operationId, string? revisionId = null,
        string? activationIntentId = null, string? desiredStateToken = null) => new()
    {
        NamespaceId = namespaceId,
        TargetNodeId = targetNodeId,
        Operation = operation,
        ...
        StableActivationIntentId = activationIntentId,
