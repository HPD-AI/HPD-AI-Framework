...
internal static class GatewayAuthorityRecordIds
{
    ...
    internal static RecordId AdministrativeExecution(string namespaceId, string intentId) =>
        CommandFact("admin-execution", namespaceId, "execute", intentId, "v1");

    internal static RecordId AdministrativeArtifact(string namespaceId, string intentId) =>
        CommandFact("admin-artifact", namespaceId, "observe", intentId, "v1");
