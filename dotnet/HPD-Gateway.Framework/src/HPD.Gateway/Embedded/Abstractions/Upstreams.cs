...
[JsonDerivedType(typeof(ServiceDiscoveryEndpointSource), "serviceDiscovery")]
...
public sealed record ServiceDiscoveryEndpointSource : UpstreamEndpointSource
{
    public required DiscoveryProfileId Profile { get; init; }

    public required ServiceDiscoveryName Service { get; init; }

    public ServiceDiscoveryEndpointName? Endpoint { get; init; }

    public required ImmutableArray<ServiceDiscoveryScheme> Schemes { get; init; }

    public required DiscoveryStaleBehavior StaleBehavior { get; init; }
}

[JsonConverter(typeof(StrictStringEnumJsonConverter<ServiceDiscoveryScheme>))]
public enum ServiceDiscoveryScheme : byte
{
    Http = 0,
    Https = 1
}
