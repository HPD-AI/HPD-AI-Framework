...
internal static class GatewayConfigurationValidator
{
        ...
        ImmutableArray<GatewayValidationError>.Builder errors)
    {
        ...
        switch (source)
        {
            ...
            case ServiceDiscoveryEndpointSource discovered:
                ValidateId(discovered.Profile.Value, $"{path}.profile", errors);
                ValidateServiceDiscoveryName(discovered.Service.Value, $"{path}.service", false, errors);
                if (discovered.Endpoint is { } endpoint)
                    ValidateServiceDiscoveryName(endpoint.Value, $"{path}.endpoint", true, errors);
                ValidateEnum(discovered.StaleBehavior, $"{path}.staleBehavior", errors);
                if (discovered.Schemes.IsDefault || discovered.Schemes.Length is < 1 or > 2)
                {
                    Add(errors, GatewayValidationErrorCode.InvalidEndpointSource, $"{path}.schemes", "Service discovery requires one or two initialized schemes.");
                    break;
                }
                if (discovered.Schemes.Any(static scheme => !Enum.IsDefined(scheme)) ||
                    discovered.Schemes.Distinct().Count() != discovered.Schemes.Length)
                    Add(errors, GatewayValidationErrorCode.InvalidValue, $"{path}.schemes", "Service discovery schemes must be defined and unique; order is meaningful.");
                break;
    ...
    private static void ValidateServiceDiscoveryName(
        string? value,
        string path,
        bool endpoint,
        ImmutableArray<GatewayValidationError>.Builder errors)
    {
        var maximum = endpoint ? 63 : 253;
        if (string.IsNullOrEmpty(value) || value.Length > maximum || value.Any(static character => character > 0x7f))
        {
            Add(errors, GatewayValidationErrorCode.InvalidIdentifier, path, "Service discovery name is outside its ASCII length bound.");
            return;
        }

        var labels = value.Split('.');
        if ((endpoint && labels.Length != 1) || labels.Any(static label =>
                ...
                label.Any(static character => !IsAsciiLowerAlphaNumeric(character) && character != '-')))
            Add(errors, GatewayValidationErrorCode.InvalidIdentifier, path, "Service discovery name is not in canonical lowercase DNS-label form.");
    }
