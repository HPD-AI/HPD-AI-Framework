...
internal sealed record GatewayCanonicalizationResult
{
...
internal static class GatewayConfigurationCanonicalizer
{
    ...
    private static UpstreamDeclaration CanonicalUpstream(UpstreamDeclaration value) => value with
    {
        Endpoints = value.Endpoints switch
        {
            ...
            ServiceDiscoveryEndpointSource source => source,
            _ => value.Endpoints
        },
        Metadata = CanonicalMetadata(value.Metadata!)
    };
