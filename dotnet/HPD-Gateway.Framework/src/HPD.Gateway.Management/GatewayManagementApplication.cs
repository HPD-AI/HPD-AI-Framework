    ...
    IGatewayManagementReader reader) : IGatewayManagementApplication
{
        ...
        GatewayRollbackMutation mutation, CancellationToken cancellationToken = default)
    {
        ...
        return await commands.SubmitAsync(new GatewaySubmitCommand(
            mutation.NamespaceId, mutation.TargetNodeId, mutation.IdempotencyKey,
            mutation.Actor, mutation.CorrelationId, "rollback", mutation.RevisionId,
            mutation.Description, ImmutableArray.Create(revision.Value.CanonicalConfigurationUtf8),
            mutation.ExpectedDesiredStateToken, true, mutation.RevisionId), cancellationToken).ConfigureAwait(false);
    }
